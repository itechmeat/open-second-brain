...
/**
 * First leaked tool-call boundary: a closing XML-ish tag for a known
 * tool-call field, or an opening `<parameter ...>` attribute fragment.
...
/** One or more backslashes immediately before a double quote. */
const ESCAPE_CHAIN_RE = /\\+"/g;
...
export function sanitisePrinciple(value: unknown): string {
  ...
  if (leak) s = s.slice(0, leak.index);
