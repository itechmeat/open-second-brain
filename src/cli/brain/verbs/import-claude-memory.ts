...
export async function cmdBrainImportClaudeMemory(argv: string[]): Promise<number> {
  ...
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (a === "--vault") { const v = consumeValue("--vault", argv[++i]); if (v === null) return 2; vaultFlag = v; continue; }
    if (a === "--memory") { const v = consumeValue("--memory", argv[++i]); if (v === null) return 2; memory = v; continue; }
    if (a === "--dry-run") { if (modeSet && mode !== "dry-run") { process.stderr.write("o2b brain import-claude-memory: --apply and --dry-run are mutually exclusive\n"); return 2; } mode = "dry-run"; modeSet = true; continue; }
    if (a === "--apply") { if (modeSet && mode !== "apply") { process.stderr.write("o2b brain import-claude-memory: --apply and --dry-run are mutually exclusive\n"); return 2; } mode = "apply"; modeSet = true; continue; }
  ...
  try {
    ...
    if (err instanceof ConflictsError) {
      ...
      for (const c of err.conflicts) process.stderr.write(`  ${c.prefId} already exists in Brain but is not in Brain/.imports/claude-memory.json\n`);
      return 2;
    }
