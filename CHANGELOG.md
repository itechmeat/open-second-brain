...
Ten tracker items, picked by priority off the board and read against the real source before anything was designed, turned out to describe one defect seen from ten sides: the product declares a boundary and puts nothing behind it. An owner-isolation filter whose field no writer ever emitted. A gate that enforced whichever scope the caller asked for. A confidence floor compared against a number that could not vary with match quality. A shared namespace written twice over with attribution nobody read. An egress registry built to stop an unguarded export, with an unguarded export outside it. A census whose docblock claimed to have closed the mis-parse class that was live inside it.

Reconnaissance corrected most of the ten task bodies before a line was written. Six of the corrections changed what the unit is rather than how it is worded, two findings turned out to be the opposite of what was filed, and two of the ten described a live defect rather than an improvement: a credential leaking through a shipped export path, and an authorisation check that read back whatever the caller claimed. Each unit then takes one declared boundary and does exactly one of three things, chosen by evidence: ENFORCE it and put a census behind the check, RE-MEASURE it against the signal that actually varies with what the label claims, or RETRACT it and say so.

Retraction is a first-class outcome here rather than a shortfall. A knob that cannot change an outcome is the silent-fallback defect in configuration form: an operator sets it, reads no error, and believes a boundary exists. Deleting it is the honest disposition, and each deletion below carries the measurement that justified it.

### Added

- **Ownership is written, or it is not a boundary.** With `integrity.owner_scope_delivery` on, every production preference writer stamps the server-resolved agent identity onto a new record. A rewrite never re-owns, so a page created before the gate stays shared rather than being retro-owned by whoever rewrote it first, and an identity that cannot be resolved raises rather than guessing. With the gate off nothing is stamped and a vault that never opted in is byte-identical, measured with a whole-tree digest rather than asserted per file.
- **A caller-supplied owner scope is checked.** Under the gate, an `agent_scope` naming an owner other than the resolved identity is refused by name, never silently narrowed - a narrowing would leave the caller believing it had read what it asked for. The refusal is a four-piece closed vocabulary registered in the verdict census, and the argument has exactly one reader in the tree, so a tool cannot opt out by spelling the argument itself.
- **The scope matrix asserts correctness, not partition.** The bucket of tools classified as returning no owned content becomes `{name, args, reason}` entries, and one two-owner fixture drives all ninety-six of them and asserts no cross-owner marker reaches the response - or the thrown message. A bare name cannot regrow into the bucket: the argument list is required at compile time and again at module load.
- **Snapshot of what the delegated agent did.** `SubagentStop` is registered, and a session turn carries the delegated `agentId` and `isSidechain`, which are the only things separating a sub-agent's turns from its parent's - a sub-agent transcript reuses the parent's session id. Both fields are emitted only where the transcript records them, so a flat transcript keeps its pre-delegation shape byte for byte.
- **An unreachable search origin is reported rather than dropped.** A four-piece reachability vocabulary distinguishes reachable, unreachable and unknown, each carrying its own reason, because "it is not there" and "I could not look" have different repairs and neither may read as a zero contribution.
- **The `vault_path` contract has an enumeration behind it.** `src/mcp/tools.ts` states in prose what a returned `vault_path` may contain; forty-four sites emit the field and three honoured it. A census lexes every emission out of `src/mcp/` and requires it to come from the one producer, with nine positive controls and a stated list of what it cannot see. The producer moved to its own leaf module, because importing it back from the tool aggregator would have closed an import cycle the architecture ratchet gates.
- **`o2b brain snapshot`-adjacent honesty on the shared namespace.** The mirror's verdict stops being a two-piece type alias outside the audited population and becomes a registered vocabulary that no longer conflates operator misconfiguration with an I/O failure, and the shared namespace is enumerable as a read origin - which gives `origin_vault` its first reader anywhere in the repository.

### Changed

- **Four thresholds stop reading a number that could not vary.** The recall-inject floor, the recall-adequacy verdict, the gap-loop auto-close floor and the cross-vault chain stop all compared a constant against a result score. The keyword lane is min-max normalised inside the candidate set, so the top row of any non-empty recall scored exactly the configured `search_keyword_weight` - shipped at 0.6, identical to `recall_adequacy_sufficient`. Every keyword recall graded `sufficient / proceed`, `weak` and `insufficient` were unreachable, a gap task auto-closed on a hit of any quality, and the chain stop meant opposite things under the two fusion modes: unreachable in `linear`, always met in `rrf`. All four now read IDF-weighted coverage, which is absolute, pool-independent and already computed - and was thrown away one line before the return on every ordinary search. No threshold value changed; what they measure did.
- **`brain_recall_gate` and `brain_context_pack` take `match_quality` beside their scores.** The two stand or fall together and an incomplete pair is refused: the level is decided by the quality alone, and deriving one from the scores is the exact substitution this release removes.
- **The `tier:` frontmatter knob reaches the ranker.** `readTier`, the weight table and the `tierByDoc` ranker input all shipped together with nothing populating the map, so a page stamped `tier: peripheral` scored identically to an untagged one. The producer is the missing half. An untagged vault emits no tier map at all and its ranking is unchanged, pinned by an ordered `(path, score)` window over the committed benchmark corpus.
- **A scoped call on an unreadable config is refused rather than delivered.** An unreadable config resolves to the strict integrity fallback precisely so it cannot loosen a gate, only close one. A call naming an owner scope asks to be answered as that owner, and under the closed gate no such claim can be verified. The fail-soft contract keeps its meaning for the unscoped call, which still returns a pack and names the config to fix.
- **One lexer for every census that reads this repository's own source.** Four census files had grown their own copy; two were one implementation typed twice, and one of the others read a backtick inside a regex literal as a template opener, blanked 9,706 of 31,608 bytes of a shipped module and hid the only binding its census could not otherwise see. The extracted helper is the first in `tests/helpers/` with its own tests. The layering guard, which decided "is a comment" from a line prefix and was wrong in both directions, routes through it too.
- **Two output schemas name one constant instead of a bare string.** With the `vault_path` substitution, an unreadable device config makes the field a degraded value naming the breakage - and the server's own output-contract assertion then destroyed the whole payload on exactly the condition that value exists to report. The descriptor language has no union form, so both declarations reference one named schema constant defined beside the producer.
- **Census counts become equalities where a floor hid a drifted reality.** The direct continuity-store reader floor sat at twenty against a real twenty-one, and `docs/observability.md` had inherited the wrong number from it. A floor looser than its measurement is a measurement nobody is keeping.

### Fixed

- **An unredacted vault export.** `o2b brain explorer --export` wrote vault content to an operator-named file without passing the redactor every other export path uses, so a credential inside a preference left verbatim. The egress census missed it because its population rule keyed on a hardcoded list of destination flag names that did not include this one - the same defect class the census exists to catch, one level up. The derivation is widened and a synthetic flag proves it fails until a site is declared.
- **The egress registry could not express the largest egress in the product.** Its single population rule requires a file-destination parameter and a raw file write, so a network destination could not be declared under it at all. A second rule finds modules POSTing vault-derived bytes to an operator-configured URL, and the embedding endpoint is declared under it with a reason stating plainly that chunk bodies leave unscanned.
- **An export that could not parse a preference reported success.** `collectExportRows` continued past a parse failure, so an export silently omitted records. It now collects every failure in the directory - one run names all of them - and refuses by name with a non-zero exit. The bare `catch { return [] }` on the directory listing one line above, which reported a permission fault as an empty Brain, raises with the reason.
- **The redactor ate signal ids.** `sig-<date>-<slug>` matched the high-entropy token rule, so an exported preference lost the `_evidenced_by` link it points through. The carve-out follows the existing content-address precedent and keys on the shape rather than the prefix, so a credential wearing that prefix is still redacted. This also surfaced a disagreement: the CLI `--json` redactor has no high-entropy pass at all, so the two surfaces answered differently about the same string. They now agree, and the agreement is what the test pins.
- **`log.md` was empty on every default install.** The OKF export's date pattern matched only the legacy un-sharded log name while the writer has produced `<date>.<deviceId>.md` since per-device sharding landed. It now reads the recogniser in the module that owns the log layout, and shards of one day fold into one dated section.
- **`schema_inspect view=lint` and `view=orphans` died on the artifact they exist to report.** A retired preference missing a field, or a directory occupying a `.md` name, threw out of the scan - and the message carried an absolute host path. Both now report the artifact as an unreadable-artifact finding, with the path rewritten out of the message, and the orphan review carries those rows too: "nothing uses this token" is a claim over every artifact, and an unread one makes it provisional. The CLI renderer, which would have printed any new finding kind under the wrong label, is an exhaustive switch.
- **An absent argument was answered with a sentence about its shape.** `brain_labels` and `brain_tiers` answered a missing `path` with "path must be a vault-relative string", so a caller who sent the wrong argument name read it as a malformed value. Both use the shared required-argument helper now.
- **A backlink index reported a confident zero.** `buildBacklinkIndex` swallowed a parse failure with `catch { continue }` and returned `count: 0` for a legacy-frontmatter preference. The failures ride on the returned index, so all nine call sites compile untouched, and under an active owner scope an unparseable artifact is withheld from the report as well - naming it would leak what the ref filter just hid.

### Notes

Some of what was asked for is not here, and each absence is a decision with a measurement behind it.

There is no git-visibility ladder: there is no git transport in this product, every `git` invocation under `src/` is read-only, and the egress registry states in prose that the vault deliberately never will have one. A fail-closed check needs something to guard. There is no staging copy and no history purge: there is no history to purge, and the export bundle already is the staging copy. There is no `off` identity mode: the bottom of the identity chain is already the literal string `agent`, itself a placeholder, so `off` has no subject distinct from it. The `passthrough` / `managed` mode names are not adopted - passthrough is the shipped, ungated default across every tool that accepts a caller-supplied agent, and the owner-scope refusal is what makes it gateable. There is no per-agent instruction block: neither transport can reach one, since both resolve identity from process environment and config rather than from the request, so "per agent" collapses to "per process", which the installer already provides - and the config reader has no block-scalar form to declare a multi-line block in. The `shared` observation-scope member stays retracted: a positive scope no reader requests narrows visibility rather than widening it, and it is reconsidered when a reader asks for one by name.

`private: true` in frontmatter is retracted as a privacy claim rather than enforced. Nothing under `portability/` reads the key, the export composers are content composers rather than visibility filters, and the `<private>` region marker is the product's only content-derived privacy primitive. A test asserts that a `private: true` page exports in full, so the label cannot be re-read as a boundary by a later reader.

The keyword lane is not de-normalised. Measured blast radius: every score in the product moves and four thresholds flip at once. Gating the floors on a signal that is already absolute achieves the same thing without moving a single score.
