...
The silent fallbacks are the part with user-visible behaviour. A config file that is present but malformed no longer reads as a config file that is absent, and a diagnostic that could not examine a directory no longer renders as a diagnostic that found nothing wrong.
...
- An unreadable search index no longer narrows recall while looking healthy. The trigram candidate lookup caught every database error and returned an empty candidate list; its documented intent was only that the trigram table may be absent, and that is now the only condition it stays quiet for.
