...
- **An owner-only ACL for the secrets keyfile on Windows.** `icacls /inheritance:r /grant:r` grants access to the current user only, on the key directory (inheritable), the keyfile and an existing `secrets.json`. It is the Windows counterpart of the POSIX 0700 and 0600 modes. The grant names the user by SID, read from the process token with `whoami /user`, not by `%USERDOMAIN%\name`, which a parent process can rewrite and `icacls` may not map. It is applied on every load, once per process, not only when the key is created, so a secrets directory restored or copied in with a vault is brought to the same ACL. `icacls.exe` and `whoami.exe` run from `System32` by absolute path. Explicit SYSTEM and Administrators entries that the new directory already carries, as it does on GitHub's Windows runners, which run elevated, stay; `icacls` removes only inherited entries, and the threat model already counts administrators as able to read the key. It is best effort: a failed `icacls` warns and never fails the write.
