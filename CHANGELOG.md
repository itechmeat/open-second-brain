...
- **An owner-only ACL for the secrets keyfile on Windows.** `icacls /inheritance:r /grant:r` grants access to the current user only, on the key directory (inheritable) and on the keyfile. It is the Windows counterpart of the POSIX 0700 and 0600 modes. Explicit SYSTEM and Administrators entries that the new directory already carries, as it does on GitHub's Windows runners, which run elevated, stay; `icacls` removes only inherited entries, and the threat model already counts administrators as able to read the key. It is best effort: a failed `icacls` warns and never fails the write.
