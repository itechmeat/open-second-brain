...
- **Per-device shards for every ledger.** The shard grammar, listing, sync-conflict exclusion and device-id resolution move out of the log reader into `ledger-shards.ts`, byte-preserving for the log, and the continuity store, idempotency ledger, preference audit, metrics and session-lineage ledger write `<base>.<device-id>.jsonl` and merge every shard at read in a deterministic order. The bare name is the shard with the empty id everywhere, so no file is renamed and a device with an id never appends to a bare file again. The lineage ledger keeps one hash chain and one sequence per shard, compacts per shard, shards its gap sidecar the same way, and its verifier reports per shard path. The doctor's `sync-conflict-log` finding sweeps every ledger directory and names the one the copy sits in.
