...
- One digest encoding. Ten modules carried a byte-identical private SHA-256 helper and two reimplemented canonical JSON serialization independently, which was survivable while every digest stayed inside the process that computed it and stops being survivable the moment one is persisted: this release persists two, into a vault that replicates between machines. The two canonical-JSON copies disagreed about an object entry whose value is `undefined`, and the divergence was unreachable - every parser feeding the rendering copy builds its objects with a conditional spread - so no existing value changes.
