...
When a turn changed files but recorded no brain event, the Stop guardrail continued the turn with `decision: "block"`. Claude Code renders that as a red "Stop hook error" and prints the whole reason, about 25 lines, in the user's transcript on every guarded turn.
...
Several changes refuse input that earlier releases accepted. They are listed under Changed, and `docs/updating.md` has a 1.58.0 section with what to check and how to opt back where an opt-out exists.

### Security

- **Remote callers no longer receive private pages through side surfaces.** A page carrying the reserved `visibility: private` token is withheld at remote reach everywhere page content or page identity can travel:
  - `brain_context_pack` applies the reach rule to its candidates before ranking and budgeting, so a withheld page contributes nothing to any lane, the skipped list, `deduped_from` or the items. A tension that touches a withheld page is still reported, without its id.
  - `brain_writes` (`list`, `plan_revert`), `brain_labels show` and `brain_unlinked_mentions` ask the reach view per target. A withheld note is answered exactly like a missing one. `brain_writes` and `brain_unlinked_mentions` move to covered in the visibility registry.
  - The reserved token dominates its co-tags: a page tagged `visibility: [private, team]` is no longer returned to a caller whose scope names only `team`. `brain_search` refuses the reserved token as a caller-supplied scope at any reach.
  - Search and chunk drill-down honour both the page's current visibility and the visibility the index recorded for that path, so a public page written at the path of a deleted or moved private page does not surface the older text before the next index run.
  - A bare artifact id resolves against the resolved vault path, so the artifact visibility rule applies when the vault path is spelled with a trailing separator or a `.`/`..` segment.
- **Private session rows stay local.** At remote reach `brain_session_grep`, `brain_session_describe`, `brain_session_expand` and `brain_idea_lineage` withhold continuity rows flagged `private`, and the session recall tools also withhold the summary nodes built from them. A withheld row is answered exactly like a missing one.
- **Error text sent to a remote caller names no host paths.** Both the tool-level error envelope and the `INTERNAL_ERROR` channel replace the vault, home and temp roots (with their real-path and slash variants) by `<vault>`, `<home>` and `<tmp>` before the shared redactor runs. Local callers keep the paths their remediation needs.
- **One reading of paths backs every `Brain/` boundary check.** Backslash is a separator, `.` and `..` segments collapse, leading separators drop, and the `Brain` segment is compared case-folded, the way macOS and Windows filesystems resolve it. Graph import resolves paths before the `Brain/` boundary check in every mode and re-checks where the bytes land, so a vault folder symlinked into `Brain/` is refused, and a node whose landing path cannot be resolved is rejected while the rest of the graph imports. Note create, update, append and batch writes use the same reading and also re-check where the bytes land, refusing a target that resolves into `Brain/`, outside the vault, or not at all (a note used as a folder, a link loop).
- **Caller-named frontmatter writes answer to the write binding and stay on notes.** `brain_labels`, `o2b brain label`, `o2b brain attr` and marker write-back require a Markdown note, and under `Brain/` only the page lanes (`sources/`, `reports/`, `distillations/`). The operator config, standing rules and the other machinery files are refused, both as spelled and where the bytes would land. `apply-markers` reports a refused target as a per-marker `refused` verdict in report and apply mode alike, and the markers after it still run.
- **Write sessions commit only into the lanes that hold agent pages.** `brain_write_session` and `o2b brain session` accept a target under `Brain/sources/`, `reports/`, `distillations/`, `notes/` or `decisions/panels/`, read with the shared case-folded path reading, and refuse the rest of `Brain/`, including the standing rules and the active context. The commit re-checks where the bytes land, refuses the standing rules file by identity and applies the write binding, so a symlinked folder or note cannot carry a session into machinery.
- **Imported content arrives with the trust the operator gave it.**
  - OKF import validates every recorded path before joining it (relative POSIX, no `..`), so the review lane stays inside `OKF Review/`. The manifest's `producer` field is a label, not a credential: machinery frontmatter (`_status`, `owner`, `origin_channel`, `_revision`, `_confidence*`, `knowledge_pack` and the rest) is stripped on every import not run with `--trusted`, and the `Brain/sources/` and `Brain/reports/` lanes open on `--trusted` alone.
  - `bank-import` restores every preference row untrusted by default, including rows the bundle already marked unconfirmed.
  - Claude memory imports land `unconfirmed` on a real trial window instead of confirmed at high confidence.
  - `brain_update_note` refuses the reserved frontmatter keys `visibility` and `origin_channel` with `reserved_frontmatter_key`.
  - `brain_ingest_source` reads a `pre_extract` source only inside the vault; anything else is reported `extracted: false`.
  - Snapshot restore checks every extracted symbolic link, including one whose target does not exist yet, and restores only links that stay inside the extracted tree.
- **Every content writer answers to the freeze.** `brain_tiers` restore and accept, and `o2b brain links --write`, now refuse on a frozen vault like every other writer; the links dry run stays available.
- **Secret custody.** The secrets directory carries a `.gitignore`, so a vault that is a git repository never commits the keyfile or the ciphertext. On POSIX the owner-only modes of the keyfile and the store are re-applied on every load. On Windows `secret set` refuses when the owner-only ACL cannot be applied, and the first `secret set` in a fresh vault works. A `.gitignore` does not reach Syncthing, which reads only the `.stignore` at its folder root: the new `secrets-sync-exposed` doctor check, and a warning on `secret set`, name the exact `.stignore` line when a Syncthing folder would carry the directory to its peers. The check reads real paths, so a vault opened through a symbolic link is checked against the folder its target sits in. Neither edits `.stignore`. An unknown secret name no longer lists the stored names in its error.
- **`brain_secrets run` matches its allowlist against the whole argv.** Pattern tokens align element by element with the argument vector, and returned output also runs the bare-token redaction pass, so an encoded echo of the value is scrubbed too.
- **Credentials travel only where the operator configured them.** Embedding and rerank endpoints must be https (loopback excepted), and all three provider transports refuse redirects. The research fetch helper refuses non-https URLs and private, link-local, CGNAT and unspecified IP literals before it attaches a key, and does not follow redirects.
- **An unknown `--tool-profile` fails closed.** `o2b mcp` exits 2 naming the bad value, where it came from and the known profiles, instead of serving the full tool surface.
- `parseBrainYaml` refuses `__proto__`, `constructor` and `prototype` keys at every level, and the snapshot manifest sidecar validates its run id the way the snapshot path does.

### Added

- **The payload registry, wired end to end.** Session recall import runs every turn through the registry before the continuity row is written. Data URIs and base64 runs longer than `sessions.payload_max_inline_chars` (default 512), and text longer than `sessions.payload_max_text_chars` (default 32000), move to `Brain/.payloads/<sha256>.txt`. The row keeps a 1,000-character head preview and an `osb-payload://` placeholder. Every stored payload is private-region-stripped and redacted before it is written. A turn that needs no externalization imports byte-identically.
  - `o2b brain payload get <ref> [--offset N] [--limit N]` pages the exact stored content; `list` shows stored payloads with reference counts, orphans and missing files; `gc` is a dry run unless `--apply`, which removes only unreferenced payloads older than ten minutes, behind a `payload-gc` recovery point, re-planning inside it under the payload store lock that session import also holds from writing a payload to appending the row that names it. Younger orphans are reported as deferred, and an import that finds its payload already stored refreshes the file's age.
  - `brain_session_expand` gains a payload mode (`payload`, `payload_chars`, `cursor`) that pages a placeholder's content. Remote callers read only a payload that a non-private session turn references, directly or through another remotely readable payload. Pages, the Brain log included, keep a payload alive for the gc but grant no remote read.
  - The doctor reports `payload-orphan`, `payload-missing` and `continuity-row-oversized`. Snapshots archive the store, the search index never admits it, and bank and OKF exports carry placeholders, never payload bytes.
- **Knowledge packs,** the install and uninstall follow-up 0.28.0 left open. `o2b brain knowledge-pack export|preview|install|uninstall|list` packs a selected subset of rules and pages into an OKF-based bundle sealed with a sha256 per file and a digest over name and version. Export blocks pages scoped by `visibility:`, carrying an `owner:` or still under review, and redacts through the shared egress guard before sealing. `preview` reports samples, integrity, conflicts and injection or privacy warnings. `install` refuses a tampered pack and lands everything untrusted: rules on a fresh trial window with the source vault's evidence, counters and pins cleared, pages staged in `OKF Review/`, each stamped `knowledge_pack: <name>@<digest>`. `uninstall` removes one pack's entries and keeps rules that gained local evidence, pages an operator promoted, and staged pages whose content changed since install (reported as `edited`, against a `knowledge_pack_sha` fingerprint the installer records on each staged page). The `knowledge-pack-uninstall` snapshot archives `Brain/` only, so it holds the removed rules; a removed staged page is one the operator never changed, and installing the pack again restores it. Installed provenance shows in `brain_search` trust metadata (`trust.knowledge_pack`) and `o2b brain query`. CLI only, because `preview` reads a directory outside the vault.
- **`embedding_allow_insecure_http` and `search_rerank_allow_insecure_http`** (and their `OPEN_SECOND_BRAIN_*` environment forms) accept plain http for that one endpoint, for a local server on another machine such as LM Studio on the Windows host seen from WSL or Ollama on the LAN. Both default to false, apply only to a base URL from the operator's config or environment, never to one from the in-vault provider registry, and warn once per endpoint.
- **`OPEN_SECOND_BRAIN_MCP_API_KEY`** supplies the MCP HTTP bearer when `--api-key` is absent, so the key stays out of the process list.
- **`bun run check:hermes-scan` and a `hermes-plugin-scan` CI job.** `hermes plugins install` scans the whole clone and blocks a dangerous verdict with no override ([#188](https://github.com/itechmeat/open-second-brain/issues/188)). The script fetches Hermes' plugin scanner at a pinned commit, checks its digests, and scans the repository root and `plugins/hermes` the way an install does.

### Changed

These can change what an existing setup sees. `docs/updating.md` covers each one.

- **Embedding and rerank endpoints must be https** unless the host is loopback or the endpoint's `*_allow_insecure_http` opt-out is set in the operator's config or environment.
- **An unknown tool profile exits 2.** `--tool-profile` or `mcp_tool_profile` naming a profile that does not exist stops `o2b mcp` instead of serving every tool.
- **`bank-import` resets every preference row by default:** unconfirmed, unpinned, low confidence, on a fresh trial window dated from the restore, and each reset row is named. `--trusted-restore` keeps the carried state for a backup you vouch for.
- **OKF import strips machinery frontmatter unless `--trusted`,** whatever the bundle's `producer` says, and only `--trusted` admits the `Brain/sources/` and `Brain/reports/` lanes.
- **Labels and attributes refuse `Brain/` machinery and non-Markdown targets.** Under `Brain/`, only `sources/`, `reports/` and `distillations/` pages accept them.
- **Write sessions commit only under `Brain/sources/`, `reports/`, `distillations/`, `notes/` and `decisions/panels/`.** Any other target under `Brain/` is refused with `target-reserved` when the session opens.
- **A top-level folder named `brain` in any letter case is treated as `Brain/`.** Note writes and graph import refuse it as they refuse the machinery root. On a case-sensitive filesystem that holds a separate user folder of that name, rename it.
- **Every existing search index is rebuilt once,** in the background at the next start, because its chunks were cut by older chunking rules (see Fixed). With embeddings on, the chunks that changed are re-embedded, which includes multi-chunk notes in every language, not only text in scripts written without spaces.
- **`brain_secrets run` allowlist patterns match the argv element by element.** Each space-separated pattern token matches one argument, `*` inside a token globs within that argument, and a trailing `*` token means one or more further arguments. A pattern such as `tool*` that relied on matching across the joined command line now matches only a single-element argv; write `tool *`.
- **`brain_update_note` refuses `visibility` and `origin_channel` in its frontmatter.** Edit them in the file.
- **Claude memory imports land unconfirmed** and go through the dream pass like every other candidate.
- **Knowledge-pack preview lives in portability.** The never-wired `src/core/brain/packs/pack.ts` preview is folded into `previewKnowledgePack`, with its guarded samples and integrity hash.

### Fixed

- **The prompt hook no longer reads the inbox for a prompt with nothing to capture.** `session-capture` built the signal dedup index, a read and parse of every `Brain/inbox/sig-*.md`, before it knew whether the prompt carried a marker or a fact. On a vault with about 4000 signals on WSL's mount of a Windows drive that took 6 to 12 s and ran into the UserPromptSubmit hook's 10 s host timeout. The index is now built on first use and shared by the rest of the event; dedup is unchanged when there is something to capture.
- **Chunking for scripts written without spaces.** The chunker counted whitespace-separated words, so a paragraph of Chinese, Japanese, Thai or similar text counted as one token and a long note became one chunk that embedding providers reject. Han, Hiragana, Katakana, Hangul, Bopomofo, Thai, Lao, Khmer, CJK punctuation and fullwidth forms now count one token per character, a block too large for one chunk is split by line, then after sentence punctuation, then at a token boundary, and every chunk stays within the cap. Short space-separated notes chunk identically; longer notes may split at slightly different points because overlap now counts toward the cap, and their changed chunks are re-embedded. The index records `chunker_version`, and the startup self-heal rebuilds an index cut under older rules once. Reported by [@Iversondwx](https://github.com/Iversondwx) in [#186](https://github.com/itechmeat/open-second-brain/issues/186).
- **A failed reindex swap puts the previous index back.** When the final rename fails after the live index was moved aside, the old index is restored before the error is raised, instead of leaving no index until the next open. From the analysis by [@rayblennerhassett-byte](https://github.com/rayblennerhassett-byte) in [#187](https://github.com/itechmeat/open-second-brain/issues/187).
- **`o2b search reindex` on a vault that is not there refuses** with `vault not found` instead of creating the directory with an empty index. The detached self-heal child no longer recreates a vault that was moved or deleted after it was spawned.
- **Hermes installs are no longer blocked by the plugin scanner** ([#188](https://github.com/itechmeat/open-second-brain/issues/188)). The lines Hermes' scanner flagged were all prose, and are reworded without changing their meaning. The one runtime string among them, the reason text of the context guard's instruction-override detector, now says "disregard" where it said "ignore"; its code and pattern are unchanged.
- **OpenClaw no longer calls Bun inside its Node bundle.** A contended first use of the installation secret or device id slept with `Bun.sleepSync`, which is undefined under Node and threw out of `second_brain_status`. Both waits use the runtime-neutral sleep, and a test drives the committed bundle under a real `node`.
...
- **`src/core/brain/schema-contracts.ts`, the TypeScript mirror of `schemas/brain/*.schema.json`.** Nothing in production read it. The 0.17.0 entry described it as a registry "used by tests and agent-facing envelopes"; only its own tests used it, and it had drifted from the published files. A contract test now runs the report generators and the MCP payloads of `brain_intent_review`, `brain_retention` and `brain_brief view=monthly` against the published schemas, and a new schema file fails the suite until it gets a generator check. No drift between the generators and the published schemas was found.
- **Two other modules nothing called:** `activityMarker()` in the activity-line renderer and the `mock-embedding` test helper.

### Internal

- **A temp entry that outlives the test run fails it** ([#194](https://github.com/itechmeat/open-second-brain/issues/194)). The test preload points `TMPDIR`, `TMP` and `TEMP` at one per-run root and fails the run naming every leftover (a warning on Windows, where a just-exited process can still hold a handle). Every test file that minted temp directories or pinned `HOME` now removes them. Closes #194.
- **Flaky tests fixed:** lock races retry a write the lock refused, so they count only lost updates; the live explorer test waits for its listening line; hook tests wait for the reindex child they start; the state-surfaces suite releases the writer lock it takes; the capability-tier provider probe hits a server that answers; context-pack round trips follow the preview envelope.
- **Weak pins replaced by measured ones:** the advertised MCP tool list is pinned by name set against the tool table, note-lifecycle and scaffold-stub flags ratchet against what each verb parses, the advisory claims are driven through the CLI, and the OpenClaw per-turn reminder is driven through the real plugin entry.
- **A cleanup pass first removed the payload registry and the knowledge-pack preview as dead code.** Both were unfinished features, so they were restored and finished instead (see Added), and the write-site census attribution was corrected with them.
- **Every workflow action is pinned to a full commit SHA,** with the version as a comment, and a test keeps a new action from arriving by tag.
- **The opencode plugin's spool directory is chosen by an if/else chain** rather than a nested ternary, with tests for each fallback.
- **The git hooks check the working tree they are committing from,** and lint and fmt skip `.claude/worktrees/`.
- **An agentic-playbooks workspace** (`.apb/`) holds this repository's release and PR-preparation playbooks. Run state stays ignored, as do the local `.zvec-grep/` index and `.ai-notes/`.
...
- **`.cmd` launchers.** `scripts\o2b.cmd`, `o2b-mcp.cmd`, `o2b-hook.cmd`, `vault-log.cmd` and `o2b-aider.cmd` sit beside their bash twins.
  ...
  - They never run a `bun` from the current directory. cmd.exe looks for a bare command name in the current directory before PATH, and an agent host starts them in the project it opened, so a repository that ships a `bun.cmd` would have run it. Each launcher sets `NoDefaultCurrentDirectoryInExePath=1` right after `setlocal`, and turns delayed expansion off so a `!` in the checkout path stays a character.
  - The hook launcher keeps the fail-soft contract and always exits 0. It passes an empty `""` argument through instead of stopping at it, and reports a hook that exits with a negative code (a crash) as well as a positive one.
...
- **`o2b install-cli` on Windows** writes `.cmd` launchers into `%USERPROFILE%\.local\bin`, because a symlink there needs Developer Mode or an elevated token.
  ...
  - A `%` in the checkout path is doubled on the launcher's command line, where cmd.exe would expand it. A launcher of ours whose command line is not the current body is rewritten and reported as `updated`.
  - When the directory is not on PATH, the verb says so and names the Environment Variables dialog (`rundll32.exe sysdm.cpl,EditEnvironmentVariables`). It prints no command that rewrites the value: Windows PowerShell's `SetEnvironmentVariable` stores the user Path back as `REG_SZ` with its `%VAR%` entries expanded.
- **MCP host configs on Windows** start the server as `cmd /d /c o2b mcp ...`, the `cmd /c <name>` form several hosts document for `npx`.
  ...
  - Every Windows entry carries `NoDefaultCurrentDirectoryInExePath=1` in its `env`. Without it cmd.exe resolves `o2b` in the current directory first, and a host starts its servers in the project it opened, so an `o2b.cmd` committed to any repository ran at MCP start. An absolute launcher path was the alternative; cmd's quote stripping breaks it for a profile or vault path with a space.
  - Because hosts quote only arguments that contain whitespace, the Windows payload refuses a vault path containing `& | < > ^ % " !` by name, rather than writing a config that cmd would split, expand or execute at spawn time.
...
- **An owner-only ACL for the secrets keyfile on Windows.** `icacls /inheritance:r /grant:r` grants access to the current user only, on the key directory (inheritable), the keyfile and an existing `secrets.json`. It is the Windows counterpart of the POSIX 0700 and 0600 modes. The grant names the user by SID, read from the process token with `whoami /user`, not by `%USERDOMAIN%\name`, which a parent process can rewrite and `icacls` may not map. It is applied on every load, once per process, not only when the key is created, so a secrets directory restored or copied in with a vault is brought to the same ACL. `icacls.exe` and `whoami.exe` run from `System32` by absolute path. `icacls /reset` runs first, because `/inheritance:r` removes only inherited entries: an explicit entry for another account, which a directory restored with its ACL can carry, would otherwise survive. Where SYSTEM or Administrators entries remain, as they can on GitHub's elevated Windows runners, the threat model already counts administrators as able to read the key. It is best effort: a failed `icacls` warns and never fails the write.
- **A `windows-latest` CI job** runs the typecheck, the full TypeScript suite, the Hermes plugin's Python tests and the `.cmd` launcher, with a 75-minute timeout. Like the Linux job, it asserts that the Hermes anti-drift test actually ran rather than skipped. The suite runs the generated and checked-in launchers through a real `cmd.exe`: end to end from the MCP payload's command, and with a `bun.cmd` and an `o2b.cmd` planted in the working directory, which must not run.
...
- **The Hermes memory provider on Windows** starts the MCP bridge without a console window, which matters because the Hermes desktop app is a GUI process. Its last-resort branch resolves `o2b.cmd` to an absolute path, because `Popen(["o2b", ...])` cannot start a batch file. Its executable lookup ignores a `bun` or `o2b` that `shutil.which` finds in the current directory, which Windows searches before PATH, unless that directory is on PATH itself.
- **`bun install` works on Windows.** The inline `sh` prepare script is now `scripts/prepare.ts`, run as `bun scripts/prepare.ts || exit 0`, so an `npm install` from a git URL on a machine without Bun still never fails on it.

### Fixed

- **Snapshot member order was not deterministic.** Tar walked each directory in `readdir` order, which is hash order on ext4 and name order on NTFS and APFS, so two copies of the same tree archived their members in different orders. Every member is now listed explicitly, depth-first in byte order, and handed to tar with `--no-recursion --null -T`, which GNU tar and bsdtar both accept. The member order is deterministic; the bytes are not, because tar headers still carry mtimes and owners.
  - The walk fails the snapshot on any error other than a vanished entry. With `--no-recursion`, a directory it could not read would otherwise be archived empty, and bsdtar (macOS, Windows `tar.exe`) reports success for that.
  - Member names travel as raw bytes, so a file name that is not valid UTF-8 (a latin1 `café.md` on Linux) is archived under its real name instead of failing GNU tar.
...
- **A lock file being deleted by another process** reported EPERM on Windows, which failed a concurrent ingest instead of waiting. It is now treated as "lock busy". An EPERM with nothing at the lock's name is a directory that refuses to create files (an ACL, Controlled Folder Access), and it fails at once with its own error instead of waiting out the budget as "lock busy".
- **The search index swap deletes its `.bak` and `.new` files with the same Windows retry as its renames,** so a stale file held by an antivirus scan no longer fails the reindex.
- **`closeDatabase` never throws.** If the fallback lazy close throws too, the error is dropped, so a caller's `finally` cannot replace the original failure.
...
- **Verified on a real Windows 11 machine,** not only in tests:
  ...
  - **The current-directory hijack:** before the fix, `cmd /d /c o2b --version` run from a directory holding a planted `o2b.cmd` ran the planted file, and the absolute `scripts\o2b.cmd` ran a planted `bun.cmd`. After it, both reach the real CLI.
...
Every card in scope was read against the live source before anything was designed, and three of the five verdicts amended the card. The version card asked to reuse an existing `packageJson.version` import and understated the problem: three independent reads of that field existed in the tree, so adding a fourth would have written the defect into the change meant to answer it. The linked-projects card is smaller than it states - both readers are already pure, typed and tolerant, and the absolute-path question it raises is answered by the shipped `expose_host_paths` contract rather than by a new rule. The connector-health card is largely refuted: per-target status is already a closed vocabulary, an unverified probe is already a named member carrying its reason rather than an assumed success, the deadline is already enforced at ten seconds per probing target, and the aggregate view its own validator believed was the remaining gap already ships as `o2b install --check`. What survives is narrower and of a different shape than the card states, and it is stated in the Notes below along with the one card that was refused outright and not built.

### Added

- **`o2b version`, and `--version` as its synonym.** The verb prints the installed version and exits 0; `--json` renders `{"version":"…"}` for a caller that parses. The root flag is routed to the same renderer rather than printing a line of its own, because two spellings of one question that answer differently are worse than one spelling - the difference is invisible until someone pastes the wrong one into a bug report. Both are declared in the command manifest, so shell completions and `o2b help --json` offer them. `o2b version latest` is a usage error rather than a print of the local version: nothing in this build learns what any other version is, and answering that question with the installed number would be the wrong answer wearing the right shape.
- **`second_brain_wiring`: what this install is wired into.** One read-only tool with a required `view` and no aggregate member, so a projects read never pays for a host probe and every payload stays bounded - the same idiom `brain_brief`, `brain_analytics` and `schema_inspect` already use, for the same token reason. `view=projects` returns one entry per registered project link carrying the four-value pointer state and whether the vault it names still exists, off `linkedProjectsStatus`, which had one consumer and none under `src/mcp`. `view=hosts` returns one entry per registered install target carrying the target, the `VerifyStatus` member, the adapter's details and its fix hint, from the same `verify()` call over the same registry and the same `InstallEnv` that `o2b install --check` makes. An absent or unknown view is refused with `-32602` naming the accepted members, and so is a `hosts` read with no resolved vault. Paths do not travel: `project_ref` and `vault_ref` render through the shipped `expose_host_paths` contract, and because the hosts view carries the adapter's own sentences - which `verify()` composes from the host home, where no store reference applies - the home prefix in those is folded to `~` under the same flag. An MCP response lands in model context, so the payload that hashes the vault path must not name the operator's account beside it.
- **The handshake instructions are rendered from the live capability report.** This server has evaluated a runtime capability window in its constructor for several releases and knows, per tool, whether it is available or withheld and why. `initialize.instructions` did not read it. Guidance whose tool this runtime withheld is now REMOVED rather than contradicted - an instruction and its retraction in one document leave the reader deciding which half is true, and the half printed first is the half that shapes the plan - and a block at the end names each removed tool with the reason the evaluator produced, inventing no second vocabulary. Each scope body is an ordered list of paragraphs of tool-tagged segments; a segment renders when every tool it names is available, the separator of the preceding rendered segment joins the survivors so a dropped clause takes its punctuation with it, and a paragraph whose every tool-bearing segment dropped renders nothing at all, its lead-in included, because "Five tools live here" is a claim that stops being true the moment one of them is withheld.

### Changed

- **One version constant, and the tree's only manifest import.** `src/core/version.ts` exports `OPEN_SECOND_BRAIN_VERSION`; the continuity export header, the MCP handshake and the stamped opencode plugin read it. A census over `src/` holds the import to exactly one module - an equality with a positive control, not a floor - so the fourth copy cannot appear quietly. The same import was previously written two ways, with and without the `with { type: "json" }` attribute, and is now written once.
- **The install environment is built in one place.** `buildInstallEnv` moved from the `o2b install` verb into `src/core/install/env.ts`, so the MCP hosts view verifies the same machine the CLI does. Two builders would be two answers to which machine is under verification, which is the split that constructor's own documentation was written to close. The CLI verb keeps its argument resolution and delegates the construction, and the sentence a run with no vault refuses with is now one constant both surfaces read.
- **The path policy generalises rather than repeats.** `vaultPathField` is now `hostPathReference(path, source)` applied to the vault; `vaultStoreReference` never required the path to be the vault. Both sides of a project link render through it, so `project_ref` and `vault_ref` follow the same `expose_host_paths` contract `vault_path` obeys - an opaque `vault://<hex>` by default, the raw path when the operator sets the flag - and a config that cannot be read renders `{ "error": … }` rather than degrading to the path the reference exists to hide.
- **The instruction block's fully-available render is byte-identical to what shipped before,** pinned against fixtures captured from the strings it replaces, so a reshaping of three bodies cannot become a rewrite of their meaning without a test saying so. One sentence in the `catalog` body is the declared exception, and it is listed under Fixed below. A segment naming a tool its scope does not register fails an orphan census in the same file, so a renamed tool cannot silently take its guidance with it.

### Fixed

- **The dream observer-equivalence test normalised a field but not the hashes derived from it.** `attaching an observer changes nothing the pass writes` compares two vault trees and already normalised the snapshot archive's `size_bytes`, which is nondeterministic because a tar carries the mtimes of the tree it was taken from and the two copies straddle a second under load. The day log's chain hashes are computed over the payload including that real size, and every later line chains off the first through `prev`, so one straddled second re-keyed the whole file in the two places the field had just been removed from. It passed on every developer box and failed on a loaded CI runner. The chain fields are now normalised alongside the size, with the reason stated: the chain has its own end-to-end integrity suites, and what this test asserts is that the observer changed nothing the pass authored.
- **A before-image stamped in the future was never pruned.** `pruneWriteImages` measures each image with `fileAgeMs` and floors the result to whole days. `fileAgeMs` deliberately does not clamp - a file stamped ahead of the clock is a real condition it reports rather than hides - so an age of -1ms became -1 whole day, which is below every retention window including zero, and the image was kept forever. The condition is ordinary rather than exotic: `nowMs` is sampled once before the sweep, and mtime granularity on a synced or networked filesystem rounds a write made just before it up past that sample. It turned CI red on one image of two. The sweep now floors a non-positive age to zero at the call site, where the floor is part of retention's meaning rather than of the measurement, and a test stages the future-stamped image deterministically instead of waiting for the race.
- **`view=projects` returned an unbounded list.** The tool takes a preview-budget exemption whose reason claimed both views were bounded by construction. That is true of `view=hosts` - one entry per adapter in a registry this build closes - and false of `view=projects`, which reads a registry nothing caps: every `o2b brain project link` appends to `projects.json`, and the serialized response grows with it, inline, into model context. The list is now capped at twenty-five and carries `total`, `returned` and `truncated` beside it, the same self-declaring shape the page-lint report uses, so a capped answer cannot be read as a complete one. Capped rather than budgeted because the preview budget is per tool and not per view: at ~2000 characters it would park the hosts view, which renders around 1700 with nothing installed, in an artifact on nearly every call. The exemption reason now states what actually bounds each view.
- **The unresolvable path reference reported the config path into model context.** When the device-local config cannot be read, `vault_path` - and now `project_ref` and `vault_ref` with it - degrade to `{ "error": … }` rather than raising or falling back to the raw path. The reason carried was `ConfigReadError`'s own message, which names the config file twice, once in the condition and once in the `chmod` remedy. That message is right on the four channels it was written for; this is not one of them. It is the field forty-five tools emit precisely so that an absolute path under the operator's home does not travel into model context, so reporting the condition by naming that path reintroduced, in the degraded branch, the leak the resolved branch prevents. The field now carries a path-safe reason and points at `second_brain_status`, which names the file from a field whose contract is to name it.
- **The catalog handshake told the agent that a withheld tool was still callable.** The frozen string said every tool omitted from `tools/list` stays CALLABLE through `tools/call`, which is true of the tools the catalog surface hides and false of the ones a runtime capability window withholds: the window removes them from the very table `tools/call` routes on. Carrying that sentence unchanged into a rendering whose whole subject is the window would ship a handshake contradicting the runtime it was rendered for - the defect this release exists to remove, one paragraph over. The sentence now separates the two cases, the pinned `catalog` fixture carries the corrected text, and the exception to byte-identity is stated in the fixture test, in the module that renders it and in `docs/mcp.md`.
- **Two places said the catalog advertises seven tools without saying when.** `evaluateToolCapabilities` applies the runtime window before counting, so a window withholding one of those seven lowers `advertised_tool_count` with it. The `advertised_tool_count` docblock and the matching `docs/mcp.md` paragraph now state that seven is the no-window count, and name the capability diagnostic as the one tool no window withholds.
- **`docs/mcp.md` said the full server advertises 111 tools.** It advertised 114 before this release and 115 after. Two further counts in the same file were stale in the same way and are fixed with it: the full surface was also given as 49 in the `.mcp.json` section, which now points at the one place the number is stated, and the `catalog` gap was given as a figure that goes stale on the next tool added. That one is restated as the relation it always was - the catalog advertises seven whatever the tool table holds - in the doc and in the `advertised_tool_count` docblock it mirrors, so there is no number left there to drift.

### Notes

- **Release awareness was scoped, verified against the source, and not built.** The card asked for a surface reporting whether a newer version is available "by reading only cached update state". There is no producer for that cache: a search across `src/` for `latestVersion`, `latest_version`, `update-check`, `updateCheck` and `registry.npmjs` returns nothing, so nothing in this tree ever learns what the latest release is. Filling the cache requires a network check, and `renderDataOwnership` in `src/core/install/ownership.ts` renders to the operator, verbatim, that this tool has no server it phones home to, no sync endpoint and no update check. Building the card as written means either making that statement false or shipping a surface whose only possible answer is that it has nothing to report. A collision the card did not see: `o2b update` already exists and means re-applying managed install blocks to host configs, so the obvious verb is taken and the obvious name would mislead about an existing one. The two honest alternatives - registering an outbound release check in the egress registry as a network site like every other outbound path, or reading a version fact some other component already fetched and saying where it came from - both amend an operator-facing promise about network behaviour. That is an operator policy decision rather than an engineering one, and it is recorded here for the operator to take rather than taken on their behalf. The upgrade-plan half of the card needs nothing: `src/core/brain/upgrade.ts` is already a plan and apply migrator.
- The `hosts` view may ask host CLIs, and its description says so. Two of the registered targets declare a host probe, so a worst-case call adds two subprocess round trips, each bounded by the ten-second deadline `--check` already enforces; a diagnostic that can take twenty seconds and does not say so is a surprise. A probe that could not run renders its named reason instead of an assumed success, which is the property the probe module already held and this view carries through unchanged.
- An unresolved vault is refused by name on the `hosts` view rather than reported as ten absent runtimes. `verify()` reads the per-vault sidecar manifest, so an unset vault produces `not-installed` for every target off a path that does not exist - a wrong answer wearing the authority of a completed check.
- The withheld block in the instructions lists the tools that text would have instructed, not the whole withheld set: a `--max-tools` window can withhold over a hundred tools, and pasting that list into the first thing every agent reads would cost more than the guidance it explains. It points at `second_brain_capabilities` on the surfaces that register it. No capability window ever withholds that tool, but the writer scope never registers it at all, and a handshake sending an agent to a tool its own server answers `unknown tool` for would be the defect this rendering removes, reintroduced by the sentence explaining the removal; there the block ends with the names.
- One test limit is stated rather than implied. `os.homedir()` in this runtime does not follow a later `process.env.HOME`, so an in-process test cannot redirect the home the `hosts` view resolves. The named-probe-skip case therefore builds the `InstallEnv` by hand, as the adapter suites do, and asserts the view's own mapping over the real `verify()` answer; a separate deep-equality test is what ties that mapping to the environment the view builds.

## [1.55.0] - 2026-09-10

Open Second Brain has known who wrote a signal or a preference since the log shipped, has taken a snapshot before every destructive pass since 0.10, and has sharded the Brain log per device since 0.10.8 so that two machines syncing one vault never fight over one file. What it could not say was who wrote a note. A `brain_create_note`, an `update_note`, an `append_note` or a `write_batch` left no event in the log and no row in any roster, and the bulk of what an agent produces was therefore invisible to attribution, to `brain_event_trace`, and to any rollback finer than the whole `Brain/` tree at a snapshot instant. This release closes that, and the four things that follow from closing it. Every note write now emits one `note-write` event carrying a write id, the operation, the target, the content hash and byte size on both sides, the caller-asserted agent and the server-derived origin channel, with the device riding on the shard name as it does for every event, and the bytes the write replaced kept in a content-addressed store so that a revert has something to restore from. A revert is a plan over those events for one agent, device or path in a window - restore, delete, or a refusal by name when another writer sits on top - sealed by a digest, applied only as the exact plan the operator read, under a snapshot, and recorded as a write of its own so that a revert is itself attributable and revertible. A freeze marker at `Brain/.state/frozen.json`, carried by Syncthing to every device, is refused by name at the one guard every content writer already passes, while the Brain log keeps recording so the refusals are auditable; `o2b brain freeze` and `o2b brain unfreeze` are the operator verbs and `o2b brain status`, `brain_status` and `brain_doctor` all say so. The shard rule the log has followed for a year is now the rule for every append-only ledger in the vault - continuity, idempotency, preference audit, metrics and session lineage each write `<base>.<device-id>.jsonl` and merge every shard at read, and the doctor names a sync-conflict copy under any of those directories, not only the log. And because the log is now the attribution record for every write, every JSONL line it appends carries a per-shard hash chain: a line that was edited, removed or stripped after it was written has a file and a line number, reported by `o2b brain log verify` and the doctor and never used to gate a read.

The wave was scoped from a comparison against a published control-model checklist for agents with delegated authority whose fleet layer - a kill path, a causal log attributable per agent and per machine, and rollback - is described in prose and not shipped. The comparison found that Open Second Brain already had most of the described log and none of the described stop, and that its own log discipline had not been applied to its own other ledgers; the five kanban cards behind this release are those findings, each verified against the live source before anything was designed.

### Added

- **Every note write is an event.** `createNote` and the write-batch kernel's `update_note` and `append_note` commits each append exactly one `note-write` log event after the bytes land: `write_id` (`nw_<14 timestamp digits>_<16 hex>`, derived from the event body so two devices that record the same write agree on its id), `op`, `target`, `hash_before` (the literal `absent` when the target did not exist, deliberately not the hash of zero bytes), `hash_after`, `bytes_before`, `bytes_after`, and `agent`. The bytes an update or append replaced are kept at `Brain/.state/write-images/<sha256>`, written before the atomic write and skipped when an image of that content already exists, so a note toggled between two states costs two files and not two per write. A log failure never fails a write that has already landed; the result and the MCP receipt carry `write_id: null` and an `audit_reason` instead, and a create that skipped an occupied target records nothing because it authored nothing.
- **`o2b brain writes` and `brain_writes`.** The record read back: filter by agent, device, path, window and operation, newest first, with the device reported as the shard the event came from. `brain_agent_query` and `brain_agent_diff` gain the `note` contribution kind off the same events, so "which notes did this agent touch" has an answer with no new reader. `o2b brain writes prune-images [--older-than-days N] [--dry-run]` bounds the image store by file age, default 30 days; an image is a copy of note content the snapshot region already archives, so removing one loses no content and only narrows how far back a revert can reach, which a revert plan then reports by name.
- **`o2b brain writes revert` and `brain_writes action=plan_revert`.** A selector - at least one of agent, device or path, with an optional window - resolves every target the selected writes touched to one action: `restore` to the bytes the oldest selected write replaced, `delete` when the oldest selected write created the note and nothing preceded it, or a refusal by name: `drift` when the note on disk is not what the newest selected write left, `interleaved` when an unselected write sits between the selected ones, `image-missing` when the before-image was pruned, `already-reverted` when the target already holds the bytes the plan would restore, `unrecorded` when the target cannot be read at all. A plan carries the log's parse warnings beside its entries, outside the digest, because an operator deciding what to undo has to be told when the history is incomplete. The plan is sealed by a digest over the selector and every entry, refusals included; `--apply <digest>` re-plans, refuses before any byte when the digests differ, runs under a `note-revert` snapshot, and records each restore and delete as a `note-write` of operation `revert`. Apply is CLI-only; the MCP tool plans and does not apply.
- **A fleet freeze.** `o2b brain freeze [--reason]` writes `Brain/.state/frozen.json` and `o2b brain unfreeze` removes it; the marker records who set it, from which device, when and why. `assertVaultIdentityForWrite` - the guard every content writer in the tree already calls before its first byte, ninety-eight call sites across sixty modules - refuses the content lane by name while the marker exists, with `frozen_at`, `by`, `reason` and the next command. The guard gains a write lane: the Brain log appender and the marker writer ask for the audit lane and are the only two that do, pinned by a source scan, so `freeze`, `unfreeze` and `write-refused` events are recorded while the freeze is in force. A malformed marker is a freeze, not an absence. The MCP boundary maps the refusal to a structured error and appends one `write-refused` event naming the tool. `o2b brain rollback` on a frozen vault is refused like every other writer; the recovering operator is exactly the operator who should decide the freeze is over.
- **Per-device shards for every ledger.** The shard grammar, listing, sync-conflict exclusion and device-id resolution move out of the log reader into `ledger-shards.ts`, byte-preserving for the log, and the continuity store, idempotency ledger, preference audit, metrics and session-lineage ledger write `<base>.<device-id>.jsonl` and merge every shard at read in a deterministic order. The preference audit is the one ledger that changes shape: a flat `<pref-id>.<device-id>.jsonl` is a name another valid preference id could own, because a preference id may itself contain a dot, so new audit lines land in one directory per preference, `Brain/log/pref-audit/<pref-id>/device[.<device-id>].jsonl`, the legacy flat trail stays readable, and a row whose `pref_id` is not the one asked for is dropped with a warning naming both. The bare name is the shard with the empty id everywhere, so no file is renamed and a device with an id never appends to a bare file again. The lineage ledger keeps one hash chain and one sequence per shard, compacts per shard, shards its gap sidecar the same way, and its verifier reports per shard path. The doctor's `sync-conflict-log` finding sweeps every ledger directory and names the one the copy sits in.
- **The log chain.** Every JSONL row appended from this release on carries `h`, the sha-256 of the canonical JSON of `{ prev, ts, kind, payload }`, and `prev`, the `h` of the previous chained line in the same shard or `null` at that shard's genesis. Editing any earlier line changes every later hash, so a tampered history cannot be re-linked without rewriting the shard from the edit forward. One chain per shard, because two machines append to two files that Syncthing delivers in whatever order it likes. Rows written before the chain are legacy and clean while they precede it; the same shape after a chained line is a stripped line and is reported, and a first chained line naming a predecessor means the head was cut off. `o2b brain log verify [--json]` reports the first break per shard as `hash-mismatch`, `prev-mismatch` or `malformed` with the line number; `brain_doctor` emits one `log-chain-broken` warning per broken shard. Report-only: `readLogDay` reads `ts`, `kind` and `payload` and nothing else, so a broken shard still yields every event it holds. The markdown twin is a derived rendering and is not chained.

### Changed

- **`readLogDay` reports the shard beside each entry.** `entryShardIds` is a parallel array at the same index as `entries`, the empty string being the legacy un-sharded pair. A parallel array rather than a payload field, because the shard is a fact about the file and not a claim the writer made.
- **`createNote`, `applyWriteBatch` and `scaffoldStub` accept a `configPath`.** Without it the note-write record resolved the agent against no config and attributed every MCP write to the unconfigured default instead of the identity the server's own config names. Attribution is the deliverable, so the three signatures grew one optional field.
- **The state-surface catalogue has 44 rows,** up from 42: the freeze marker and the before-image store, each with a written recovery story. The write-site census admits the two new writers as shared-helper rows.

### Notes

- The `agent` on a note-write event is a caller-asserted claim, exactly as `src/core/write-binding/index.ts` has stated since 1.52.0; `origin_channel` and the device shard are the facts. No gate in this release keys on the agent name, and the freeze keys on the vault, not on who is asking. A policy layer over who may write remains a separate card.
- The chain is the one place this release changes bytes for a vault that enables nothing: two keys on every new log line. Legacy lines read exactly as before.
- Revert refuses on doubt and never merges. A target another writer touched after the selected writes is reported, not reconciled; the plan digest covers the refusals, so the operator who applies a digest applies the plan they read, refusals and all.
- The lineage state-surface row still derives the bare file name, which on a device with an id does not exist; the row's reason says so. Deriving a shard name there touches the inventory contract and is left for a later card, as is re-pointing the two ledgers that already carried their own copy of the shard grammar (`truth/store.ts`, `decisions/receipts.ts`) at the shared module.
- An independent review of the finished branch returned three blocking findings and six non-blocking ones, all applied: the before-image prune threw on any foreign file name in its directory and now reports such names as `skipped`; the preference-audit shard grammar let one preference read another's trail, closed by the per-preference directory above; the idempotency ledger and the continuity store had started swallowing every shard read error where the unsharded reader propagated it, so a permission fault read as "key never seen" - both now rethrow anything but a missing file; a crafted `target` on a note-write event aborted the whole revert plan with an unnamed error and is now the per-target refusal `target-invalid`; one throwing entry in a revert apply aborted the rest after the snapshot and is now a named `failed` bucket the others continue past; and the audit-lane pin reads both spellings of the lane.
- Note-write events carry no session or turn id: the MCP server context does not hold them today, and inventing one at the seam would be a claim with nothing behind it.

## [1.54.0] - 2026-08-27

Open Second Brain has shipped a `visibility:` frontmatter field, a predicate that reads it, and a filter that applies it - and until this release the field was decorative. The filter was wired into exactly one call site, and the scope it filtered against came from the caller, so a remote caller that asked for the private scope was handed it. Of the 71 callable surfaces the project's own visibility census enumerated, 3 consulted the field and 68 did not. The previous wave measured that rather than closing it: 1.52.0 shipped the census, the registry behind it, and an `o2b search check` finding that stated in the operator's own diagnostics that this boundary did not exist. This release is the enforcement that census was built as the coverage map for. One reserved token is denied by default at remote reach and enforced at the three roots every read surface reaches page content through - the ranked search pipeline, the page walker, and the by-path and by-chunk-id read primitives - and a new sweep proves mechanically that there is no fourth root. The reach that decides is minted by the transport and refused by name when a caller supplies it, because a trust claim echoed back from the request is not a trust claim. A vault that never wrote the token sees no change at all, and the schema-12 backfill is what guarantees that rather than an assumption.

Both source cards had premises refuted against the live source before anything was designed, and the verdicts ship as deliverables rather than as obstacles that were quietly designed around. The first card's claim that no `visibility:` handling existed in the read surfaces is refuted by `src/core/graph/visibility.ts` and `src/core/search/result-filters.ts`, and the carrier it proposed - extending `gatedOwnerScopeView` - is refused as an approach: that view short-circuits every predicate to true unless `integrity.owner_scope_delivery` is set to `fail`, so riding it would have shipped privacy that is off in every default install. The second card's claim that no registry-driven harness and no fail-closed gate existed is refuted by the agent-scope matrix and the surface census, both of which already fail on an unclassified newcomer; what survives is that the harness's axis is ownership rather than visibility, which is the axis this release adds. The wave's own stated risk is corrected too: `indexRevision` is a query-cache generation counter consumed by `computeCorpusGeneration`, not a schema or index-format revision, and moving it forces no reindex on anyone.

### Added

- **One reserved visibility token, denied by default at remote reach.** `isRemotelyReadable` is the kernel the rest of the wave consumes: it denies exactly one token - `REMOTE_DENY_VISIBILITY_TOKEN`, written once in the normal form `pageVisibility` reads tags into, so a page cannot spell it a second way and mean something else - and answers true for everything else at every reach. Every other token, reserved-adjacent or not, keeps the caller-liftable semantics `isVisible` has always given it; this constant reserves one identifier and defines no policy for any other. The caller's own `visibility` argument still works and can still narrow. What it can no longer do is name the reserved token and be handed the page. There is no configuration key to switch this on, because a boundary an operator has to enable is the defect the previous release's honesty finding named.
- **The reach a request arrived at is minted by the transport and refused from the caller.** `TransportReach` is a closed two-member vocabulary - `local` and `remote` - and it is a transport fact rather than a request field. stdio and the CLI mint `local` because the caller already started the process and holds whatever filesystem access it runs with; an HTTP bind mints from `isLoopbackHost` alone, keyed on the bind rather than on a caller-supplied `Host` header, so every non-loopback bind is `remote`. Each value is set after the caller's runtime options are spread, the same guard `sendNotification` already uses, so a runtime option cannot claim a reach the bind did not establish. A caller that puts a reach-shaped argument in a `tools/call` is refused with `-32602` and the token `caller-supplied-reach`, ahead of the unknown-argument gate and without consulting the tool's schema, so an open schema is covered too; names are compared case-folded and separator-free, so `reach`, `Reach`, `transport_reach`, `transport-reach` and `transportReach` are one argument rather than five holes. `disclosure` is deliberately not reserved - it is the unrelated result-depth mode on the recall surfaces. What the `local` bypass proves is written down where it is defined rather than overclaimed: filesystem-equivalent access, or a request that originated on this host. It does not prove operator intent, since a remote host can spawn a stdio subprocess. The alternative - no bypass at all - would take an operator's private notes away from the operator's own shell.
- **Enforcement at three read roots, and a sweep that proves there is no fourth.** Root A is the ranked pipeline, where `applyVisibilityScope` is reimplemented on the predicate rather than left beside it, and the reach rides on `SearchOptions.transportReach` with an absent value resolving to the narrowest. Root B is `listVaultPages`, where the reach is a required option rather than a defaulted one - the two ways to get it wrong fail in opposite directions - so each of its 23 call sites states which caller it walks for and the compiler is what asks the question; the maintenance lanes answer once, through `MAINTENANCE_LANE_REACH`, because a repair lane that stopped seeing reserved pages would corrupt the link graph it repairs. Root C is the key-addressed reads, where both key spaces are enumerable - a chunk id is a sequential integer and a preference id is a topic slug - so a withheld read answers with the message an absent one produces, byte for byte, rather than with a distinguishable refusal that would be an existence oracle over exactly the population being hidden. `DIRECT_VAULT_READ_REGISTRY` and its architecture sweep make the closure claim checkable instead of prose: every file under `src/mcp/`, `src/cli/` and `src/openclaw/` that opens a vault path through `node:fs` rather than a root must carry a row with a closed category and a written reason, and the sweep fails in both directions. It found two on its first run - `brain_clusters` listing `Brain/clusters/*.md` and `brain_bridges` reading `Brain/proposals/bridges.md` - and both now ask the rule at the site of the read. What the sweep cannot see is written down beside it rather than implied.
- **`documents.visibility`: what the index measured, backfilled by schema 12.** Three distinct states - a JSON array of the page's normalised tokens, `[]` for a page that declares none, and NULL for a document the index holds no frontmatter chunk for. The backfill is the whole of why this is more than an `ALTER TABLE`: a column added without one is NULL for every existing row, and the only fail-closed reading of unmeasured is reserved, which would withhold every document in every vault on upgrade. The frontmatter is already in the index as its own chunk at `chunk_index = 0`, so the backfill is one table scan and a parse per document with no vault walk and no operator action. The design's open question is answered by measurement: it runs inside the migration transaction, and not for speed - 86 ms over 5000 documents, against 8.9 seconds to index the same corpus - but for atomicity, since a half-filled column leaves measurable rows reading as unmeasured. The column reports and the live file decides: moving the read boundary onto the column would trade promptness for a saved frontmatter read, so a page reserved a minute ago would stay readable until the next index run. What the column earns is exactness - `anyVisibilityTagPresent` was a `LIKE '%visibility:%'` scan that read a quoted key or a body line as a tagged vault, and both false positives are gone - plus the two populations `o2b search check` now reports rather than absorbs.
- **The OpenClaw page walker joins the boundary and the census.** `src/openclaw/index.ts` was the one surface the previous census named as un-swept, which meant the place already identified as a gap was the place the sweep could not have found it. Its page search takes the narrowest reach, and the distinction from the CLI is the caller rather than the machine: a CLI verb is typed by the operator, while these tools are called by a model over a conversation the plugin cannot see. `second_brain_query`'s `total_pages` is now counted after the filter, closing a difference between `total_pages` and `returned` that the status-scope filter had already made observable.

### Changed

- **The `o2b search check` visibility finding states what is now true.** It used to say the field was a caller-supplied view filter and not a privacy boundary, which was true and is not any more. It now reports the three things an operator needs in order to size the change rather than infer it from a search that came back shorter: how many indexed documents reserve themselves against remote reads, how many of the enumerated note-returning surfaces still do not consult the field, and how many documents the index holds no measurable frontmatter for - reported rather than folded into "declares nothing". Every number is read at render time off the registry and off the index's own column; none is written down here. The finding fires whenever there is something to be honest about, which includes an index whose column is NULL for every row - the state that count exists to surface, and the state in which an earlier gating bug printed no line at all.
- **The visibility registry reclassifies, and the counts fall out of it.** 24 of the 76 enumerated callable surfaces now consult the field, up from 3 of 71: 13 MCP tools, 7 CLI verbs and the 4 templated `osb://` readers, each with a written reason naming which root covers it. The 52 that remain are the honest remainder, each carrying what it actually discloses rather than being waved through. The `index_store` row is kept and reworded rather than dropped: a reserved page's text is still in `chunks` and `chunk_fts`, deliberately, because excluding it would take an operator's private notes out of the operator's own local search. That is the same trust boundary as the vault's own Markdown files, and this release does not close it.

### Fixed

- **The query cache is partitioned by the reach that computed it.** `buildCacheKey` enumerates every result-affecting option explicitly, and the reach became one the moment it decided which pages are in the pool. It was not in the key, so a `local` outcome and a `remote` outcome collided on one row of the shared `query_cache` table that the operator's CLI and the HTTP server both write: whichever warmed the row first served the other, handing a remote caller the reserved pages it had just been denied, and poisoning the operator's own shell in the reverse direction. Folded in resolved rather than raw, and unconditionally, because every row written before this boundary existed was computed without it.
- **The retrieval trail stopped counting withheld rows.** `scope-filters-dropped-rows` rides out on every `brain_search` response and is not gated behind `explain`. Its baseline was taken in front of both rules, so a remote caller searching a term nobody public wrote about got zero results and a trail saying two rows had been removed - an existence oracle enumerable one query term at a time. The two rules are separate functions now, the reach rule runs first, and the trail's baseline is taken between them, so the number it publishes is exactly what the caller's own arguments removed. The assembly's one-shot backfill keeps the other count in its own field, because a reserved page crowding a narrow rank cap still has to widen the window.
- **The successor pull-in re-asks the rule.** Relation polarity runs after the pool filters and does not only reorder: it resolves typed `superseded_by` edges to documents that were never in the filtered pool, fetches their chunks, and appends them with full path, title and content. A public page declaring a reserved successor was therefore a route to that successor's body, and the pool filter could not have caught it because the row was not in the pool. Root A is asked again over the whole pool. The identical gap for the caller's own `visibility` scope and `agentScope` predates this boundary and is recorded at the seam rather than silently carried.
- **A caller-supplied benchmark is scored at the caller's reach.** `runRecallBenchmark` pinned `local` on every search it made under a comment calling itself an internal lane. It is not one - `dataset` is a required argument of `brain_benchmark` and `brain_tune`, both callable over any transport. The reach is an option now, resolving to the narrowest when absent. The reported premise is refuted and recorded rather than dropped: a tagged page is outside every benchmark run at every reach because the benchmark passes no visibility scope, so the observable effect is the unmeasurable page - a document still in the index whose file was deleted or made unreadable - and that is what the pinned tests measure. `captureRecallFeedback` had the same defect in sharper form, re-running the judged query unscoped and returning `resultFound`.
- **Root B fails closed on a page it could not read.** Roots A and C substitute the reserved token for a page whose file cannot be read, on the stated ground that an unreadable visibility claim is not the absence of one. Root B decided off the parsed map alone, and the parser resolves an unreadable file to an empty map - byte-indistinguishable from a page that declares nothing - so `listVaultPages` listed it at remote reach with its path and its filename-derived title. The trigger is routine rather than exotic: a page deleted, renamed, moved to a failing mount or made unreadable between one call and the next. Toggling a reserved page's readability was itself the oracle. The walk now carries the read verdict beside the parsed map and substitutes the same token its twin does, spelled from the same exported constant so the two cannot drift.
- **Two counts that reported what they had withheld.** `brain_query mode=topic` filtered its signals and log events and then reported the pre-filter count - a number written to the recall-telemetry log and handed back verbatim by `brain_recall_telemetry`, so a caller that saw an empty topic could read through a second tool how many rows it had been withheld from. `brain_clusters operation=run` reported community membership filtered by owner scope alone; both rules are ANDed now, and a community either rule withholds is dropped whole, because a community of seven reported as four is not a narrower true finding but a false one.
- **A withheld preference names the id an absent one names.** Both refusals claimed byte-identity with the absent form in their own comments and neither had it. `brain_query mode=preference` passed an already-formatted sentence into a constructor that formats the sentence itself, and `osb://preference/{id}` echoed the raw id on the withheld branch while the absent branch rethrew the normalised one - so for the bare-slug key shape the presence of the `pref-` prefix was a one-bit existence oracle over the whole reserved population. Both key shapes are pinned on both surfaces now.
- **`brain_bridges` and `brain_backlinks` ask the rule about what they name, not only about what they return.** `brain_bridges` was classified covered off its `list` mode alone, while `discover` returned proposals naming two pages each by vault-relative path; a proposal is dropped whole when either end is withheld. `brain_backlinks` and its CLI mirror published every ref the index returned, and a ref names its source artifact, so a withheld target now answers with the empty backlink document and a visible target no longer names sources the caller may not see.
- **The reference grammar stopped failing open on spelling and on shape.** A reference the resolver could not place was read as naming nothing and let its row through - a fail-open on a rule whose whole posture is fail-closed. Three cases were live: aliased, anchored and line-ranged forms such as `pref-x|alias` and `pref-x#Raw`, which `brain_apply_evidence` documents for its own `artifact` argument; multi-artifact payload values, which are `string | ReadonlyArray<string>` and answered undefined as arrays; and an enumeration of four payload keys where a census of every `appendLogEvent` body in the tree finds artifact references under thirteen. Every string in the payload is offered now, because enumerating a payload whose writers are open-ended is the defect rather than the particular keys that were missing, and offering too much is safe in the way offering too little is not.
- **Two surfaces that were outside the denominator.** The census sweeps a file only when it imports a note-content primitive directly, and `brain_hygiene` and `brain_skill_proposals` reach page content through a `src/core/` helper - so neither had a registry row, neither was in the number `o2b search check` reports, and both were returning reserved page paths at remote reach. Both are filtered at the handler seam the owner rule already sits on, so a hygiene detector registered later inherits the rule, and both recompute their own count from the visible rows.

### Notes

- What this release deliberately does not close is stated rather than implied. A reserved page's body stays in the index, so anyone with file access to `brain.sqlite` reads it - the same trust boundary as the vault's Markdown files. Owner-scope delivery and this boundary remain independent rules over different subjects, and neither is folded into the other. Non-reserved tokens keep exactly today's caller-liftable semantics. And 52 enumerated surfaces still do not consult the field; the visibility matrix measures which of them can still name a reserved page at remote reach and asserts that list in both directions, so a surface leaving it is the wave that covered it and one joining it is a regression.
...
- The new visibility matrix is a sibling of the owner matrix rather than a second axis inside it, decided on evidence the design left open: the two cannot share one fixture, because the owner matrix builds its contexts without a reach and every one of its assertions would have had to say which reach it was asking at. The recipes are shared through one catalogue rather than copied, since a second copy of "what is the whole surface" is the enumeration defect the first matrix was built to remove. Every probe goes through the real `tools/call` path and asserts over the whole serialised response, which covers `structuredContent`, the rendered text, a thrown `message` and the `_meta` member in one check.
- One recorded limit on the new column: an unterminated frontmatter block is measured as declaring nothing rather than as unmeasured, because the chunker leaves chunk zero of such a page byte-indistinguishable from that of a page with no frontmatter. It does not matter, because the live parse on the real file reads an unterminated block the same way - the column and the boundary agree rather than one of them being wrong in private.
- A pre-existing test flake was root-caused and fixed on the way. `dream progress > attaching an observer changes nothing the pass writes` failed about once in twelve loaded runs and never in isolation; the whole difference was the snapshot entry's `size_bytes`, republished into the day log the assertion compares, and whether it tripped depended on whether the copy straddled a second. Verified against `main`'s own source at the same rate before being fixed by normalising the one field rather than excluding the pass's main authored artifact.

## [1.53.1] - 2026-08-27

Reported as [#180](https://github.com/itechmeat/open-second-brain/issues/180) by [@BlackKite84](https://github.com/BlackKite84): `o2b brain page-dedup` merges a duplicate pair, stamps `merged_into` on the secondary, and then proposes the same merge again on the next scan, forever. Confirmed as filed, and the seam the report named - `findDuplicateCandidates`, which never read the pointer the merge writes - is the right one; it is the single chokepoint behind `page-dedup`, `brain actions`, the digest, `brain_doctor`, `vault_health` and the operator trust verdict, so one filter closes all six. Verification found two escalations the report did not mention, and both raise this from a noisy digest to a silent vault mutation: on pages with no `created_at` the canonical ordering falls back to an mtime the merge itself moves, so a second `--apply` reverses the pair and writes a merge CYCLE that leaves `lint --consolidate` unable to repair any link to either page; and `setMergedInto` overwrote an existing `merged_into` without asking, so a deliberate operator merge was repointed at whatever canonical the next run picked. One side-claim in the report is rejected: identical text under different scopes cannot re-collapse, because the composite scope is already folded into the dedup key and a test has pinned that since v1.37.0.

### Fixed

- **A page a merge already resolved leaves the dedup candidate pool.** Any `merged_into` pointer takes the page out of candidacy, not just one aimed at another member of the same group - the narrower reading the issue proposed leaves the two cases that cost something. A pointer aimed OUTSIDE the group kept the page a candidate and let the next `--apply` overwrite the operator's own merge decision, and a chain that leaves the group and comes back (`b -> x -> a`) survived a one-hop test. A dangling target counts as resolved too: the canonical was lost, which is a lint problem and never a licence to re-merge the page somewhere else. Members are dropped individually rather than whole groups skipped, so a third page duplicating the canonical after an earlier merge is still proposed. The predicate is defined once and the two sibling detectors with the same blind spot adopt it - the digest's `## Merge suggestions` and the hygiene `dedup` detector, both of which repeated the finding in their own sections - which needed `parsePreference` to expose `merged_into` so the digest's pre-parsed path reaches the same verdict as the filesystem one.
- **The merged-into write seam refuses cycles and silent repoints.** `setMergedInto` checked for a self-pointer and nothing else. It now reads the existing pointer first: re-setting the same canonical writes nothing and reports `changed: false`, a different canonical is refused by name unless the caller passes an explicit repoint, and a canonical whose chain already resolves back through the secondary is refused before a byte is written. The guard stays even though the dedup pass can no longer propose either write - the proposer is one caller, and this is where the damage happened. The chain walk behind the resolver and the new cycle guard is now one function, with the resolver's `CYCLE` / `DEPTH` / `MALFORMED` behaviour, dangling targets included, unchanged.

### Notes

- The N2 idempotency test pinned the defect: it asserted the merged pair was still reported on the rerun. The invariant it exists to defend - "a rerun re-collapses nothing new" - is strengthened rather than broken by dropping resolved members, so it keeps its title and now asserts zero clusters. `page-id.test.ts`'s "updates an existing pointer" pinned the silent overwrite and is rewritten as the refusal / explicit-repoint pair it should always have been.
- A `merged-into-dangling` doctor lint is deferred. A new doctor code needs an entry in `DIAGNOSTIC_SIGNALS` or `DOCTOR_EXIT_EXCLUSIONS` under the exit census, plus a frontmatter pass the doctor context does not carry - registration machinery rather than the small reporting-only addition it looked like from outside.
...
The units share their machinery on purpose. One pure reconciliation vocabulary - attempted, found, and missing with every missing key named, no bare-count constructor - is spoken by the import read-back census, the multi-artifact write accounting, and the embedder record audit, and is pinned by the same verdict-vocabulary census that guards the project's other closed vocabularies. Two facts the wave needed - which process a write came through, and which transport carried a request - are derived by the server at the entry point, never accepted from a caller: twenty-five tool schemas already take a caller-supplied `agent` string verbatim, and this wave declined to add one more unverifiable claim beside them.

### Added

- **`o2b search check` measures the pending-vector count instead of inferring it.** The old recommendation branch fired on a provider-reachable proxy with no term for existing vectors, so a fully embedded, healthy vault was told to "compute the first vectors" - a false statement on the happy path, and one no test pinned. A COUNT(*) anti-join through the read-only seam replaces it: a fully embedded vault gets no recommendation, a partial one gets the count and the dry-run backfill verb that prices it, and an absent or unreadable index reports unrecorded - never zero, because an unmeasured quantity is not a measured nothing.
- **An embedder record audit, and a restamp for the drift that costs nothing.** Nothing compared the recorded index dimension against the width of the vectors actually stored; a meta row claiming one dimension over data holding another was invisible. The audit reports that disagreement as its own third state - the record contradicts the data - distinct from drifted and from unrecorded. `o2b search restamp` repairs the one drift that needs no provider, a stale `vec_version` token: dry-run by default, `--apply` writes one cell, and a real model or dimension disagreement refuses by name because that repair costs money and belongs to the apply-gated backfill.
- **Note updates refuse to destroy what they could not read.** An existing-but-unreadable note - permissions, a directory standing where a file should be, a mid-sync fault - parsed as an empty one, so an update destroyed frontmatter and body while reporting `updated: true`, and an append destroyed the prior body the same way. The read now raises `target_unreadable` (and a scanner-lossy frontmatter raises `target_frontmatter_lossy`) before anything is projected, and a blank body over a non-empty note refuses unless the caller passes the new `allow_empty` flag on `brain_update_note` or `brain_write_batch`. Genuinely new empty notes are untouched; the guard's docblock records the three placements it deliberately refused.
- **Every structured Brain record carries a server-derived `origin_channel`.** `mcp-tool`, `cli`, or `import`, resolved once at the process entry point and threaded as an option - never a tool or CLI argument, and a census test proves no schema offers one. Log events, signals, continuity records, and caller-named notes are stamped; existing records are not rewritten, because backfilled provenance is the opposite of provenance; and the write-site census asserts the measured remainder that carries no stamp rather than describing it.
- **Brain status derives its maintenance debt instead of counting it.** `log_events_since_dream` is computed from the log shards the status reader already opens, and is named for what it measures - caller-named note writes emit no log event, and the docblock says so instead of borrowing a bigger name. An unreadable shard turns the figure into a named lower bound, a dry-run dream moves nothing (pinned by a test), and the always-loaded `brain_context` carries only a newest-shard overdue flag so the bootstrap path stays cheap.
- **The three note-producing envelope lanes hand the model its wikilink vocabulary.** Rollup, diarization, and design-note envelopes now carry a bounded `link_candidates` manifest - built from the basename walker with no embedding, filtered through the ownership rule so a withheld note is indistinguishable from an absent one, and carrying its own total and truncation flag so a partial list cannot read as complete. The envelope spine is byte-identical; the three lanes that produce no wikilinked note carry nothing new.
- **Multi-artifact envelope writes account for themselves, durably.** The one lane that commits several artifacts from one validated payload reports attempted, found, and missing in the shared vocabulary with the first real error - never a partial success - and a mid-batch failure now also leaves a dead letter under the vault's state directory, content-addressed so a retry rewrites one file instead of accumulating stamps. A caller that drops the response is no longer the only record that a partial write happened.
- **Imports read back what they claim.** Both import lanes re-read the vault after committing and report claimed, found, and missing with the keys named - a count alone is the misleading no-op this wave exists to remove. The sessions lane also gains resume on the ingest lane's existing checkpoint substrate - extended, not introduced - so an interrupted transcript import continues at the turn boundary instead of re-reading everything, a resumed run re-collects what its recall analysis needs so resumption never loses data, and a malformed checkpoint discards itself by name instead of silently resetting to zero.
- **A visibility surface census, and the honesty it enforces.** `visibility: private` frontmatter is a caller-supplied view filter, not a privacy boundary - any caller may lift it, one read lane honors it, and the index stores tagged content regardless. A standing architecture census now enumerates every note-returning surface as covered or excluded-with-reason, and `o2b search check` states the boundary's absence on vaults that use the field, with the surface counts derived from the census itself rather than hand-written. The census corrected the wave's own reconnaissance on two surfaces, once in each direction.

### Notes

- The wikilink-stub card is closed as already shipped: `scaffold-stub` landed earlier end to end - core, tool, verb, repair-lane integration, tests - and the one unbuilt clause, auto-materialize on capture, is a silent write the shipped design declined in writing. The per-page privacy card stays open with its redesign verdict: real enforcement must reach the walker, the search pipeline, and the indexer, and belongs to a wave of its own; the census above is the coverage map that wave starts from.
- Also refused, each with its reason recorded in the design documents: a stored writes counter (no transaction, no chokepoint, and a counter would be a new state surface where a derivation cannot desync), a caller-supplied channel field (an unverifiable stamp is a lie with a schema), backfilled provenance, a flipped embedding-ABI default (the rebuild-loop argument stands), and any re-embed verification outside the explicit apply gate.
- An independent reviewer was given the finished branch with no knowledge of how it was built and returned two blocking findings and eleven non-blocking ones, all applied: a resumed import's recall analysis silently dropped every turn before the checkpoint boundary while reporting success; a frontmatter-only update deleted a line the scanner could not express while reporting success; a malformed checkpoint silently reset to zero; the visibility census could not see a tool registered under a constant name; the origin-channel vocabulary was invisible to the vocabulary census that should have pinned it; dead letters accumulated one file per retry against their own docblock; an unreadable log shard under-counted the maintenance debt with no signal; the dream pass built link-candidate manifests it then discarded; and a bad ingest plan id disabled resume through a bare catch. The other blocking finding was the version bump this entry is part of.

## [1.51.0] - 2026-08-22

Eight tracker cards picked by priority, and reconnaissance against the real source before any design - a pass that corrected three of the eight rather than following them. The salience gate the first card asked for had nothing to stand in front of: the dream pass owns no per-item model lane, only a count-triggered rollup ladder, so the gate governs what enters that fold instead. The architecture diagram the second card wanted could not draw dependencies, because the scanner's own design doc excludes import analysis and its module facts carry no edges - so the diagram draws what the scan truthfully knows, containment, and says so. And the codegraph enrichment the third card described would have required inventing a partner query surface against a constant whose docstring calls its four tokens the entire surface, absolute - so what ships is the honest half, a verdict region that states what the partner index holds without pretending to read symbols it cannot see.

What the eight had in common only became visible against the source: this wave adds three new lanes where a model writes into the vault, and the project already carried three needs-llm-step envelope shapes sharing five field names and no shared type. So the first unit of the wave is the type those six shapes now share, plus a registry for the payload constraints the deliberately shallow shape language cannot express - a design-note payload must carry exactly one recommended alternative, an extraction batch must respect a cap and a confidence floor, and both rules now live in one fail-closed mechanism instead of two ad-hoc ones.

### Added

- **A deterministic salience gate in front of dream synthesis.** `dream.salience_threshold` partitions the rollup fold set by a score built from three signals the system already computes - decayed evidence mass, the Wilson confidence bound, observed reuse - with no model call, no cache, and no second round trip. Every excluded fact is named in the run summary with its score and all three raw signals; unset threshold reproduces the previous behavior byte-for-byte, proven by running the same vault both ways. A `retriage` action on the staged-bundle verbs reports what a changed threshold would admit or exclude without mutating the bundle.
- **`o2b brain extract-signals` - model-mined session signals that cannot skip the trial lane.** A two-phase batch verb over an already-imported session: phase one emits one envelope carrying the user turns to mine, phase two validates the returned payload against a declared shape plus a per-session cap and per-item confidence floor, then routes survivors through the capture boundary, dedup, the durability denylist, and write-approval staging into `Brain/inbox/` as speculative signals under a new closed-vocabulary `source_type: auto_extract`. Confirmed preferences remain reachable only through the dream pass's own promotion. The live hook path hosts none of this - it runs under a process ceiling that has no room for a model round trip, and the import command's output is now pinned byte-identical to prove it stayed out.
- **`--delete-linked` - a note delete that can follow provenance, and nothing else.** The cascade adopts the source-cleanup blast-radius rule: only a single-purpose derived file whose structured provenance traces solely to the deleted note joins the deletion set; a page that merely mentions the note in prose, or cites any second source, is reported and preserved. One snapshot covers the whole set, the count guard asserts its exact size, and the dry-run lists deletions and reports separately before any confirm.
- **Capture notes carry operator guidance, and the operator finally has a capture verb.** `WriteCaptureInput` gains a free-form `guidance` field - part of the capture identity hash, so two captures differing only in guidance are two captures - and `o2b brain capture` becomes the first ingress beside the Telegram bridge: body from argument or stdin, source, sender, timestamp, guidance.
- **Expiration is now settable, and later changeable, from every writing surface.** `--expires` on the feedback verb and an `expires` parameter on the feedback tool stamp a lifetime at creation; the new `o2b brain expire` / `brain_expire` mutate or explicitly clear it afterwards, addressed by artifact id because the note-path surfaces refuse the `Brain/` root by design. Every path funnels through the one normalizer, clearing is a word rather than an empty string, and an equivalent re-set reports `changed: false` and writes nothing.
- **Agent Skill drafts mined from the pages that earned them.** A `mature_page` pattern kind in the skill-proposals pipeline: the page-meta trio gates candidates, observed reuse supplies the evidence floor, installed-skill coverage vetoes duplicates with the veto named per page, and each admitted page yields one envelope whose validated draft stages as a pending proposal inside the vault. Accepting it materializes a `SKILL.md` under the configured skills root through the existing write-ahead journal - the only write that may leave the vault, and only on explicit accept.
- **The architect overview now names its grounding.** A `codegraph` region states the partner graph verdict - `graph-present` with node, file, and edge counts and health findings, or `graph-absent, lexical-only` with the specific state and reason - from the report the partner already publishes, through zero new partner invocations. A `module-map` region renders a deterministic Mermaid containment diagram from the scanner's own facts, and a decisions note lists the repository's mined ADR candidates by the `repo_key` both generators already share, with an explicit empty state when there are none.
- **`o2b brain design-note` - a one-shot grounded decision artifact.** Given a topic, the verb assembles tension, decision, and truth records matched through the vault's own similarity kernel, reports an empty grounding by name rather than failing on a young vault, and emits one envelope whose payload must carry named alternatives with exactly one recommended - a cardinality the semantic-check registry enforces, since the shape language deliberately cannot. The validated note lands beside the panel outputs under `Brain/decisions/`.

### Notes

- The local transformer embedding provider (the ninth card) is refused rather than shipped. A written invariant stands against bundling a model runtime, the lexical provider already gives a key-less install working recall, and the only runtime-free version - a registered provider name that cannot embed - is a stub wearing a feature's name. The refusal is the explicit choice the card itself asked for; revisit only if the invariant is deliberately lifted.
- The salience gate is deterministic on purpose. An LLM triage scorer with a verdict cache was designed and rejected: all three inputs already exist as pure functions, recomputation is free, and a gate the kernel can evaluate without a model keeps the pass's no-model property intact.
- An independent reviewer was given the finished branch with no knowledge of how it was built and returned one blocking finding - a caller-supplied-agent census pinned one wave behind the tools that declare it - and eleven non-blocking ones, all applied: two tools reported absolute host paths where their siblings redact, the cascade's inherited rule read prose mentions as provenance, a corrupted retriage manifest produced a spurious drift note instead of a refusal, the human-readable dream output was the one path that never mentioned the gate, an equivalent expiration re-set logged a change, the envelope builder's stamped status could be overridden by a hostile payload, guidance containing its own heading split at the wrong seam, and a mid-batch write failure reported nothing about the items already landed.
...
- **A test file could fail inside another file's configuration.** The fake HTTP server the embedding tests run against identified itself by its ephemeral port, and the operating system hands that number back as soon as a server closes. The embedding concurrency ceiling is process-scoped and keyed by endpoint URL, and refuses by design when two configurations disagree about the limit for one endpoint - so two unrelated test files met inside one ceiling whenever a port came back around, and the file that configured `embedding_concurrency` second failed on a limit the first one set. Each fake server now carries a per-instance path segment, which is what it already was: a distinct endpoint.
- **The anti-drift test had still never run in continuous integration.** 1.50.0 put the command-line entry point on PATH for it and added a build step to fail if it skipped. It kept skipping, for the other of its two reasons: `o2b mcp` refuses to start with no vault configured, no runner has one, so the handshake saw EOF - and the build step matched only the first skip message, so that skip still read as a pass. Three repairs, because the test was hiding behind all three: it gives the server a throwaway vault (`tools/list` reads nothing and writes nothing), it names the server's own last words when it does skip instead of sending them to `/dev/null`, and the build step now fails on any skip. On a clean HOME the suite goes from `OK (skipped=1)` to `OK`, which is the first time the vendored schemas have been compared against a live server anywhere but a developer machine that happened to have a vault.
...
Reconnaissance corrected three task bodies before a line was written. A committed vault-level configuration already existed; what was missing was that install and hook generation never read it, and that no resolver anywhere could say which layer a value came from. The tool-profile mechanism already existed; what was missing was the one adapter whose host publishes a limit selecting nothing. And the tool count on the card was 90 against a measured 110.

Then the finished branch went to six reviewers who did not know how it was built, four by area, one told only to find tests that cannot fail, and one to fix what they found. They returned forty-one confirmed defects with reproductions, and the release is what survived that. Two were secret leaks inside the code that exists to stop secret leaks. Three were data-loss paths in a command whose entire purpose is moving data safely. One reviewer finding was rejected with an argument, and the duplication underneath it fixed instead.

### Added

- **One declaration per runtime.** A closed target vocabulary and a fact table stating, per agent runtime, its published tool ceiling, the profile that fits under it, the session adapter it maps to, where it keeps session logs, and how its binary can be asked what it has registered. Session roots are pure functions of an injected home and environment, and a test poisons the real environment to prove nothing leaks in. A census derives the population from the live adapter registry and compares it to the table in both directions, so neither an adapter without a row nor a row without an adapter can ship.
- **A capped host gets a surface that fits it.** Cursor publishes a per-workspace ceiling of about forty tools and was being handed 110. Its generated registration now names the `catalog` profile: seven advertised, every verb still reachable through `tool_hydrate`. The count is not asserted from a fixture - the test builds a real server and calls `tools/list`. An unchecked ceiling is a third state, never folded into "no limit": eight runtimes publish nothing anyone has found, they get no profile, and the capability report says so with the reason rather than staying quiet.
- **`o2b install --friction`** compares hosts by capability dimension, every cell derived from the fact table and the adapter's own plan rather than a hand-written column. Three further dimensions were dropped rather than faked, each with the reason in the module: they have no structural source, and a column read from one adapter's private constant is a hand-written table in disguise.
- **Hosts that can be asked are asked.** Every configuration comparison used to end with the same sentence about no handshake being attempted. A runtime that declares a probe now gets a keyless, free `mcp list`-style query - no model turn, a ten-second cap - and prints either the host's own answer or a named skip. The blanket sentence survives only where no probe exists, which is where it is true.
- **A Codex install adapter**, so a four-step manual procedure becomes `o2b install --target codex --apply`. It reuses the existing TOML editor rather than growing a second one, and every unrelated table in the operator's configuration survives byte for byte. Its probe is deliberately not the JSON form: the shared parser reads the first token of each line, which under `--json` is the same key on every line, so a JSON probe would report every host as empty - a false negative dressed as an answer.
- **`o2b state status`** reports all forty declared state surfaces with the path each resolved to, whether this run could reach it, and which configuration layer put it there. Reachability is a tri-state: a permission error reports unchecked with its errno rather than being rounded down to absent, because "it is not there" and "I could not look" have different repairs. Rows are tiered by recovery story rather than by location.
- **`o2b state migrate` and `o2b state rollback`** move that state and put it back, binding source type, roots, path inventory, byte counts and per-file digests into a manifest. Every check runs before anything is committed, and the whole plan is refused rather than half applied - on a symlink, a special file, an occupied destination, insufficient space, a reserved namespace, an unreadable surface, or a held index writer lock, the one loss a digest cannot undo. Within the apply, every file is copied and re-digested at the destination before a single source byte is removed.
- **Both transports drain.** On a termination signal the server stops accepting, reports draining with an in-flight count on the unauthenticated health endpoint, waits a bounded ten seconds, names any still-open request on standard error rather than dropping it, and exits with the signal's code so the hooks that checkpoint the index and release locks still run.
- **`o2b brain import-session --status` and `--discover`** answer what session logs this machine holds and which were never imported, keyed by content digest so an unchanged log is reported as imported without being parsed again. Found equals imported plus gap plus unparsable, and a Cursor database counts unparsable rather than gap, because a gap is work an operator can close.
- **`o2b brain export --format transcripts-jsonl`** emits one JSON record per conversation. Not instruction-and-response pairs: that is a lossy projection needing three judgements the runtime never recorded, and while a harness can derive pairs from a conversation, nothing derives the conversation back. Tool names travel; tool inputs never do.
- **A committed `install:` block** in the vault configuration supplies the generated hook timeout and the tool profile, and outranks the machine-local tier on purpose - two machines cloning one vault must not generate different hooks because one of them has a stale local key.

### Changed

- **Config values can say which layer produced them.** A provenance-returning resolver was added and the existing one delegates to it, so none of its roughly fifty-five call sites changed and a test asserts the two agree on every branch. The alternative - widening the shared function - was the largest risk in the design and bought nothing.
- **The environment tier no longer reaches a written registration.** The invariant is not how many tiers there are but which survive to the next invocation: a committed file and a machine-local file do, a shell variable does not, and a verifier that reconstructs from one cannot be fed by the other. The variable still selects the surface of a server you start yourself.
- **The maintenance lane's task names are a vocabulary**, and its values are operation references rather than fresh strings, so a lane task is by construction an operation that has a deadline budget. Both surfaces build their lists from it, and a census derives the command-line flag set and the tool property set from source and requires each member to reach the other or carry a written exemption - now including the bounds, since the two accepted different ranges for the same knob.
- **`brain_dream` with a single step runs under a deadline.** It was the one path that did not, and the pass is synchronous, so an unbounded step held the whole server. The two functions it calls take a guard now. What is not interruptible is stated precisely rather than glossed: the vault listing and the phrase build are single phases, so the guarantee is one page plus whichever of those is running.
- **`brain_maintenance` reaches the escapes the command line already had.** Its streak refusal used to tell every caller to use a flag half of them cannot reach; it names both now. The `force` description named two gates while force disabled four.
- **Session parsing streams.** All five adapters read whole files and split them, so peak retention was about twice the file before the first turn was yielded. Measured on a 10.79 MB session: 12,058,624 bytes retained before turn one, against a 262,144-byte window now. Byte identity was proved rather than claimed, by diffing the old and new adapters over sixty file pairs including a character straddling a chunk boundary.
- **Session log roots are declared once.** The discipline scanners independently knew where three runtimes keep their logs and swept them for a different question; both consumers read one table now, proved by a test that relocates a root and watches both follow.
- **The egress site is renamed** from a name that said preferences to one that says export, because it now guards whole recorded conversations.

### Fixed

- **A refusal printed the secret it was refusing to write.** When the transcript export declined because a session filename was secret-shaped, it prefixed the refusal with that filename - into CI logs and shell scrollback, on the one path whose purpose is not letting it out. The guard's own message says locations rather than values; the command printed the value in front of it.
- **The redactor could not see a prefixed environment assignment.** Its rule opened with a word boundary, and an underscore is a word character, so `API_KEY=x` was caught and `ANTHROPIC_API_KEY=x` was not. Everything that appeared to work was being caught by the vendor-prefix rule on the value instead. That is the most common shape a key takes in an agent transcript, and this release is what first ships transcripts off the machine.
- **A bare high-entropy identifier left verbatim.** A session filename and a turn id are named by a foreign harness, and the reasoning that lets vault-authored identifiers skip the entropy scan does not transfer to them.
- **Rollback could destroy the only surviving copy.** It read the source root from the manifest without checking it still existed, so an operator who migrated and then renamed their vault had the old path recreated, the destination copy removed, the manifest deleted, and a success exit. It refuses by name now, and `--to` names where the vault went.
- **Rollback overwrote a file written between plan and apply**, deciding "already in place" once and copying later without re-checking - the one thing the module says it must never do.
- **The reserved-namespace checks compared resolved strings without following links**, so a symlinked destination ancestor moved the whole state tree inside the vault it was migrating.
- **`o2b update` hashed a payload it does not write**, so it reported no change while `o2b install --check` reported drift on the same target, and a Cursor install kept its oversized registration. Two commands contradicting each other about one file.
- **The host probe asked the ambient machine** rather than the one being verified, so a relocated Codex home could verify against a different home's registrations.
- **A directory import recorded files that failed to import as imported**, making them permanently invisible to the coverage report until their bytes changed - including after a future release ships an adapter that could read them.
- **A ledger failure after a successful import reported the import as failed** and suppressed its report, so the machine-readable consumer got nothing while the signal was on disk.
- **One empty transcript aborted the whole export.** Agent runtimes leave zero-length files routinely, and the offered remedy was to move them out of a directory holding a thousand.
- **`--since` silently dropped transcripts with no clock**, which is precisely the case its own comment forbade: the guarantee held only for a malformed timestamp, which never occurs, while the synthesised epoch sentinel took the forbidden path.
- **An unreadable vault configuration escaped as a stack trace** and took every install target with it, because the detection sweep runs all adapters and one of them reads that file.
- **`o2b install --friction` crashed on the same file**, though it is documented as a report that never exits non-zero.
- **A failed removal still dropped the install manifest entry**, so the retry demanded a force flag to clean up what it had just forgotten.
- **A commented-out table counted as a declared server**, so an operator who commented both out and removed the binary from the path was told the install was fine.
- **One tool declared the pairing and the other only enforced it.** v1.49.0 made `scores` and `match_quality` all-or-nothing on `brain_recall_gate` and `brain_context_pack`, and said in the changelog and in the upgrade notes that the schema states it as `dependentRequired`. It said it on one of the two. Both refuse an incomplete pair through the same coercion, but only the gate declared the keyword, so for the context pack the rule stayed exactly what that keyword exists to end - discoverable only by making the call the server refuses. The pairing is now built beside the function that enforces it and parameterised on the same argument name, so a tool's property key, its `dependentRequired` entry and its handler read one constant. A new test derives its population from the live tool table rather than a list: every tool accepting `match_quality` must declare the pairing on its own scores key and must refuse each half alone when driven, so a third tool cannot gain the property without the rule. It fails against the shipped code with no mutation.
- **The vendored Hermes schemas had drifted from the live server, and the test that guards them was skipping itself.** The drift was structural, not stale prose: `brain_recall_gate`'s vendored copy was missing the `match_quality` property and the `dependentRequired` keyword outright, and `brain_context_pack`'s was missing the property - so a provider holding those copies could believe a payload was legal that the server rejects with `-32602`, which the provider's own call wrapper swallows. Latent rather than live, because neither payload is built with scores today. The reason nobody was told is worse than the drift: the anti-drift test needs a live server, skips when the command-line entry point is not on PATH, and nothing put it there in continuous integration - so it had been reporting `OK (skipped=1)` on every run, and the one guard against a vendored copy diverging fired only on a developer machine that happened to have the symlink. The entry point is on PATH there now, and a following step fails the build if the test skips again, because a skip and a pass read identically in the summary line.
- **The snapshot gate reported a cause it invented and hid the one it had.** Reported from outside as a Windows failure, root-caused here as a platform-independent defect and reproduced on Linux. The retry key matched any `EEXIST` anywhere in an error's cause chain, without regard for which path collided - and the first thing a snapshot does is create its own archive directory. So a failure to make `Brain/.snapshots` read as "another process took this candidate run id", laddered through all sixty-four candidates, and reported that every one was claimed, while the real error sat in a `cause` neither command-line wrapper printed. Nothing had claimed anything, and the directory need not have existed at all. The retry now matches only a collision naming this candidate's own archive; an error carrying the code but no path no longer counts, because treating unknown as "yours" is what produced the fiction. The directory is diagnosed before it is written into, naming what was actually found there, and error causes reach the operator through one renderer rather than being dropped at the last line.

### Notes

Some of what the reviewers asked for is not here, and each absence is a decision.

Three adapters answer a refuting probe differently and were not collapsed into one verdict, because collapsing them would make one wrong: one leaves no artifact, so the host's registry is the record and its no means the registration is gone; the other two compared an artifact and found it correct, so the host has not reloaded. The defect was that one rule was argued three times in three bodies. It is argued once now.

The streaming bound is asserted on the reader's own accounting rather than on resident memory, and the suite says so and records the adapter it cannot catch - one that streams correctly and also holds the file. Repeated measurement showed resident size swinging between zero and sixty-six megabytes on the same code path, because mid-iteration heap is dominated by parse garbage rather than by the reader's window. A bound that reports thirty megabytes for a reader holding two hundred and fifty-six kilobytes is not a bound.

Two progress emitters cannot produce an advance record in a test - one needs an embedding provider and a network call, the other owns no loop - so they carry declared exceptions checked in both directions, and an exception that outlives its condition fails.

With `--recall` set, session import still accumulates every kept turn in memory, about 1.4 times the file, because the recall store takes a materialised array. The adapters underneath it no longer do. Bounding that means changing the store's signature, which is its own unit.

A defect that could not be given a failing test is named rather than quietly fixed: the platform refusal in the grok adapter's detection path reads the process platform directly with no seam, so it cannot be driven from this one.

The Windows half of the report that produced the snapshot fix is not fixed, and will not be claimed. Whether this runtime raises `EEXIST` when asked to create a directory that already exists on that platform is a question no machine here can answer, there is no Windows job in continuous integration, and shipping an unverifiable fix is the thing this release exists to stop doing. What did ship makes the next report falsifiable: the command now names the directory and what is in it instead of a run-id ladder that never ran.

That report also stood inside a boundary this project declares and does not hold. Windows is listed as an unsupported platform, but the refusal sits on the third branch of configuration-path resolution, after two environment variables have been consulted - so setting either one runs the whole product on a platform it says it refuses, and the code already documents that as intentional. It is a sign on an open door. Making it a real boundary would break two documented ways people are using today, and calling it advisory means saying so in the README and stopping treating those reports as in-scope. That is a decision for the operator, and it is recorded rather than taken.

## [1.49.0] - 2026-08-16

Ten tracker items, picked by priority off the board and read against the real source before anything was designed, turned out to describe one defect seen from ten sides: the product declares a boundary and puts nothing behind it. An owner-isolation filter whose field no writer ever emitted. A gate that enforced whichever scope the caller asked for. A confidence floor compared against a number that could not vary with match quality. An egress registry built to stop an unguarded export, with an unguarded export outside it. A census whose docblock claimed to have closed the mis-parse class that was live inside it.

Reconnaissance corrected most of the ten task bodies before a line was written. Six corrections changed what the unit is rather than how it is worded, two findings turned out to be the opposite of what was filed, and two described a live defect rather than an improvement: a credential leaving verbatim through a shipped export path, and an authorisation check that read back whatever the caller claimed. Each unit then takes one declared boundary and does exactly one of three things, chosen by evidence: ENFORCE it and put a census behind the check, RE-MEASURE it against the signal that actually varies with what the label claims, or RETRACT it and say so.

Then two reviewers were given the finished branch with no knowledge of how it was built, and told that a comment is a claim rather than evidence and that a test which cannot fail proves nothing. They turned the release's own thesis on it and it did not survive contact. The isolation probe this release is proudest of — one two-owner fixture driving every classified tool — was itself a label with nothing behind it: flipping it to run with the gate OPEN and demanding the marker appear gave 15 pass and 81 fail. Eighty-one of the ninety-six entries were driven against arguments naming artifacts the fixture never created, so they could not have failed under any setting. Three more surfaces were leaking behind that green: `brain_hygiene mode=apply` retired another owner's preference and named both hidden ids, `brain_analytics` filtered one view of four, and `brain_health` returned a verdict folded over findings it had just withheld. The MCP resources surface had no ownership filtering at all. The central measurement the four rethreaded thresholds rest on was wrong by 0.05 in seven places. Everything named below is what shipped after that.

### Added

- **Ownership is written, or it is not a boundary.** With `integrity.owner_scope_delivery` on, every production preference writer stamps the server-resolved agent identity onto a new record. A rewrite never re-owns, so a page created before the gate stays shared rather than being retro-owned by whoever rewrote it first. With the gate off nothing is stamped and a vault that never opted in is byte-identical, measured with a whole-tree digest. "Every writer" is enforced by a census derived from source rather than a hand-written list: a module that builds a preference path and writes bytes must reach the stamp or carry a declared exemption. The hand-written list is what let `import-claude-memory` write an unstamped preference through its own renderer.
- **A caller-supplied owner scope is checked.** Under the gate, an `agent_scope` naming an owner other than the resolved identity is refused by name, never silently narrowed — a narrowing would leave the caller believing it had read what it asked for. An unconfigured install cannot become an owner either: the placeholder identity `agent` is refused by the same predicate on the write side that already refused it on the read side, so the two halves cannot disagree.
- **The scope matrix asserts correctness, and asserts that it asserts anything.** Ninety-six classified tools carry 220 call recipes, one per mode, view or operation, because one executed view is not an executed classification. Every recipe runs twice against the same two-owner fixture — once with the gate closed, once open — and the classification decides what the open run must show. The 31 recipes claiming to reach owner-taggable content must surface the marker with the gate off, which is what stops the closed run from passing vacuously; the other 189 carry a claim of unreachability, executed directly where nothing is hidden. All three numbers are pinned as equalities.
- **The MCP resources surface honours the gate.** `osb://preference/{id}`, `osb://topic/{slug}`, `osb://backlinks/{id}` and `osb://log/{date}` returned another owner's file verbatim, including its `owner:` line, while the delivery path correctly withheld it. A withheld resource now answers with the same not-found message an absent one produces. The topic scope reaches the selection rather than filtering after it, so a shared record is not hidden merely because another owner's sorted first.
- **The delegation boundary the host already delivers.** `SubagentStop` is registered, and a session turn carries the delegated `agentId` and `isSidechain` — the only things separating a sub-agent's turns from its parent's, since a sub-agent transcript reuses the parent's session id. Both are emitted only where the transcript records them, so a flat transcript keeps its pre-delegation shape byte for byte. The Grok installer registered six of eight hook events and captured nothing from a delegated sub-agent; a parity test now derives the expectation from `hooks/hooks.json` itself.
- **An unreachable search origin is reported rather than dropped**, with a four-piece vocabulary distinguishing reachable, unreachable and unknown, each carrying its own reason where it has one — "it is not there" and "I could not look" have different repairs, and neither may read as a zero contribution.
- **The `vault_path` contract has an enumeration behind it.** Forty-four sites emit the field and three honoured the contract stated in prose above them. A census lexes every emission out of the tool surface and requires it to come from the one producer. The producer moved to its own leaf module, because importing it back from the tool aggregator would have closed an import cycle the architecture ratchet gates — and that move is also what let the OpenClaw plugin, a second runtime with three emissions of its own, reach it.

### Changed

- **Four thresholds stop reading a number that cannot vary with match quality.** Whenever the keyword lane is non-empty, the top row of any recall scores at or above the configured `search_keyword_weight` — shipped at 0.6, identical to `recall_adequacy_sufficient` — and 0.65 measured end to end on a freshly written keyword-only vault, where the ranker adds the freshness prior on top of the min-max normalised lane. Nothing about how well the row matched can move it below that floor. So every keyword recall graded `sufficient / proceed`, a gap task auto-closed on a hit of any quality, and the cross-vault chain stop meant opposite things under the two fusion modes. All four now read IDF-weighted coverage. No threshold value changed; what they measure did.
- **Match quality can say it does not know.** `idfWeightedCoverage` is `number | null`, and `null` exactly when there is no IDF mass to weigh. It used to return 1.0 — maximum confidence — for any query it could not weigh, which included every one- or two-character CJK query, because "significant term" carried a three-character floor that was an English-shaped stopword rule in structural disguise. That floor is gone; a token qualifies if it carries a word character, and nothing branches on script. The same floor sat one layer up in the demand log, where it made a CJK query unable to form a bucket at all, so its recall gaps were invisible to the gap-promotion loop. `null` is nullable rather than a flag so the compiler forces every consumer to choose: the targeted retry does not fire, completeness gains an `unmeasurable` verdict rather than folding into `sparse`, the chain stop does not stop, the gap loop does not close, and recall-inject abstains with a named reason.
- **What match quality is, stated narrowly enough to be true.** It is absolute as against a score — no rank position, no lane magnitude, no fusion mode — so two retrievals for one query are comparable. It is NOT independent of which rows were delivered: it is computed over the returned window, so the same query at `limit: 1` and `limit: 20` differs, and that is pinned by a test. The first version of this release claimed pool-independence in five docblocks.
- **The `tier:` frontmatter knob reaches the ranker.** It shipped with a weight table, a breakdown line and no producer, so a page stamped `tier: peripheral` scored identically to an untagged one. An untagged vault emits no tier map at all and its ranking is unchanged, pinned by an ordered `(path, score)` window over the committed benchmark corpus.
- **`brain_recall_gate` and `brain_context_pack` take `match_quality` beside their scores**, all-or-nothing: the level is decided by the quality alone, and deriving one from the scores is the substitution this release removes. The tool description advertised the call the handler now refuses; it names the pairing, the schema states it as `dependentRequired`, and the verdict returns `match_quality` so the quantity that decided the level travels with it.
- **A scoped call on an unreadable config is refused rather than delivered.** An unreadable config resolves to the strict integrity fallback precisely so it cannot loosen a gate, only close one. That rationale is about READERS, and this release first wired a writer to it — one bad token in the YAML silently began stamping ownership onto every new preference, permanently. The writer now refuses and names the parse failure: it cannot infer an operator's intent to enable ownership from a file it could not read.
- **One lexer for every census that reads this repository's own source.** Four census files had grown their own copy; one read a backtick inside a regex literal as a template opener, blanked 9,706 bytes of a shipped module and hid what its census could not otherwise see. Its own offset invariant was then broken by an astral character — both views were built by code point while the scanner indexed code units — so four instruments were reading one shipped module one character out of step. Six further single-file assertions that stripped comments by naive regex now route through it too.
- **Census counts become equalities where a floor hid a drifted reality.** The direct continuity-store reader floor sat at twenty against a real twenty-one, and the observability doc had inherited the wrong number from it. The `vault_path` and write-site censuses carried the same shape and are pinned too.

### Fixed

- **`brain_hygiene mode=apply` wrote across the owner boundary.** The owner view was constructed inside the `scan` branch, so a caller whose scan correctly returned nothing could hand `apply` a finding id and retire another owner's preference — and the response named both hidden artifacts. Finding ids are a digest of their targets, derivable rather than secret, so withholding them was never the boundary. The plan is built from the filtered report, which also closes the existence oracle: a hidden id now answers exactly as an id nobody issued. `brain_doctor repair` had the same shape and rewrote another owner's frontmatter.
- **Three surfaces disclosed what they had just withheld.** `brain_analytics` filtered `timeline` and left `concept_synthesis`, `belief_evolution` and `dedup` open — the first returned another owner's preference id and a verbatim line of its body. `brain_health` returned a verdict folded over the unfiltered findings while the arrays beside it were empty, telling the caller a hidden artifact had tripped a detector. `brain_sources` was reclassified rather than filtered: it groups signals by author, and signals carry no owner in this product, so the reason was wrong and not the payload. The widened probe found five more — `brain_clusters`, `brain_dream`, `brain_trigger`, `brain_doctor repair` and `brain_agent_diff` — all closed, none moved to the unscoped bucket.
- **An unredacted vault export.** `o2b brain explorer --export` wrote vault content to an operator-named file without the redactor every other export path uses. The egress census missed it because its population rule keyed on a hardcoded list of destination flags — the same defect class the census exists to catch, one level up. Its two synthetic positive controls could not have caught the fix either: both appended a fabricated path string to an already-derived population, and blinding the derivation to an empty list left them green. All three now run the real derivation over the intruder's real source.
- **The egress registry could not express the largest egress in the product.** A second population rule finds modules POSTing vault-derived bytes to an operator-configured URL. Five such sites exist, not one: two embedding providers, the cross-encoder reranker — which POSTs the query and the candidate documents — the Telegram capture reply, and the research transport. Each is declared with the reason redaction would corrupt rather than protect the result, and an operator-facing table in the CLI reference names what leaves and when. A default install reaches none of them.
- **An export that could not parse a preference reported success**, silently omitting records. It now collects every failure in one run and refuses by name with a non-zero exit, and the message is vault-relative rather than carrying the host path. The bare `catch { return [] }` on the directory listing above it, which reported a permission fault as an empty Brain, raises with the reason.
- **`o2b brain bank-import` no longer aborts on a malformed rule in the destination.** The refusal's justification — no caller for whom a shorter list is the right answer — is true of an export and false of a prior-topic map, where a destination-side defect has nothing to do with the bundle being imported. The scan runs on the readable rules and reports each file it could not read, because a collision list over a partial map is a partial answer and not an empty one.
- **The redactor ate signal ids**, so an exported preference lost the link it points through. The carve-out keys on the shape rather than the prefix. It also surfaced that the CLI `--json` redactor has no high-entropy pass at all, so the two surfaces answered differently about the same string — only one of them ever ate it, and the tests now pin both.
- **`log.md` was empty on every default install.** The OKF export's date pattern matched only the legacy un-sharded log name while the writer has produced per-device shards since sharding landed. It reads the recogniser in the module that owns the layout, and shards of one day fold into one section.
- **A backlink index reported a confident zero** on a legacy-frontmatter preference, swallowed by a `catch { continue }`. The failures ride on the returned index, so its call sites are unaffected, and the CLI verb — the surface an operator debugging a legacy vault reaches first — renders them instead of printing the unmeasured zero.
- **The gap loop measured coverage over rows its own gate rejects.** The path exclusion that stops a gap-task note closing itself applied to only half the condition, so the note the loop is trying not to count still inflated the number the floor read. The membership rule now travels with the query. The hook that drives it was then passing a retriever of the old shape — which TypeScript accepts, since a function taking fewer parameters is assignable to one taking more — so the hook threw and its outer catch swallowed it, doing nothing silently. It reports a distinct failed action rather than a zeroed healthy one.
- **`schema_inspect view=lint` and `view=orphans` died on the artifact they exist to report**, with an absolute host path in the message. Both report it instead, through one shared rewrite the export refusal now uses too, and the orphan review carries those rows because "nothing uses this token" is a claim over every artifact.
- **An absent argument was answered with a sentence about its shape.** `brain_labels` and `brain_tiers` answered a missing `path` with "path must be a vault-relative string", so a caller who sent the wrong argument name read it as a malformed value.
- **The owner-scope view failed open on a reference it could not resolve**, against its own stated convention, for two reasons: three directories holding owner-taggable id-addressable pages were missing from its list, and a bracketed wikilink matched neither branch.

### Notes

Some of what was asked for is not here, and each absence is a decision with a measurement behind it.

There is no git-visibility ladder: there is no git transport, every `git` invocation under `src/` is read-only, and the egress registry states in prose that there deliberately never will be one. A fail-closed check needs something to guard. There is no staging copy and no history purge: there is no history to purge, and the export bundle already is the staging copy. There is no `off` identity mode: the bottom of the identity chain is already the literal `agent`, itself a placeholder. The `passthrough` / `managed` mode names are not adopted — passthrough is the shipped, ungated default across every tool that accepts a caller-supplied agent, and the owner-scope refusal is what makes it gateable. There is no per-agent instruction block: neither transport can reach one, since both resolve identity from process environment and config rather than from the request, and the config reader has no block-scalar form. The `shared` observation-scope member stays retracted: a positive scope no reader requests narrows visibility rather than widening it.

`private: true` in frontmatter is retracted as a privacy claim rather than enforced. Nothing under `portability/` reads the key, the export composers are content composers rather than visibility filters, and the `<private>` region marker is the only content-derived privacy primitive. A test asserts that such a page exports in full, so the label cannot be re-read as a boundary.

The keyword lane is not de-normalised: every score in the product would move and four thresholds would flip at once.

Three things this release does NOT achieve, stated because the obvious reading of the bullets above is wider than the truth.

On a default keyword-only install, match quality is effectively binary. The keyword lane matches with implicit AND, so every returned row contains every query term and coverage is 1 whenever anything comes back and 0 when nothing does. The measurement is honest — that material really does cover the query — but `weak` remains unreachable on that configuration, for a different reason than before. Partial values need the semantic lane or the broadened second pass. A consequence worth naming rather than burying: the cross-vault chain stop has inverted. It never fired in `linear` mode and now fires on the first origin that returns anything. It is opt-in and off by default.

Computing match quality on every search is a new cost, not a recovered saving. An earlier draft of these notes claimed the number was already being computed and thrown away; it was reachable only in evidence-pack mode, which is off by default. Measured on the committed benchmark corpus it adds about 0.15 ms to a 5.9 ms query — roughly 2.5%, inside run-to-run noise at that size — and it scales with the query's term count rather than with the corpus.

`warn` is inert on the surfaces that now filter by the server-resolved identity. Reporting how many rows `fail` would withhold is itself the disclosure, so `warn`'s observability there is the `owner:` stamp appearing on new files, not a count in any response. The docblock that promised otherwise says so now.

## [1.48.0] - 2026-08-15

Nine units picked by priority off the tracker, and reconnaissance against the real source before any of them was designed. That pass corrected six of the nine task bodies rather than following them, because they describe a different system: one with a resident daemon, a chat-model client of its own, and an ingest that locks too often. This one has none of those. What it does have is long, resource-hungry work that reports nothing while it runs, and surfaces that print claims nobody checked.

The two are the same defect from two sides. `o2b brain dream` produced its first observable character after it had already finished, so a caller could not tell a slow pass from a hung one. A cancellation contract - `signal`, `SafeguardAbortError`, abort checked in priority over the deadline - was fully written and passed by no production call site, which made that error class dead code. A herd of detached reindexes was spawned from startup with `stderr: "ignore"`, so every loser of the writer-lock race threw `INDEX_LOCKED` into a discarded stream. And the install success path was asked to state that every memory is a file in the operator's own vault - a claim with a known counterexample, a machine-local secret keying the vault identity every tool returns, and a vault-resolution chain that disagreed with the rest of the CLI about which directory it was talking about.

Then five reviewers were given the finished branch with no knowledge of how it was built, told that a comment is a claim rather than evidence and that a test which cannot fail proves nothing. Two of them were asked to break the production code in a scratch copy and report whether the test caught it; one ran thirty-seven such mutations, of which ten went green. They returned forty-four reproduced findings, and the release's own argument caught it six times.

Three of the six are worth naming, because each is the release doing precisely what it accuses the codebase of. Three of the five long operations emitted progress and then simply stopped arriving - which is the shape of a completed run, a crashed run and a hung run at once. `docs/mcp.md` stated that four long tools ran under a cooperative deadline; four of the six call paths reaching a consolidation pass had none, and a synchronous pass with no deadline holds the MCP event loop for its whole duration. And the census that the previous release's entire argument rests on - declaration plus a census is what makes a fix durable - turned out to read no source at all: it is a hand-maintained import list, and a reviewer added a vocabulary carrying three of the exact defects it audits for and got 63 pass, 0 fail.

Three findings were resolved the opposite way to the obvious one, and in each case a measurement decided it rather than an argument. Ctrl-C cannot stop a synchronous pass: 27.3 million checkpoints ran with the signal pending and the handler never fired. The obvious fix - make the pass async and await at checkpoints - does not work either, and would hide the bug: 4.8 million awaits with a signal pending, still nothing, because microtasks drain without yielding to the event loop. Only a macrotask yield works, at 184 thousand per second against 13.7 million synchronous checkpoints per second, across 137 call sites where any plain `await` silently recreates the same false claim. So the four synchronous verbs install no signal handler at all, Ctrl-C keeps killing them as it always did, and nothing claims otherwise. The retrying ingest lock was going to have its budget cut, until the wait was measured: the tail is queueing, not hold length, and a shorter budget made this branch's own designed-contention test fail intermittently. And the over-release guard a reviewer proposed for the shared semaphore cannot see the case that matters, because with a waiter queued the pool count is zero and a stray release passes the check and is handed straight to that waiter.

### Added

- **A long pass says what it is doing while it does it.** A synchronous progress sink, optional on the same options interface that already carries the operation's deadline, emitted from the checkpoints that already existed. `total` is optional and its absence means something: the index walk consumes a generator, so counting first would cost a second full traversal, while the embedding phase holds an array and knows its denominator exactly. One event shape carries both, and a renderer that sees no total shows a counter rather than inventing a percentage. `--progress` on `o2b brain dream`, `bridges discover`, `clusters run`, `maintenance run`, `architect`, `search index`, `search reindex` and `search vector-backfill`, as newline-delimited records on stderr - never stdout, which twelve top-level commands own as a payload a caller parses.
- **Progress over MCP, where the transport can carry it.** The stdio transport emits `notifications/progress` against the client's token. The HTTP transport writes one response and closes, so a token supplied there receives a typed refusal naming the transport rather than events that never arrive. A call with no token is byte-identical to before.
- **Ctrl-C reaches the passes that can observe it, and nothing pretends about the rest.** `o2b search index`, `search reindex`, `search vector-backfill` and `brain maintenance run` stop at a checkpoint and exit 130. The four fully synchronous passes install no handler, because a synchronous function never yields the event loop and a handler that cannot run is worse than none - it swallows the keystroke that used to work.
- **A fourth gate on the maintenance lane, which can say it cannot see.** Host pressure joins the window, busy and lease gates. Where the metric is degenerate - Windows, or a CPU quota in force - the gate does not evaluate and says so on its own journal line, distinct from the line a gate that evaluated and let the work through writes.
- **A consecutive-failure streak, and a narrow way past it.** A task that has failed three times running is refused rather than spending the lane's lease every pass; `--retry <task>` attempts that one task with every other gate still in force, where `--force` disables them all.
- **The install says only what it checked.** The ownership statement measures the vault, what filesystem backs it, whether the search index is inside it, and each of four outbound integrations. What it cannot measure - ten kinds of state the tool can leave outside the vault - is worded as a catalogue rather than a scan, and every row names the verb that created it and the verb that removes it.
- **A doctor check for a model with an announced decommission date**, and one for the age of the newest recovery point. The sunset survey ships sixteen sourced entries from one published deprecation page and an operator can declare a date the shipped table cannot know; a model outside the survey is reported as unsurveyed, which is a different statement from no shutdown announced.
- **A switch for the codegraph partner check** (`partner_codegraph_disabled`), which `doctor()` accepted and nothing could set.

### Changed

- **`embedding_concurrency` bounds requests in flight process-wide**, per embedding identity against one endpoint, rather than per `embed()` call. Two models on one host are two budgets, so that host sees up to `embedding_concurrency` times the number of identities at once.
- **The project scanner walks once, and stops walking what the repository ignores.** Measured on this repository: 24148 files visited became 2729, and 530.6 ms became 57.5 ms. The `.gitignore` question was settled by measurement - a dot-directory rule removes 88% of the files, and full `.gitignore` support adds 207 more, every one of them already inside a dot directory.
- **The ingest path takes the lock it never took.** Three shared read-modify-writes were unlocked; a four-process race behind a wall-clock barrier loses 74 of 100 entries without them.
- **A language tie-break in the scanner no longer depends on the host locale**, in a module whose whole contract is deterministic regeneration.
- **The prose recognizer on the write path carries no word list**, which its sibling module's rule had forbidden all along.

### Breaking

- **`o2b install` resolves the vault through the same chain as every other verb.** It previously used `--vault`, then the config `vault` key, then `VAULT_DIR`. It now uses `--vault`, then `VAULT_DIR`, then a project pointer, then the active profile, then the config key, and it expands a leading tilde where it previously passed one through literally. Where the answer changes, the new one is what `o2b status` and `o2b brain doctor` were already using - the old behaviour let the installer write one vault into an agent's config while the rest of the CLI read another, with nothing reporting the split.
- **`o2b doctor --readiness` exits 6 for a probe that did not complete**, where it exited 1. A timed-out probe was reported as a failure, so on a loaded machine the doctor declared a healthy system broken. Code 6 is the number `o2b search check` already spends on the same condition, and a test asserts the two cannot drift.
- **`o2b brain maintenance run` exits 7 when a task is refused for a failure streak.** Not 1, which would report a failure this run did not have; not 0, which would tell a nightly cron that a lane that has stopped doing heavy work is healthy.
- **Two new `_brain.yaml` blocks**, `maintenance:` and `embeddings:`, both absent by default and templated commented. A vault that declares only one of `embeddings.sunset_model` / `sunset_at` fails to load rather than ignoring the half it got.
- **Two new doctor warnings and three new uncertain codes.** Under `--strict`, a vault that previously exited 0 can now exit 2.
- **The hook self-ceiling default falls from 55 s to 8 s**, derived from the 10-second host timeout every hook entry declares. The old default could never fire, because the host killed the process first.
...
- **A recovery-point check reported a vault whose archives could not be read as a vault that never had any.**
- **The host-pressure probe read the root cgroup rather than the one governing the process**, so a CPU quota in force yielded a confident percentage computed from the host's run queue.
- **`brain_dream`, its staged actions, `brain_brief view=operator` and `brain_review_candidates` ran without a deadline** while the documentation said otherwise.
- **The self-heal rows paired on a process id** in a file that syncs between devices and whose own docblock forbids machine-local values in a row.
- **`Bun.which` and `Bun.spawnSync` resolved against the PATH the process started with**, so a long-lived server would answer with a stale binary's result.
- **The test suite's verdict was a function of the machine it ran on, for the third release running.** `doctor()` consults the codegraph partner whenever that binary is on PATH, and the partner's cache lives under HOME - which this suite pins to a clean directory. `tests/cli/cli.test.ts` went from 96.03 s to 1.09 s with the partner switched off, same tests green. CI never saw it, because the binary is absent there.

### Notes

Two censuses were rewritten because reviewers evaded them. The progress census went from five regular expressions to a lexer and three parsers: fourteen evasions now fail it, each with a permanent positive control, including one where a brace inside a generic constraint did not merely miss an interface but silently parsed the wrong one. Four limits it still has are written into its docblock rather than left implied. The verdict-vocabulary census now reads the tree instead of a hand-kept import list. And a new execution census runs each entry point for real and asserts records arrive, which no syntactic bypass can defeat.

`README.md` recorded, one release ago, a decision this release reverses: there was to be no registry of embedding-provider shutdown dates, because a hand-maintained table over an open set of endpoints would rot in place and be believed. The objection was right, and the design that ships answers both halves of it. A surveyed negative expires and degrades to undetermined; a positive carries the date it was reviewed; a model the survey does not know is reported as unsurveyed rather than as having no announced shutdown; and an operator who has read a vendor's announcement can declare it without waiting for a release. The reversal is stated rather than quietly performed.

Some of what was asked for is not here. There is no daemon, no scheduler and no supervisor, so the startup grace period and crash-loop recovery two task bodies asked for have no subject; the portable analogue is a gate evaluated at the top of a one-shot command with its state on disk. There is no cap on model inference, because this process makes no chat-model call - it hands model work back to the caller as typed envelopes - and naming a ceiling "inference" would be a hardcoded falsehood about what the code does. There is no concurrency in the project scanner, because rendering and writing are 7.9 ms of a 396 ms run. There is no whole-ingest writer lease, because the batch-planning surface exists so a caller can dispatch each batch as a parallel subagent and the sync lock has no retry by design.

Three things are stated rather than fixed. The confidence floor the new bench leans on cannot fire through the shipped retrieval path: min-max normalisation pins the first result at the keyword weight regardless of match quality, so the abstain that actually fires is "no matches". The reindex herd's thinning is exactly zero in the scenario it was built for - four parents probing inside the 0.71-0.75 s window all spawn - and helps only a caller arriving later. And `brain_dream step` still runs without a deadline, because bounding it means threading a guard into two functions that do not accept one; the documentation now says so on its own row instead of implying otherwise.

## [1.47.0] - 2026-08-15

Nine units picked by priority off the tracker, and reconnaissance against the real source before any of them was designed. That pass found they were not nine problems. In every one the capability already existed in this codebase - exported, tested, used elsewhere - and the defect was that the site needing it did not call it.

The clearest instance is the destructive-operation gate. `snapshot-gate.ts` opens by stating its guarantee: no destructive brain mutation runs without a recovery point on disk first. It had two call sites against roughly twenty-five destructive operations, and the consolidation pass took its own snapshot inline, which that same header names as the anti-pattern it exists to prevent. The second clearest is the ranker, which read a document's authoring instant nine lines after it had finished ranking on the file's modification time. Search classified sources through two functions the third write path never called. Five of six export paths never called a redactor with eighteen importers. A live embedding probe ran on every check and its answer was downgraded to a warning the exit code ignored. The consolidation pass compared raw strings where the read path canonicalised. A bundle carried preferences and the import counted them.

So the argument is not a subsystem. It is that a mechanism which must be called by hand is a mechanism that will be missed - and the answer is declaration plus a census test, an idiom already running here for raw `fs` write sites, for vault-identity assertions, and for doctor exit codes. Two registries were added on that pattern rather than routing twenty-five operations through new shared code, which is the change least compatible with the byte-identity this project measures rather than asserts.

Two of the nine task bodies were corrected by the reconnaissance rather than followed. A credential scanner was filed against the git commit path; the vault has no git transport and deliberately never will, a constraint written verbatim in seven design documents, so the intent landed on the boundary that exists - the export surface. An alias task was filed against the sub-recalls the consolidation pass issues; that pass is synchronous and issues none, and the real asymmetry was its clustering key.

Then five reviewers were given the finished branch with no knowledge of how it was built, and told that a comment is a claim rather than evidence. They returned thirty-two findings, twenty-seven reproduced against real vaults, and the release's own argument caught it out three times. The gate's docblock claimed its retention pass was safe "by arithmetic rather than by luck" - the archive listing sorts by modification time, so a pre-existing archive with a clock-skewed timestamp survived and the recovery point just written was pruned, after which the live tree was destroyed and the verdict read `covered`. The destructive-site census, the thing that was supposed to make the fix last, had six syntactic bypasses and a gate flag that was per file rather than per site, so one commented-out mention exempted every removal in a module. And a tool description promised its pages were excluded from ordinary reads while the retrieval trust gate defaults off. A release arguing that uncalled mechanisms get missed had shipped three of them.

### Added

- **Note files have a lifecycle.** Rename, move, archive and delete over MCP and the CLI, wikilink-aware. Both the source and the destination go through the same nine-step path envelope every other note write obeys, so a `..` segment, an absolute path, a POSIX backslash, a symlink leaving the vault and a destination inside `Brain/` are refused identically on each. The relocation relinks rather than re-reading: `linkSync` attaches the source inode to the destination name, so byte preservation is a property of the operation rather than of a guessed encoding, and a note that is not valid UTF-8 survives intact. A case-only rename is decided by inode identity rather than by platform, and stages a third name so the bytes are reachable under some name at every instant. Delete sits at the top of the confirmation ladder - dry run by default, an explicit confirm, and a blast-radius count guard.
- **A rename says how fresh the evidence it acted on was.** It cannot honestly claim to have fixed every inbound reference: no write path updates the link table, so an index that exists is stale with respect to a relocation that just happened. The result names the files it rewrote, the ones it could not, the bare-basename spellings it withheld because they were ambiguous, and the state of the index it consulted. A withheld rewrite is visible rather than absent.
- **A dangling wikilink can be materialised.** The gap was reported in five places and closed programmatically in none: the repair lane decided `skip-missing-target`, the synthesis report emitted the advice "write the missing note", the doctor emitted structured targets and sources, and a stub renderer already existed. Scaffolding is opt-in, never a side effect of a repair pass, and it refuses when the index is only partially resolved rather than reporting no dangling links from an index that has not finished resolving.
- **A typed recoverability verdict, and a census over every destructive site.** The gate expressed "could not prove recoverability" as an untyped throw, so an operation deleting files no archive covers still reported a snapshot path. The verdict is a frozen object with sorted tokens, in the family the trust gates already use, and it distinguishes full coverage from partial, from unproven, from nothing at risk. The census enumerates removal sites syntactically - the write-site census's rule with a different call set - and each is routed through the gate or carries a written declaration of the coverage it actually has.
- **The export boundary redacts, and refuses twice.** A payload the redactor could only partially scan refuses rather than writing a file that cannot claim to be clean. A secret-shaped identifier - a filename, an id, a mapping key - refuses rather than being redacted, because redacting an identifier does not hide it, it renames it, and two names that redact to the same placeholder collapse onto one path.
- **`o2b brain doctor` surfaces preference topics that collide once folded**, so the consolidation pass's contention warning has a tool behind it rather than an instruction to go looking.

### Changed

- **The freshness prior reads the content's authoring instant**, falling back to storage modification time. A batch of historical conversations imported today no longer all receive the maximum freshness boost as though captured this morning. A declared instant later than the reading clock is refused at both index time and rank time, with different dispositions: the index drops the value so a poisoned row cannot outlive many index runs behind the content-hash fastpath, and the ranker falls back without touching what is stored. Falling back rather than clamping is deliberate - a clamp leaves the note at the top of the decay curve forever, which is the defect. On a corpus declaring no authoring instants, ranking is byte-identical to the previous release, measured rather than asserted.
- **The consolidation pass folds the topic key it clusters on**, with the same normaliser the read path already used - Unicode normal form, case, whitespace, quote variants, no language-specific rule anywhere. Two distinct preferences landing on one folded key plan nothing at all rather than letting filesystem enumeration order decide which rule gets retired, and the contention is named in both summary shapes. Exactly five code points of 1,112,064 fold differently from before.
- **Preferences restore from a bank bundle** through the audited transaction that owns their lifecycle rather than being counted and discarded, so the confidence band, trial window, revision counter and audit trail are maintained. A row whose trial window is inert restores on a window derived closed from its own timestamps and is listed as derived, never folded into the restored count as though the backup had carried it. A row whose deadline is live is still refused, because no field it carries determines one and a fabricated deadline silently re-dates a promotion.
- **The distillation write path classifies its source.** It wrote claims under the top authority tier from a caller-supplied path with no trust classification at all, while the two paths hardened in the previous release both classified theirs. It now routes through the same two functions, records absence as a verdict rather than as the literal string `missing`, and bounds the source read to the vault - it was resolving `../../etc/passwd`, hashing it into the page, and leaking a raw errno through the boundary.

### Breaking

- **`o2b search check` exits 5 for an embedding provider that is configured and was proved unreachable**, and 6 when the probe did not complete. It exited 0 over a provider it had already proved dead, so a script gating on the exit code read it as healthy. Code 5 deliberately matches `o2b install --check` for the same condition, and a test asserts the two cannot drift. A provider that is not configured still exits 0 - absent is not broken. The `--json` key `provider_reachable` is replaced by `provider_probe`; a boolean had to answer four questions with two values, so a provider that refused and one that never answered were the same `false`.
- **`o2b brain bank-import` exits 1 when a rule was genuinely left on the floor**, where it previously always exited 0, and its result gains `derived`, `failed` and `topicKeyCollisions`.
- **A second top-level directory, `Archive/`**, receives archived notes. `Brain/` is refused for every note path, and an operator whose backup rule covers only `Brain/` should extend it.
- **`mergePage` fails closed on a malformed `Brain/_brain.yaml`**, because the link-rewrite walk now resolves vault scope like every other walker.
- The config-surface redaction placeholder is now `***REDACTED***`, matching every other redaction, where it was `[REDACTED]`.

### Fixed

- **The gate destroyed the recovery point it had just written, and reported success.** The retention pass sorts archives by modification time, so one pre-existing archive stamped in the future - the state `rsync -t`, `cp -p`, a clock-skewed server or a stepped system clock leaves behind - survived and the new point was pruned. Reproduced end to end: the live tree gone, the verdict `covered`, the named archive absent. Every gate user was exposed, not only rollback. Protection is now by run id rather than by sort position, so equal timestamps cannot decide it either, and the archive is probed on disk before the operation runs.
- **A rollback pruned the archive it was still restoring from.** Only the tar is extracted before the discard; the manifest sidecar and the derived-store archive are read afterwards. At the default retention this was reachable whenever the target was the tenth-oldest or older, and with derived-store coverage on it silently returned "coverage unknown" over a store that was never rolled back.
- **The destructive-site census could not fail.** Default imports, a `promises` binding, a bare `fs` specifier, dynamic imports, computed member access and `.mts` files all passed it, and its gate flag was per file, so a mention inside a comment exempted every removal in the module. All fifteen shapes are now fixtures.
- **A rename could write outside the vault** through a symlinked directory - a common Obsidian layout - and could rewrite paths the operator had excluded from indexing, returning their filenames across the MCP boundary. It also rewrote `Brain/log/`, which is an append-only record of what was said at a time.
- **A partial scan could be defeated by content quoting the marker**, so a 1.4 MiB payload exported with roughly 700 KB silently dropped and a success exit. Truncation is now a fact the scan returns rather than a substring search on its own output.
- **`okf-export` merged pages whose filenames were secret-shaped** into one bundle path, destroying two page bodies and exiting 0 with the pre-merge count.
- **A redacted markdown page's frontmatter was not valid YAML** - an unquoted placeholder opens an alias node - and the same expression consumed the line following an empty secret key, deleting a key that was never a secret. Caught only by asserting with a strict parser; this repository's own parser is lenient enough to accept it.
- **`o2b export-config` mangled the operator's own paths.** Folding the key-name-only copy into the shared redactor brought value scanning to a surface whose entire purpose is handing a support person the resolved paths, so a container id or a hashed cache directory was redacted out of the answer.
- **`continuity export` leaked exactly the tokens the guard exists to catch**, because it called the redactor with no options, and the census actively failed if the call was corrected. Its scan also ran at write time only, so a record already on disk was never examined.
- **An import restored a record whose identity had been redacted away.** A bundle carrying `***REDACTED***` where a preference id, topic or alias belonged was written into the vault under that name, and two such records collided on it. Identity-bearing fields now refuse per record, on both the preference and the page-graph halves, while a placeholder in a principle or a body still restores - that is an honest redaction, and the difference is the same line the export side draws.
- A prune that could not remove an archive removed its manifest and derived-store companions anyway, orphaning the survivor.
- Both new CLI verbs were missing from the help surface, and `--help` on them was a usage error.
- The stub tool leaked the absolute host vault path into model context, and accepted arguments belonging to the other action without reading them.
- An AWS secret access key passed the export boundary verbatim, because the entropy detector's character class excludes the two characters base64 needs.
- Two readiness tests that spawn the CLI twice were budgeted; they fitted the default only on an idle machine, so the suite's verdict depended on the load of the box it ran on.

### Notes

Some of what was asked for is not here. There is no repo-visibility verification, because there is no push path whose destination could be verified. There is no cascade delete of linked memories: a cascade is strictly more dangerous than the single-note delete this release introduces, and building it on a recoverability gate that has not shipped once inverts the order of risk. The trust-classification registry the consultant proposed was not built, because there are three such write paths and a census over three items costs more than it protects. The distillation quarantine admits its limit rather than overstating it: the marker is written and read correctly, but the retrieval trust gate that acts on it defaults off, and turning it on changes three signals, the result shape and a cache key at once - a release-wide decision that does not belong to a tool description. The description names the setting, and a test fails if the default changes and the wording does not. What the destructive-site census does not hold is stated in its header rather than implied: the atomic-overwrite class is excluded, one member of it overwrites user-authored bytes that no producer can regenerate, and a test measures that the gap is real rather than leaving the claim to rot.

## [1.46.0] - 2026-08-15

Nine units, and the same argument nine times: a claim the code could not back up. Four came from the public tracker and five from an upstream mining pass, and reconnaissance against the real source ran before any of them was designed. That pass changed the shape of three fixes and killed a fourth. The one the issue asked for - let the MCP host supply the trust verdict instead of the caller - turned out to be unbuildable, because there is no unforgeable caller identity anywhere in this surface, a conclusion this project had already reached and written down in `src/core/write-binding/index.ts` and then not applied here.

What the nine have in common is not a subsystem. It is that the information needed to tell two cases apart either existed already and was discarded, or cost one filesystem call nobody made. A closed union classifying a store fault was computed and then interpolated into a sentence. An embedding error carried a category that was stringified and dropped. A rank cap and a pre-visibility count were computed and thrown away. Trust was decided from the shape of a string with no file behind it. Readiness was asserted from the presence of files rather than read off disk. Freshness compared two modification times and had no clock at all.

Extraction happened only where duplication was measured rather than merely suspected. One segment-wise path predicate replaced six copies. One wall-clock age helper replaced five, which spelled a day three different ways and disagreed about whether a failure to read meant fresh or stale. One write-result envelope replaced four hand-built shapes across the note-write tools. Nothing was extracted because two things looked alike.

Some of what was asked for is not here. There is no embedding-provider sunset registry, because the only implementation is a hand-maintained table of shutdown dates over an open set of endpoints and no provider exposes the metadata, so it would rot in place and be believed. There is no per-caller filtering of the advertised tool catalog, because it depends on per-client surface state that has not shipped. The schema-completeness guard covers input schemas only: the output vocabulary declares union-typed fields with no `type` on purpose, so the rule would demand a lie there, and 202 of 202 output property nodes carry no description, which is a separate decision rather than a side effect of this one. The new argument gate enforces the declared closure at the top level only, and the fifteen nested nodes it does not reach are named in its module header rather than left as a second unenforced promise. And `distill-source.ts` has the same defect as the intake trust fix, with no trust classification at all; it has no reporter, it is not in this scope, and it is filed rather than quietly included.

Four reviewers were then given the finished branch with no knowledge of how it was built, and told that comments in the diff are claims rather than evidence. They found twelve defects, most of them reproduced against real vaults, and two were regressions this branch had introduced: a snapshot gate whose lost-race retry was dead against the real creator, recovered by the previous release and not by this one, and a rank-cap degradation that fired whenever a corpus ended exactly on the cap, which broke the byte-identity this release claims for a healthy search and filed an invented recall gap into telemetry. The branch's own tests could not see either: the first injected a synthetic error instead of calling the real creator, the second used a two-document vault against a cap of thirty. Every one is fixed below.

### Added

- **A positive `vault.include_paths` allowlist.** Vault scope was exclude-only, so indexing nothing but `Brain/` meant naming every other top-level directory and every stray root note, and anything created later was indexed silently until someone noticed. Absent, the key changes nothing, and that is measured rather than asserted: a recorder ran against the previous release over a fixture vault and pinned what each of the three walkers returned, and those literals are the test. An empty list is refused, because a list admitting no path is an off switch rather than a boundary; a `..` segment is refused, because it matches nothing the walkers produce and would defeat the check meant to catch it by resolving outside the vault. Requested by @lymeswold in #155.
- **A retrieval trail on every search answer.** Fifteen degradation signals existed and none reached a caller as anything a machine could read. Each code names a site that exists, the detail carries identifiers and integers only, and both the CLI and the MCP envelope get it through one function with the codes declared in the output schema, so an undeclared one fails the contract rather than travelling. An empty result now names its cause: from the trail when a degradation attributes it, and otherwise from the corpus classifier that already existed and was reachable from exactly one surface.
- **Write operations return the lint findings for the pages they just wrote.** The vault lint was a separate pass measured at 110 ms over 2200 pages, so an agent that wrote a broken wikilink learned about it from a sweep that might never run. The per-page pass reuses the three detectors that already existed, costs 6.5 ms for one page and 6.9 ms for five, and never walks vault content. It runs after the commit and never gates the write. Update, append and batch previously ran no document validation at all, so callers will now see findings on pages that were accepted silently before.
- **A channel dimension on recall-delivery telemetry, and a doctor check that reads it.** A "recall never fires" report was undiagnosable, because a hook that was never installed and a hook that runs and decides not to inject produce identical evidence, which is none. The channel is required rather than optional, so every emit site had to name itself to compile, and the injecting hook now emits for all three of its decisions, since a hook that ran and abstained is exactly the signal that separates the two silences.
- **Readiness reads installed state off disk**, through each adapter's own verify, and gained an `unknown` status, because a manifest can exist and refuse to parse and among pass, fail and skipped there was no answer for that which was not a lie in one direction.
- **A wall-clock ceiling on materialisation freshness**, `health.materialize_max_age_days`, and a three-state verdict with named reasons in place of a boolean.
- **Every advertised tool parameter is documented, and CI says so.** Fifty-nine parameters across thirteen tools carried no description and nothing guarded it. The other five completeness dimensions were already clean at 108 of 108, measured rather than assumed, and are now ratchets. A composition keyword the walker cannot follow is a violation rather than a silent skip, which matters precisely because there are none today.
- **An unknown tool argument is named back to the caller** with a suggestion from pure edit distance over the schema's own property names, instead of being silently ignored. All 108 tools declared a closed argument set that the server enforced on none, so a caller who sent `quiery` got a success envelope computed from defaults.

### Fixed

- **A concurrent write dropped its record.** Two writers raced the same free-name probe, both picked the same candidate, one won the exclusive create and the other exited with "already exists". Replaying the pre-fix sequence across eight subprocesses lost twelve events out of twenty-four in three rounds. The defect was never signal-local: dead ends and capture notes had it too, and the signal writer alone has eight callers, three of which write many records per run on the same date. Reported by @daltondellinger1-coder in #161.
- **Intake trust was decided from a claim nobody checked.** `brain_intake_entities` classified a source by the shape of a caller-supplied string and never asked whether the file existed, and the caller is the same agent that extracted the entities from the content being classified. What the fix does not buy is stated in the code, because it matters: an attacker forced to name a real file names `README.md`. The free bypass is gone and the claim is auditable through a recorded content hash; the lie is not. Reported by @daltondellinger1-coder in #160.
- **The Hermes plugin and the core resolved different vaults.** The Python resolver's docstring claimed to mirror the TypeScript one exactly and mirrored two of its four steps: no project-pointer walk-up, no active-profile lookup, no tilde expansion, and the first duplicate key won where the core takes the last. A parity table driven through both implementations found eighteen disagreeing rows, two of which nobody had named: a value with backslashes came back JSON-encoded and therefore resolved to a different path, and `agentName` beat `agent_name` by file position rather than key priority. An unreadable config no longer reads as unconfigured. Reported by @kaykayyali in #130.
- A snapshot collision was unrecognisable to the gate written to retry it, so a destructive operation aborted where the previous release recovered.
- A malformed `merged_into` value produced neither a fix nor a finding, printing a clean vault over a link pointing at a page that had been merged away.
- The Hermes resolver parity check applied to every user of the command line, because `plugins/` ships in the published package, so a machine without Python had a permanently failing `o2b doctor` telling the operator to install Python for a plugin they do not use.
- A dead-include-root lint fired at error severity on a legal bare-name root, asking `existsSync` at the vault root about a rule whose grammar matches at any depth.
- `clusters run --if-stale` swallowed a configuration it could not parse and answered `fresh`, exiting zero over a vault every other verb refuses loudly.
- The new unknown-argument suggestion ran an unbounded edit distance over a caller-supplied name: one call carrying a one-megabyte argument key blocked the event loop for fourteen seconds, inside the transport's own body cap. The bound is exact rather than approximate, so it changes no answer.
- A raw retriever error message reached a synced, model-readable telemetry payload, and raw operator paths reached the four write tools' responses and an intake error, which also made that tool an existence-and-permission oracle over an arbitrary path.
- An unreadable continuity store silently erased the entire recall-channel check, producing exactly the undiagnosable silence that check exists to remove.
- Five copies of wall-clock age from a modification time disagreed about the failure: one treated an unreadable file as stale, one returned age zero so an unreadable note read as brand new, one let the raw errno abort a whole scan.
- Two operator surfaces over-reported index coverage: the scope walker and the path inspector never applied the admission filter the indexer applies.
- The readiness module promised a per-check timeout it cannot honour for a probe that blocks the event loop, and now says so instead.
...
- **A starter vault went stale ninety days after the bundle was authored.** The dropped bundle carries a fictional five-day history with fixed May 2026 dates, one of them the evidence behind a `pinned` preference, so from 13 August 2026 on every vault initialised with `--starter` reported `pinned-without-recent-evidence` on its first `o2b brain doctor` run. The bundle is now aged to the day it is dropped: every date moves by the same whole number of days, so the trial window still open, the evidence two days back and the retired entries months behind all survive the move, and the starter is clean whenever it is installed rather than for one quarter after it was written. `bootstrapBrain`'s `now` seam, reserved for exactly this and until now unused, is what the tests pin it with.
- A test that deletes `OPEN_SECOND_BRAIN_CONFIG` from the real process environment never put it back, and every test file the runner ordered after it lost the hermetic default the suite's preload installs. On a developer machine the fallback to a real `~/.config/open-second-brain/config.yaml` hid it completely; on a bare runner it was forty-two failures or four, depending on where the ordering landed, and it turned the suite's verdict into a function of the machine it ran on.
...
In six of the eight, the information needed to tell the two cases apart was already computed and then discarded. The section budgeter returned the list of keys it dropped and its caller kept only the text. The manifest walker hashed a tree whose sibling store it did not mention. The retirement path wrote the alias that makes the reverse lookup possible and nothing performed the lookup. The trigger store computed a recurrence and held it for the duration of one scan. A grounding pass over the code preceded the design and falsified a load-bearing claim in six of the eight task descriptions, so what shipped has a shape none of them predicted.

Two independent reviewers then read the finished branch against the base, with no knowledge of how the work was done, one told to treat every claim as unproven. They found this release's own theme broken by its own code in five places, and each is fixed below: a cron health gate that passed on input its parser could not read, a test-runner guard that reported its own setup failures as concurrency, a staleness audit that discarded a computable finding and then said nothing was knowable, an unreadable optional manifest field that voided the record drift detection runs on, and a per-record refusal with no per-record boundary that took a whole queue down with one corrupt file.

Some of what was asked for is not here. There is no maintenance-lane task that runs the external code indexer: it would be the first lane task to spawn a foreign binary and the first to touch state outside the vault, and the cron recipe delivers the capability without either. There are no snapshots at session, plan or decision boundaries; the history is typed and has a log surface, but changing when snapshots happen interacts with retention and with the derived store's size and should be measured first. Retention still orders by modification time, so a replicated archive arriving from another device can evict a locally newer recovery point - real, and a data-loss path, but the fix changes which snapshot a retention pass deletes and deserves its own release rather than a line in this one. The doubled injection ceiling is untouched: two preamble sources are each budgeted against the full configured limit, so the effective ceiling is twice the configured number, and halving an operator's context window is not a silent change. And the sixteen sites that inline a hash with an ad-hoc truncation are left alone, because they persist nothing this release reads.

### Added

- Schema-pack integrity is reported as `ok`, `modified` or `unverified` with a named reason, on both read surfaces and in `o2b brain schema report`. The expectation it compares against is new: applying a mutation now seals the pack by writing a digest of the rendered schema block into its audit record, so a hand-edit after an apply is detectable rather than indistinguishable from the pack that apply produced. Digesting the rendered block rather than the file is what makes it invariant to comment and whitespace churn elsewhere in the config.
- Negative recall is typed. `not_found`, `unknown` and `did_not_happen` are separate answers, and a complete negative requires a digest-bound receipt over the document set actually searched - the index side, with the divergence from the configured note roots reported rather than folded in. Every negative therefore says how far it was checked: an absent index, stale embeddings, an unreadable index instant and a configured root the index does not cover each produce `unknown` with their own reason instead of a confident nothing. A vault that never configured an embedding model still receives a `not_found`, with the vector count and the absent model signature inside the receipt and its digest, because running without embeddings is a supported configuration rather than an unfinished job - and a verdict nobody can obtain stops being read. An interrupted embedding run is the case that is genuinely indeterminate, and it says so.
- A reverse stale-dependency audit joins the instant a state stopped being current against the instant each consumer was written, and names the context receipts, decision receipts and live records that rest on it. The join needs no new store: retirement already writes the former identifier into the retired file's aliases, so the backlink index answers across the rename. It plugs into `o2b brain doctor` as a warning. The arm that reads the backlink graph needs no telemetry and always runs, so a vault with the receipt gates off still gets every finding the evidence supports; what an absent receipt trail costs is named separately, and only when something actually stopped being current.
- `o2b partner codegraph resync --cron-template` emits an installable recipe that re-indexes an external code graph after the watched repository moves. Open Second Brain writes nothing: the recipe is text, and every write in it is a shell command the operator's own scheduler runs on their host, which keeps the module invariant that this project never writes into the partner's store. `o2b partner codegraph report --fail-on-health` gives the health verdict an exit code, so a script can gate on it - the report returned success unconditionally before, including for an index built against the wrong root.
- The proactive trigger queue gains `suppress` and `unsuppress`. Suppression is indefinite by construction rather than by a second timer, because expiry applies only to open statuses, and it records the status it interrupted so restoring returns the finding to exactly where it was, with its original cooldown arithmetic intact. Alongside it, the recurrence ledger the audit trail needed: a blocked finding now increments a persisted count inside the directory lock, so the operator can ask how often a silenced finding has fired since they silenced it.
- `o2b brain snapshot` can cover the derived store, off by default. What it protects is spend rather than information - a rebuild restores everything except the embeddings, which cost an API key - and retention keeps ten copies inside a vault that replicates to every peer, so inclusion is a cost decision and the live store size is reported in every manifest whether or not it was included. Every incompleteness is named: an absent store, a store the integrity scanner has condemned, and one over the configured ceiling each refuse with their own reason, and the store is archived before the tar so a refusal leaves nothing behind.
- Every recovery point carries a typed reason, the event log gains the `snapshot` kind it had been missing since `rollback` was added, and `o2b brain snapshot log` completes log, diff and revert over the snapshot family. A reason written by a build that did not know it reads as unrecorded rather than being guessed from the run-id prefix.
- An operator-authored `Brain/standing-rules.md` is injected first, ahead of everything the agent learned about itself, and is exempt from the adaptive budget. It survives a memory-layer failure because it is read outside that boundary, and it is never written to the injection cache, so a stale constitution cannot be served from disk. The file is treated as opaque bytes throughout - read, trim, and cut at a line boundary - so it behaves identically whatever language the operator writes in, and its truncation notice is built from counts alone.
- Truncation says what it dropped. The section budgeter already returned the list of dropped keys and the caller kept only the text, so the notice named nothing; it now names the sections that were cut and the character delta.

### Fixed

- On a host without `zstd`, the gzip fallback wrote the snapshot archive with a plain file write and silently overwrote an existing one, while the `zstd` path refused - and the run-id collision logic depended on that refusal. Two concurrent destructive operations could therefore destroy each other's recovery point with no error. Both paths now refuse through one guard ahead of either compressor.
- A rollback replaced every entry under `Brain/` and only then looked for the derived-store archive, so a missing companion threw over work that had already completed: exit 1, nothing logged, and the vault rolled back regardless. Both conditions are checked before anything is touched, which is the ordering the create path already used.
- An unreadable optional field in a snapshot's sidecar failed the whole manifest closed, file map included, which reproduced the loss that keeping the manifest version unchanged exists to prevent: a peer on a later build writes a value this one does not know, this one discards the record, and the rollback gate then reports no sidecar at all and skips drift detection. Each optional field now has three states - valid, absent, or present and unreadable - and the third is recorded rather than folded into either of the others.
- `o2b brain schema sync` returned a success report and performed no work at all; it did not even take a vault. It refuses now, because what a schema backfill should write is undefined and an invented one would rewrite notes nobody asked it to touch.
- An absent `Brain/_brain.yaml` was substituted with a valid empty schema pack, so a vault with no ontology was indistinguishable from one that declares nothing.
- A malformed grounding-artifact list on a trigger degraded to empty, so an unparseable finding presented as an ungrounded one. It refuses now - and the refusal is confined to the record that is broken, because the first form of it had no per-record boundary and one corrupt file took down list, history, the morning brief, delivery marking, every transition and the scan that records new findings. Two callers then swallowed that into a bare catch and the brief rendered with its pending section absent, which is indistinguishable from an empty queue. Every surface now reports what it could not read alongside what it could.
- A trigger record with no creation instant was written back with an empty last-seen field and refused on the next read, so its first recurrence write made it permanently unreadable.
- The occurrence counter was race-free only against itself: the two writers outside the scan path wrote the whole record from an earlier snapshot without the directory lock, so a scan interleaving with a suppress lost one of the two.
- A decision-receipt directory that could not be read reported as a vault that had never recorded a decision, which additionally defeated the duplicate-append guard directly above it.
- The snapshot diff classifier predated per-device log sharding and mislabelled both the sharded Markdown and the machine-primary log - the surface a pre-restore diff is most likely to be showing.
- `Brain/.artifacts/` is documented as never backed up and was archived in every snapshot and hashed into every manifest, so ephemeral tool output tripped the drift gate.
- The derived-store archiver resolved the store by its default location and ignored a configured override, so a vault with one either refused as absent for the wrong reason or archived a stale database and reported success.
- An operator configuration error in the vault-instruction path was swallowed into a null field: the key simply vanished from the response and the operator was told nothing.
- The emitted resync recipe gated on `if jq -e`, and a bare `if` collapses every non-zero exit into one answer - so a parse error read as no mismatch and the recipe re-indexed over a root nobody had verified, against the comment three lines above it. The status is read explicitly and only a literal falsy result is a pass.
- Shell completions hand-listed the three root commands with subcommands, so every `o2b partner` verb had been offered by no shell since that surface was added, and an absent completion reads as a command that does not exist. The groups are derived from the command manifest at every depth now, and a test asserts each shell's output covers them.
- `o2b brain trigger`'s verb map routed any accepted verb that was not `ack` or `dismiss` to the act transition, and `o2b search reindex` declared none of its eight flags in the command manifest, so help and completions could not offer the cron template.
- Two test files ran CLI invocations concurrently. An in-process run swaps the environment, the working directory and both output streams and restores them in a `finally`, so overlapping runs restored each other's swapped state: one left a temporary config path installed for the rest of the process and seventeen assertions in unrelated files failed with a vault that could not be found, at a stack with nothing to do with the test that broke them. The runner refuses to overlap now, which immediately surfaced a second such race that had been silently wrong.
- The tree-wide format check walked into nested git worktrees, because the vendored bundle it excludes was matched by a root-relative pattern, and failed for changes that were entirely clean.

### Changed

- One digest encoding. Ten modules carried a byte-identical private SHA-256 helper and three reimplemented canonical JSON serialization independently, which was survivable while every digest stayed inside the process that computed it and stops being survivable the moment one is persisted: this release persists two digests, into a vault that replicates between machines. The three canonical-JSON copies disagreed about an object entry whose value is `undefined`. In the two that feed a persisted digest the divergence was unreachable, because every parser building their input uses a conditional spread, so no stored value changes. The third keyed an in-process request cache and rendered such an entry as an explicit null, which was distinguishing two requests that are byte-identical once they leave through `JSON.stringify` - so absorbing it closes a latent cache miss rather than changing a value.
- Every closed verdict vocabulary this release introduces registers in one census that asserts its frozen object, its membership list and its type guard agree, with synthetic negative cases proving each assertion bites. The status partition behind the trigger queue was three hand-maintained copies across core, CLI and the tool layer, with a fourth copy of the status list as a literal in a tool schema and nothing asserting the two matched; there is one owner now.
...
- **The committed OpenClaw bundle was two releases stale, and the gate that would have said so only ran on a tag.** `openclaw/index.js` was last rebuilt at 1.37.0 while its source changed at 1.43.0, so the plugin bundle OpenClaw users install did not contain that release's quarantine filter - the release workflow failed on exactly this check for both 1.43.0 and 1.44.0 and skipped its publish job, which is why the releases were cut by hand and the breakage went unread. The bundle is rebuilt, and the same check now runs on every pull request so the change that staled it fails rather than the tag that ships it.
...
The rule for the refactoring commits was that behaviour is byte-identical, and the existing suite is what proves it, so no test may be edited to make one pass. Where that guarantee was load-bearing it was checked directly rather than left to the suite: the config validator was run against its predecessor over 3135 generated configs comparing resolved values, key order, warnings and thrown errors; the search command line was run through 92 invocations on identical fresh vaults with stdout, stderr and exit code diffed byte for byte; the store's schema statements and the recall pipeline's cache-key composition were diffed line for line against their previous versions, because either one changing would strand data already on disk without failing a test.

Three ratchets grew rather than being excused. The terminal-state census followed the code out of the file it used to read, and three exclusions that predate this wave turned out to be detectable once it did. The doctor's code census now enumerates its check modules from disk. A new census walks the static import graph and fails on any cycle at all.

Independent review of the finished branch - four reviewers given the diff and the claims, with no knowledge of how the work was done, one of them tasked with treating every changed test as guilty until proven innocent - found fourteen further defects, four of them regressions this wave introduced. Two are worth naming here because they are the shape this exercise is supposed to catch. Narrowing a swallowed database error to the one condition it documented turned a lane that only ADDS search candidates into one that could fail the whole query. And the new import-cycle ratchet was proved to pass on an empty set twice over - with its source root pointed at an empty directory, and with its patterns altered to match nothing - so it would have reported an acyclic graph over a fully cyclic one, permanently and silently. Every finding is fixed and named below; each ratchet now carries a demonstration that it can fail.

The silent fallbacks are the part with user-visible behaviour, and review turned one correction into a theme: absence and inability to look are not the same answer, and nearly everywhere this code conflated them the result was not imprecision but destructive advice. An unreadable vault root was told to run `brain init` over a populated Brain layer; an unreadable index was told to re-initialise an index that already existed; a manifest behind an untraversable parent was told to run an update that regenerates the file and leaves the permission fault in place. Nine such sites across six subsystems are closed.

### Changed

- The vault config validator is one module per `_brain.yaml` block, each owning its default table, its read-side resolver and its parser together, above a shared constraint vocabulary and key index. The public path and all fifty-five exports are unchanged, so none of the seventy importing modules moved. Nine duplications and dead branches went with the split: a YAML-safety loop written three times, an array guard written at five sites, two bound checks with one message, a dead `Set` branch, a ternary whose arms were identical repeated across six blocks, a default parameter no call site overrode, a boolean check split out of the loop handling its four siblings, validation sitting a thousand lines from the construction it guarded, and three parameters threaded loose through twenty call sites.
- The search index store keeps its class and its full method surface - twenty-five modules hold that abstraction - and the bodies moved into seventeen modules, one per table or lifecycle concern, each taking the database handle as an explicit parameter. The on-disk format is untouched: the schema module has an empty diff, the version is still 10, and no table-creating statement exists anywhere in the split.
- The recall pipeline reads as the stages it runs. Each stage is a module with an explicit input and output type, and the gate deciding whether a stage runs is visible at the call site rather than buried inside it. Warning order is preserved exactly, because it is observable: each stage returns its own list and the coordinator drains them in the original sequence.
- Every `o2b search` verb is its own module under the layout the brain verbs already established, rather than a second layout invented for it. Six duplications went with it, including the vault, config and database flag schema written out fourteen times and the whole report-emission block copy-pasted between `index` and `reindex`, comment included.
- The vault doctor is a registry: a twenty-six entry array a new check joins by being appended to, over one module per check family. Each check declares its own fail-soft flag beside its implementation, encoding exactly the guard placement the single body had. The forty-one diagnostic codes are unchanged, verified by running the census's own patterns over both trees.
- The dream pass separates deciding from writing. The apply module owns the order of writes that keeps the on-disk invariants true and decides nothing; the planning modules decide and write nothing. Moving the rename out made the new module write-capable and the vault-guard census correctly failed on it - the fix was to make the guard honest by asserting write identity on entry, not to add an exclusion.
- The two import cycles are gone. Both existed because one large module played two roles at once: declaring a value everyone needs, and aggregating types from its own dependents. The search error vocabulary, the ranked-row shape and the vault-relative path constants now live in leaf modules that import nothing, re-exported from the modules that used to declare them. Both export surfaces were compared symbol by symbol against the previous commit through the TypeScript checker rather than by eye.

### Fixed

- A `_brain.yaml` that is present but malformed no longer reads as one that is absent. Five config readers shared a blanket catch that collapsed the two, so an operator whose configuration had a typo was served the documented defaults while every surface reported healthy. Absent stays byte-identical and no longer even opens the file; malformed, structurally invalid and unreadable each raise with the path named. No new diagnostic code or configuration key was needed, because every call site already had a visible channel: the feedback scope raises before the durable write, since a signal recorded under a silently wrong scope is a record no later fix reaches, and the snapshot retention read raises inside the gate's existing best-effort catch so the destructive prune is skipped rather than run at a guessed retention. The one reader that still resolves rather than raises is the integrity gate, which now documents why - the search layer reads it on every store open inside a fail-soft path, and its strict fallback means an unreadable config can only close a gate, never open one. A test pins that asymmetry so the two shapes are not later unified by someone tidying.
- The doctor now says when it could not look. Two sweeps caught every read failure and returned nothing, and for a diagnostic no findings renders as a clean bill of health - the operator was told the vault was fine exactly when the check was unable to examine it. A missing path stays silent, since a path that is not there was never a subtree to sweep; a permission failure, an I/O error or a file standing where a directory was expected now names the path, the reason and the specific claim the sweep can no longer make, through the doctor's existing uncertainty stream rather than by raising, because a doctor that dies on the first odd directory reports less than one that continues. Two further catches of the same shape were found in the removed-tool scan and fixed with it.
- One of those guards could never have fired. The vault walk beneath it swallowed every directory failure internally, while accepting a notices sink in its own options type and dropping it - so the guard above was inert and the real silence was one level down. The walk now honours the sink its sibling branch already did.
- An unreadable search index no longer narrows recall while looking healthy, and no longer fails the query either. The trigram candidate lookup caught every database error and returned an empty candidate list; narrowing that to the one condition it documented then raised for everything else, which review showed could kill a query the keyword and semantic lanes had already answered - an indexing pass holding the write lock past the busy timeout is enough, and so is an index whose trigram tokenizer the running build does not have. The lane only ever ADDS candidates, so it degrades and the query carries the reason, from a closed vocabulary that never forwards the database message: the full-text engine reports a rejected match by echoing the expression, which is built from the caller's own query terms. The absence predicate is also anchored now - a missing shadow table means the index has lost one, which is corruption, and it used to read as the table never having been built.
- A neutral fold of the learned recall weights could attach a ranking reason to itself. Two booleans were derived separately from one read, one gating the composition and the other gating its explanation, so using the wrong one credited a fold that did not move the ranking. They are one nullable value that is non-null exactly when the fold applied.
- A `void parseFrontmatter;` statement in the doctor claimed in its comment to prevent a dependency being stripped, while the import was genuinely used the whole time; a `void basename` in the dream pass claimed a barrel re-export that never existed; and a planning function there was a pure stub. All three deleted.
- A vault that cannot be examined is no longer reported as one that was never set up. An unreadable root made the doctor advise `brain init` over a populated Brain layer; an unreadable index made the code-graph partner advise re-initialising an index that already existed; a manifest behind an untraversable parent was reported missing under a fix that regenerates the file and leaves the permission fault. The status payload's `vault_exists` field, which also gated three blocks, answered `false` for a vault it could not stat, so an operator saw four confident negatives for one missing execute bit. The field carries a named reason on that branch only - both answerable cases stay byte-identical - and the blocks it gates degrade themselves rather than being omitted, since an omitted block reads as the same confident negative. The second runtime publishes the same field from its own payload and now shares one formulation of the condition rather than a second copy of it.
- The plugin config read failure had nowhere to land. The command-line dispatcher formats two error classes and rethrows the rest, so an unreadable config printed a source excerpt and eight stack frames with zero checks reported - worst on `doctor`, which prints the correct diagnosis the moment the crash is out of the way - and the JSON surface produced empty output plus a trace. It refuses with a formatted message now, both JSON surfaces stay parseable, and the remedy travels in the error because it escapes through four unrelated channels. Two tool surfaces computed their whole payload and then threw on the one field that reads the config, discarding the report that names the broken file; both report it in the field instead. The server resolved the agent identity from that same config before any handler ran, which made those repairs unreachable - including in the handshake, so a real client would never have issued a call at all. The identity is resolved where it is used now, still raising, still never substituting a placeholder name.
- An existence check in front of the config read returned false for every errno, so a readable config behind an untraversable parent silently reverted every setting - in the commit that claimed to have fixed exactly that. It is one stat now, which makes the split free: the absent case pays the syscall it always did and constructs no error. Invalid UTF-8 was never really read as missing either; it produced an empty config that every resolver reads as "the operator set nothing", while the write path would have persisted the file with each undecodable byte replaced.
- The pre-compaction memory pack died on a broken config instead of degrading. The integrity gate's strict fallback resolves owner-scope delivery to `fail`, which takes the scoped branch, which renders the active head, which now raises - so one bad line cost a scoped agent the whole pack. Compaction happens whether or not the tool answered, so a refusal deletes the memory at the one moment it cannot be re-fetched: the pack is delivered, the scope-narrowed head is withheld, and the reason rides on the payload.
- One uncapped, undeduplicated uncertainty entry per sweep meant four identical lines for one unreadable subtree, and an existence check in front of every sweep sent parent-directory failures past the classifier entirely. Three ordinary broken-vault shapes also made the whole diagnostic pass throw with zero findings, which the hoisted shared context had made the first thing it did.
- The `pref|ret|sig` artifact id grammar had two homes and could drift. It is now a predicate on the module that already declares itself the one place the wikilink grammar lives - the other direction would have closed an import cycle, which the new ratchet catches.

### Notes

- Three things were found and deliberately not changed, because fixing them alters output and that belongs in its own release: the same constraint renders two different messages depending on which config block declares it; the same command-line flag is refused with different wording by different subcommands; and several reads in the recall pipeline swallow a failure to keep a single unreadable file ranking neutrally rather than failing the query.
- The measured effect: no file in the repository now scores worse than the seventh-worst did before, the worst cognitive-complexity count in the touched set falls from 441 to 32, mean cognitive complexity across the tree falls from 34.9 to 30.7, and the import graph is acyclic.
- Three ratchets can now be shown to fail. The import-cycle census pins the size of the graph it searched, with separate floors for the two narrow declaration kinds, since the total is dominated by plain imports and either could stop matching without moving it; a type-only import stays an edge and a deferred dynamic import stays excluded, both asserted rather than left as comments, because counting the deferred ones closes two cycles on this tree - which is to say those deferrals exist to break them. The terminal-state census applies the same reason, class and cap gates to both of its inventories, so a state rescued by the widened module walk can no longer be excused without a written argument. Its hop limit was measured inert beyond one and is one now.
- One gap is stated rather than closed: seven degradation codes reach the doctor's uncertainty stream with neither a registered command nor a written exclusion. Closing it needs a judgement per condition.
- The next tier by the same measurement, untouched here: the preference parser, the skill proposal store, the search and recall tool surfaces, the indexer, the digest and the command-line entry point.

## [1.41.0] - 2026-07-27

A signals-that-survive wave: eight units that carry a signal the kernel already computes the last few metres, to the decision it should have changed. A repository declared which files it considers noise and the ingest planner ignored the declaration, while the hygiene scan beside it had honoured git for releases. A recall attempt was graded weak and the grade evaporated when the call returned, so nothing could tell that the same question had come back insufficient four times. A query named a period and the ranker went on applying a freshness prior pointing the other way. A parse error carried its file path in prose and as a field, so the doctor printed it twice. Three write paths accepted model-authored payloads and parsed them loosely, straight into the vault.

A ninth unit was written, reviewed, and then reverted rather than shipped, and that is worth stating plainly: a tie-break layer for the write router turned out to be unable to fire at all. Proving it byte-identical when its gate is absent also proved its reach — only a rival claiming a captured span in full can enter its margin, and the three pattern families this router chooses between cannot do that, because a complete match of one carries a character the others do not admit. The routing ambiguity the task describes is real, but it lives on a different write surface; this router chooses between structurally disjoint families and has nothing to disambiguate. Shipping the mechanism would have shipped a decision layer that never decides and a telemetry record that is never written.

Reconnaissance against the code invalidated a premise in four of the nine scoped tasks before a line was written. Two more were falsified during the release itself: one recorded during scoping and corrected in the design document rather than quietly dropped, and the one above. Independent review of the finished branch — reviewers given the diff and the claims, with no knowledge of how the work was done — found fourteen further defects that the implementing agents' own passing tests had not, most of them in error paths and option branches those tests did not model. Every one is fixed in this release and named below. The kernel still calls no model, no natural-language word list was added in any language, and every new flag and configuration key is byte-identical when absent.

### Added

- Git-aware ingest source discovery. The batch planner honours the repository's own root and nested `.gitignore`, `.git/info/exclude`, and submodule boundaries, through a discovery module extracted from the hygiene scan so both paths compose one implementation and the submodule predicate - the only net-new piece - exists once rather than in two twins that drift. The layers stack repository-declared first and the operator `--exclude` layer last, re-stacked at every directory so it wins at any depth; a submodule boundary is a boundary rather than an ignore rule, and `--exclude` cannot re-include it. A malformed pattern in a repository's own ignore file is returned as a structured warning and surfaced on both operator surfaces - carrying source, line, pattern and reason as fields - rather than printed from the kernel or failing the plan; a malformed operator `--exclude` still throws. Under `--subpath` the directories between the source and the walk root were never visited, so their `.gitignore` files were silently skipped; they are now layered. A tree with no ignore files produces the same plan identifier it produced before.
- A routing hint on an unroutable capture. A feedback signal recorded with no scope now names `scope` as the missing routing signal and lists the scope slugs the vault actually contains, ranked by document frequency over confirmed preferences and the unconsumed inbox - so recency is structural rather than clocked, and candidates come from frontmatter rather than from a word list. A vault with no scope corpus returns nothing rather than empty prose. The forward pointer resolves a registered diagnostic code through the advisory rail, resolved at module scope so registry drift fails at import instead of silently printing nothing, and the hint is recorded under its own log event kind - a hint nobody counts cannot show whether unrouted captures fell. The write always lands; no configuration key was added.
- Query-side temporal intent. A query that declares a window - from ISO tokens or `since:` / `until:` field tokens, never a natural-language phrase in any language - biases ranking toward that window through a capped additive layer over the existing proximity function, and damps the freshness prior when the window is historical. Field values accept only language-neutral forms, checked by shape rather than by a second parser, so the word grammar the explicit parameter has kept for releases is not exposed to free query text. A bare token that merely looks like a date but does not resolve declares nothing and stays in the query as content, rather than failing the search. Freshness and temporal intent stay separate entries in the reasons and in the score breakdown, so a ranking change is attributable to one of them. A tie-break rung sits above the freshness rungs and above the keyword-score rung, because that score is already a component of the total and re-applying it on an exact tie double-counts a signal the temporal contribution cannot outweigh once clamped. Detection runs on the raw query before expansion or any other lexical restructuring, and the directive is stripped from the text lanes before matching: the full-text builder joins tokens by implicit AND, so a directive left in the query is a required term no document carries and the search returns nothing. A query that declares a window and no terms is an input error naming what is missing.
- Weak recall becomes tracked work. The recall-adequacy verdict, already computed deterministically from numeric scores at two sites, is stamped onto the demand record under the bucket key the demand log already derives from document frequency, with its existing redaction and caps. The knowledge-gap loop gains that as a second signal source beside its structural one; the two are never summed, the task key is namespaced by source, and the agenda labels which is which, so a coarse telemetry bucket and a fine term bucket stay distinguishable. Promotion is capped per run and a skip costs no budget, so an established backlog cannot starve a new gap.
- `fact` and `skill` marker kinds on the existing inline grammar, so plain text can seed all three destinations rather than only a taste signal. No second grammar and no new entry point: fence-awareness, the consumed sentinel and the rewrite machinery apply unchanged. A fact lands as an unconfirmed preference through the existing derive path with premise validation still fail-closed; a skill lands in the pending proposal store, never in accepted. A declared skill deliberately bypasses the mining verifier rather than satisfying it - that gate demands three distinct pieces of structural evidence and a confidence floor, which a single declaration can never meet, so running it would reject every marker and ship the feature inert. For the same reason the declared frontmatter writes no confidence key: an absent claim already reads as zero self-reported support, and writing a number would fabricate a statistic.
- Response-shape validation on the three model-authored write paths. Each ingress checks its payload against a frozen descriptor before any field is read; a violation raises a named error carrying code, path and message as fields, nothing is coerced, and a batch whose third item is malformed writes none of the first two. Validation is unconditional - a validator that can be switched off is one the write path cannot rely on, and a strict path shipped inert behind a permissive default would be exactly the silent no-op this project forbids. The validator is not new: the JSON-subset checker in the Model Context Protocol output contract was promoted to core and both sides consume one definition, replacing the hand-rolled coercion checks that had been duplicated between each command-line verb and its tool.
- Durable work identity above the timing crutch. A session may declare a work id and a lane, and a rung above the fifteen-minute window links two entries sharing a work id on identity alone - no freshness bound, no working-directory or branch predicate - so resumed work re-attaches after a model, account, branch or worktree switch. Identity is declared, never derived, through three rungs where the first to declare either field supplies both; deriving it from remote and merge-base into a union registry was rejected because that places mutable, effectively irreversible state inside a peer-replicated vault with no arbiter. A declared lane mismatch excludes a candidate at both rungs rather than only at the identity rung - a separator one rung ignores is not a separator - and the refusal is a named outcome reported ahead of predicates a candidate merely fell short of. The abstention layer from 1.39.0 is consumed rather than re-implemented.

### Changed

- A parse-error finding names its file once. The path moved out of the message and into a field on a typed error that composes its message from a bare detail plus that path, so every caller surfacing the message directly receives the same string it received before and none needed editing. Both branches were duplicating, not one: the status-folder-mismatch error was documented as already path-free and was not - its constructor appended the path to its own message. That error carries its location in the middle of its historical sentence rather than at the end, so it composes its own message through a single declared seam instead of having its layout rearranged.
- Four optional-field helpers in the preference parser threw with no path at all, so a malformed list-valued field named no file on any surface. They now carry the location they never had.
- Repeated keys inside a marker fence silently last-won, and one kind carried a bespoke duplicate guard to compensate. Both marker shapes now funnel through one collector with per-field arity, so a repeated single-valued field is refused as ambiguous for every kind and a repeated multi-valued field keeps all its values.
- A malformed value in an explicit `since:` / `until:` search field is an input error rather than being matched as literal text, which is how the equivalent parameter already behaved. A bare date-shaped token in ordinary query text is not a declaration and is left alone.
- The ranking boost caps were unnamed literals with no single place to compare them, so the new temporal cap had nothing to be proportioned against. They are extracted as named constants and the proportion is asserted against those rather than re-hardcoded.

### Fixed

- A `--src-subpath` walked past exclusions and boundaries the unscoped walk honoured. Pruning only ran on directories reached as a directory entry, so a subpath starting below an ignored directory re-entered it, and one starting below a nested repository crossed into it - the same tree answering differently depending on how it was addressed, which also forked its plan identifier. The ignored case is refused and reported with the override that opens it; the boundary case is refused outright, because continuing would attribute another repository's files to this plan and an empty plan would be indistinguishable from nothing to do.
- An unreadable repository ignore file was swallowed by a catch that returned without touching the warnings array in its own signature, so a file the repository declared excluded was queued for ingest with no signal on any surface. Only a genuine absence stays quiet now; every other failure names its reason, and only the error code is reported so no host path reaches a payload. The same code was inherited verbatim by the hygiene scan and is fixed in both. A symlinked ignore file is refused rather than followed, and the read is size-capped.
- A declared search window was silently voided under query expansion, and the search returned nothing. Detection read the text after expansion, whose tokenizer destroys the field grammar, while stripping still ran on the original - so the window vanished and its fragments became required terms. The expansion path had no test at all, which is why it shipped.
- A scan preview hid every routed marker. The whole routed branch sat behind the write guard and the counters only counted the older kind, so `--dry-run` reported zero pending work while writes were queued and the next real run performed them. Preview and write now share one planner, and a test runs both over the same fixtures and compares refusals so they cannot drift apart on what is acceptable.
- A skill re-declared under a name a human had already rejected or accepted vanished: nothing counted, nothing reported, and the marker was consumed with its pointer aimed at a file whose body differed from the one just declared. The outcome is named and reported, and the marker is not consumed - the queue stored nothing, so the marker is the operator's only copy of that body.
- A repeated single-valued optional field in a marker was silently discarded while a required one was correctly refused, so a signal declaring a scope twice was written unscoped and unroutable. Both are refused as ambiguous now; the inline shape had the same drop before this release and is fixed too.
- An inherited work identity never expired, so every later session in a directory chained onto the previous one and lineage depth grew without bound. The bound applies to the inherited rung only: an explicit declaration by something live still re-attaches after a month, and the marker directory is capped in the same idiom the lineage ledger already uses.
- The response-shape descriptor for research reports accepted blank strings the parser it replaced had rejected, writing a report whose citations rendered as empty links. Descriptors can now require a non-blank string, with the replaced parser's own wording.
- The gap-task directory had no bound: its topic space used to be a fixed set of telemetry codes and the new source makes it arbitrary normalised query text. Closed tasks past a retention window are pruned and minting past a cap is refused, both reported.
- Two different opt-ins gated one feature - a configuration flag on one surface and a per-call argument on the other, so the demand log wrote when it should not and stayed silent when it should not. One key now gates both.
- The rolling-cap test in the demand log took five seconds against a five-second limit and would have flaked under parallel load. Its cost was append count rather than bytes, so it reaches the same byte-budget branch with 890 appends instead of 20,000 - and it now asserts that compaction actually ran, which it never did.

## [1.40.0] - 2026-07-27

A no-dead-ends wave: eleven units that make the system say what to do next, do it where the fix is provably mechanical, and refuse by name where it is not. Reconnaissance against the code invalidated at least one premise in eight of the ten scoped sources before a line was written, so most of this release is not new machinery but delivery of machinery that already existed and did not reach the caller. A registry of structural next commands had twenty entries and two consumers; the notice channel carried its command inside an English sentence; three repair pipelines each decided privately what they could fix; a repair planner shared a vocabulary with the detector while re-scanning the vault independently; the dream journal marked phases complete before their work began; the command manifest omitted a third of the real verb surface that shell completions are generated from; and two advisories stated things that were not true, one of them contradicting the same screen it printed on. The kernel still calls no LLM, and every new flag, argument, and configuration key is byte-identical when absent.

### Added

- One advisory rail for every forward pointer the command-line interface prints. It takes a diagnostic code, never a sentence, so prose cannot enter the surface, and it owns the question of whether advisory output is legal on the current stream - the twelve commands that render their own JSON receive nothing on stdout, while the envelope-wrapped family is safe because the envelope buffers. A suppressed emission still resolves, so a machine caller carries the command as a field instead of losing it. A ratchet fails the build when a forward pointer is hand-written outside the rail; four sites remain sanctioned, each with a written reason and a pinned count so a new pointer cannot hide behind a neighbour's justification.
- `o2b brain dream run --step <scan|heal-enrich>` and a repeatable `--gate <name>=<true|false>`, with `step` and `gates` on the `brain_dream` tool. The override applies to one run and never touches `_brain.yaml`. Any other step, including all five phase labels, is refused with an error that names the coupling literally: two planning steps mutate each other's accumulators specifically so a preference is never both refreshed and retired, and running the refresh half alone with writes enabled would write a preference the same pass had decided to retire.
- `o2b brain skill-proposals recover [--discard-unreadable]` and a `recover` operation on `brain_skill_proposals`. The accept transaction added in this release could strand a vault two ways, and both now name their exit: an unparseable journal marker blocked every future accept with no operator-facing recovery path, and a crash left an accept lock that no command mentioned. The lock is not broken automatically - a live writer cannot be told from a dead one - so the refusal names the exact file and the command that inspects it.
- A skill contract on acceptance: prerequisites, rollback, side effects, and verification are written and read back by the procedural-memory reader, so they are a contract rather than decoration. A proposal's self-reported evidence is now resolved against the independently recorded procedural outcome ledger, joined by a key derived from the proposal's own slug that the accept path already causes to exist. Three states stay distinguishable and are never conflated: recorded successes, recorded failures, and no recorded outcome at all.
- `--dry-run` on `o2b brain pending apply` - the one repair path that had no preview. It runs every check the apply runs and stops before the two disk calls, and a conflict an apply would reject is now reported by the preview rather than surfacing only at write time.
- Ingest dedup counts as a queryable signal: a new `ingest_dedup` continuity record with a `brain_analytics` view over it. The counters existed and were reported one call deep, then discarded, so a re-ingest spike could not be seen over time. The record is dimensioned by surface and source, and a run that deduped nothing writes nothing rather than a zero row that would make a real zero indistinguishable from an absent measurement. The semantic layers are reported as what they are - nominations, never drops.
- A `project` scope on session digests, reusing the existing scope axis, its slug rule, and the `project_scope` argument spelling already carried by search rather than introducing a parallel dimension.
- An applier capability table: per finding code, whether repair is mechanical - naming the fixer - or refused, naming the reason. The three repair pipelines consume it instead of each deciding privately, and a test re-derives the population from the fixer registry and the three planners so the table and the code cannot disagree in either direction.
- Machine-readable broken-link findings: the field name, the target, and the source list are carried as fields, where a consumer previously had to match the message text.
- Expected-output blocks in every per-runtime install document, bound to the adapters by a conformance test that installs each target into an injected environment, calls its verify, renders through the command-line interface's own renderer, and names the document in the failure.
- Ratchets that keep the release honest after it ships: a terminal-state census enumerated from the dispatchers, a manifest-completeness ratchet, a configuration-key ratchet over the generated template, a doctor-exit census, and the forward-pointer ratchet above. Each fails until a new surface is classified, and none can be satisfied by an empty reason.

### Changed

- Every issue `o2b brain doctor` reports now carries its structural next command on the human surface and as a field under `--json` and on `brain_doctor`. Where no single command resolves a class, the doctor says so instead of falling silent: an additive `no_exit` object gives the reason once per code, so an operator can tell "there is no command for this" from "the system forgot". All forty-one doctor codes are classified - three gained a genuine exit, twenty-four are excluded with written reasons - and four of them were assembled by string concatenation and existed nowhere in the source as readable literals.
- The command manifest gained sixty-four entries the dispatchers already handled - two top-level commands, seven search subcommands, and fifty-five brain subcommands - and `o2b help` is now rendered from that manifest instead of a parallel hand-written constant. Shell completions and `o2b help --json` widen accordingly. The two help surfaces previously described materially different command sets.
- The dream workrun journal now marks a phase complete where its durable writes actually land. Three markers were previously appended before a single preference was written and four after every mutation had finished, so a crash mid-write left a journal claiming work that had not started. Emission order is now execution order rather than the reporting order, which moves one marker to the end; any consumer outside this repository treating marker position as monotonic progress should re-read it. A fifth safeguard checkpoint was added before finalize, covering the log writes, the ledger write, snapshot pruning, and digest regeneration that ran unguarded.
- The repair planner derives its plan from the findings the doctor already produced, instead of re-scanning the vault independently and sharing only a vocabulary of codes with it. No hidden re-scan was kept: one finding population turned out to be a strict superset of the other, and that is recorded rather than papered over.
- `_brain.yaml` is generated from the resolver's own default tables. Sixteen of twenty-three top-level keys resolved silently from defaults because a hand-maintained template and a set of resolver tables had drifted apart with nothing keeping them in sync. Newly surfaced keys are emitted commented, so the resolver stays authoritative and a later default change is not silently frozen into every vault created today.
- Repair-path guards were unified: assert at the applier's entry point, before any other work, and only when the call will write. One of the three pipelines refused dry runs and no longer does.
- `o2b brain init` creates the two directories it declared but never made, and the runtime notice channel carries its command as a field rather than inside a sentence a consumer had to match.
- The stale search-index notice was corrected. It claimed recall returns nothing without an index; the read path builds the index and retries, so recall returns late rather than empty.
- `--` is honoured when detecting a JSON request, so a value after the terminator is a positional rather than a flag.

### Fixed

- A read-only verb could fail with a write-path error: the consolidation lint asserted vault identity before its apply branch, so `o2b brain actions` could refuse a pure read on a vault root whose marker differed from the pinned one.
- Two advisories stated things that were not true. The empty-Brain pointer fired after every initialization, including re-runs where nothing was created, and `o2b init` contradicted itself two lines apart on one screen by naming a command its own checklist then asked for again. Each now fires only where its claim holds, and a table-driven test asserts that for every occurrence of every registered terminal state.
- A repair item with no fixer was silently skipped and reported as success; it now raises a named error.
- The profile registry silently overwrote an existing entry, performed no directory check at create time although switching did, and did a lock-free read-modify-write so two concurrent creates lost an update. All three are closed, and both concurrent creates now land.
- A configuration key that gates a destructive path reached block merging but was dropped when the typed configuration was assembled, so it could not be switched on from `_brain.yaml` at all, and no warning said so.
- Preferences created with forced confirmation wrote an absent confidence value that read back as null, which ranks below zero on the two surfaces that rank on the number. They now write an explicit zero, matching what the dream pass already does for new preferences, and the doctor check that detects this population names recording real evidence as the exit. Reported as [#149](https://github.com/itechmeat/open-second-brain/issues/149). Both fixes proposed in that report were declined with measured reasons: one writes an apply-evidence row asserting a rule was applied to an artifact when it was not, which is defined in this codebase as the canonical durable signal that a preference was exercised against real work; the other cannot see the timestamp it needs, breaks two assertions, and drifts about one ten-thousandth per hour against a one-millionth no-op threshold, so every dream pass would rewrite every zero-evidence preference forever. Measured: the reported feedback loop has no code path, because the injected context applies no confidence cutoff and orders by band rather than by value, so both proposed values would have left the preference in the same position.
- An emission record did not carry the code it was made for, while the batch form's own documentation promised unregistered codes come back "by name". The test covering it asserted only the outcome, so the gap was invisible from both sides.

## [1.39.0] - 2026-07-26

A context-integrity wave: ten units that convert a silent failure in the read and delivery path into an explicit, named one. Owner-scoped isolation was enforced on one surface out of thirteen; a context pack carried no record of the vault state it was built from; session continuation resolved ambiguity by picking the most recent candidate; the lineage ledger lost history on compaction and had no writer lock; an embedding store written against a different model, dimension, or sqlite-vec ABI returned zero or garbage neighbours with no check on the read path; a frontmatter line the parser did not understand was dropped without a trace; nothing counted broken links vault-wide, nothing measured the SessionStart injection, nothing joined retrieval receipts across a session, and a mis-resolved vault root was materialized by the first write while the doctor reported it clean. Two shared seams carry the wave: a closed-vocabulary degradation channel and a stamp-and-compare module with three-mode gates. The kernel still calls no LLM, and every new surface is byte-identical when its flag, argument, or config key is absent.

### Added

- Typed degradation channel: a closed `DegradationCode` union with a notice record (`code`, `site`, `path?`, `detail`) and a one-line English renderer, carried by the report types that already exist. The union is exhaustive-switchable, so a consumer with a `never`-typed default arm fails to compile on an unhandled member.
- Stamp-and-compare seam and the `integrity:` block in `_brain.yaml`: `owner_scope_delivery` (default `off`), `embedding_abi` (default `warn`), and `pack_validity_seconds` (default 900). An unrecognised mode or a non-positive validity raises `BrainConfigError` naming the dotted field; an absent block resolves to the documented defaults. Under `off` the gate does not compare at all and returns no evidence.
- Owner-scope isolation on every content-returning surface, not only search. Preference-backed surfaces (context pack, pre-compress pack, morning brief, digest, `active.md`) route through one shared collector and are gated by `integrity.owner_scope_delivery`; search-backed surfaces filter whenever the caller passes a scope. An optional `agent_scope` argument joins eleven MCP tools and an `--agent-scope` flag joins `o2b search query`, `o2b search expand`, and `o2b brain sgrep`. A table-driven test partitions all 108 tools into scoped, deliberately-unscoped-with-a-reason, and non-content, and fails until a new tool is classified. `brain_search_expand` on another owner's chunk now raises the same `chunk not found` error as a genuinely missing chunk, closing an enumeration surface over sequential integer ids.
- Context pack provenance: a stamp pairing the index corpus generation with a digest over path, mtime, and size for the directories the pack walks, plus an expiry from `integrity.pack_validity_seconds`. The anticipatory cache stores both and refuses a drifted or expired entry as a miss naming the reason; its schema version bumped so pre-existing cache files read as misses.
- Embedding ABI detection: the sqlite-vec version joins the model and dimension already stamped in `index_state`, and the three are compared on the read-mode open, where previously nothing checked. `o2b search check` reports drifted fields with a copy-pasteable fix, ungated. Nothing is ever rebuilt automatically - the existing self-heal path rebuilds keyword data only and would leave stale vectors in place.
- Frontmatter diagnostics: a note whose frontmatter uses grammar the line scanner does not support still indexes, and now leaves a trace. `o2b search index` reports `frontmatter_notices`, `o2b brain doctor` reports `uncertain` entries, each naming the file, the line number, and the offending text. The index run reports files it re-read on that run; the doctor sweeps the whole Brain tree and is the standing report.
- Vault-wide broken-link ratchet: a deterministic count of link rows the read-time resolution ladder cannot resolve, sharing that ladder's SQL from one constant. `bun run link-ratchet:check` is wired into both CI workflows and exits 1 on a rise, naming `bun run link-ratchet` as the fix. The committed ceiling in `link-ratchet.json` measures the `templates/brain-starter` repository template, not any operator's vault, and encodes the link rot present there today.
- SessionStart injection meter: one receipt per injection with per-source byte and token attribution for the runtime-notice block, the budgeted `active.md` body, and the `lessons.md` digest, measured after the fail-open loader returns, together with the loader source, the hook event, and the char budget each sub-body was charged against. The meter runs after the context is emitted and inside its own try/catch.
- Session-scoped retrieval receipt summary: a third `summary` operation on `brain_context_receipts` folding existing records into receipt count, item total, distinct items, and per-item injection frequency and token cost. Receipt emission is opt-in, so a window with no receipts returns `{recorded: false}` carrying no counters at all - the return type is a discriminated union, so "nobody enabled receipts" can never be read as "retrieval injected nothing".
- Fail-closed session continuation: the crutch resolver returns a discriminated outcome - linked, or an abstention naming `no-candidate`, `self-known`, `no-workspace`, `ambiguous`, `stale`, or `evidence-missing` - instead of a bare `null`. Two concurrent sessions in one directory inside the freshness window both abstain where one was previously stitched to the other. Repo, branch, and commit join `cwd` as required-match predicates, captured through bounded `git` invocations; a remote carrying credentials canonicalizes to the same identity as the same repository cloned without them, and the credential never reaches a recorded field.
- Lineage ledger integrity: sequence numbers and a hash chain, appends under a writer lock, and compaction that retains recent lines verbatim instead of folding one summary per session - so `firstSeenMs`, per-session event history, and any field a later writer adds all survive. A dropped observation is recorded as a named gap in `Brain/.state/session-lineage-gaps.jsonl` and excluded from continuation matching, so a lost write can never become a false stitch. A tampered, truncated, or unreadable ledger is reported by verification and still resolves; the read path stays fail-soft.
- Vault identity guard: `o2b brain init` stamps an idempotent `Brain/vault-id.json` carrying an opaque id and no absolute path. Brain writers compare the root's marker against the identity the process pinned on its first write - a present-and-different marker refuses with a named error ahead of the first byte, an absent one warns through the degradation channel and proceeds. Of 106 write-capable modules, 97 carry the guard and 9 are excluded with a stated reason, measured by a census test that names any unguarded module.

### Changed

- `o2b brain doctor` reports a `Brain`-less root as a named `brain-root-absent` warning with an `uncertain` entry recording that every check below was skipped, instead of certifying it clean. `o2b brain doctor --strict` on such a root now exits 2 where it previously exited 0.
- `vault profile switch` refuses a profile whose vault is not an existing directory, so activation fails at switch time instead of in a later process. A profile can no longer be switched to before its directory exists.
- The lineage ledger's compaction retention changed from the most recent 256 sessions to the most recent 256 lines, so a chatty session ages other sessions out sooner. The writer lock has no retry: concurrent sessions on one vault can drop lifecycle observations, each named in the gap sidecar rather than lost.
- Twelve MCP tools gained input arguments - `agent_scope` on eleven, and `since`, `until`, `max_receipts` plus a widened `operation` on `brain_context_receipts`. No tool was added or removed (the count stays 108) and no output schema changed.
- `brain_hygiene` gains an additive top-level `link_integrity` key. It refuses to produce a number unless the index's last run was a forced full pass, reporting `measured: false` with reason `partial-resolution` rather than a misleading zero.
- Retirement keeps ownership. `BrainRetired` carries an `owner` field and every retired-page surface applies the same visibility predicate as an active preference - retiring a memory withdraws it from the active set, it does not publish it.
- Five `brain_brief` views that cannot honour a scope (`daily`, `weekly`, `monthly`, `operator`, `today`) now reject an explicit `agent_scope` with `INVALID_PARAMS` naming the view, rather than accepting and ignoring it. The server-identity fallback is unaffected.

### Fixed

- Ownership resolution fails closed. A page whose frontmatter cannot be read, and a page whose `owner:` is present but not a plain string (a block-style or inline YAML list, or a nested mapping - the shape Obsidian Properties writes), are withheld from a scoped caller instead of being treated as shared.
- A per-request scope no longer drives a shared write: `brain_context` filters what it returns at render time and leaves `Brain/active.md` complete, and the anticipatory cache path keys on the enforced scope, so a warm cache written by the SessionStart hook is found by the MCP reader under the default gate.
- The vault write guard runs ahead of the first byte everywhere. The secrets store, the schema mutator, page archiving, hygiene apply, the pinned-context batch path, tension transitions, gap auto-close, and snapshot pruning previously wrote or deleted before their assertion ran.
- An unreadable `_brain.yaml` no longer silently disables an isolation gate: an absent config resolves to defaults, while a present-but-unreadable one falls back to the strictest mode and surfaces a `brain_config_unreadable` notice.
- The link-ratchet gate no longer passes on a subject that measures nothing: an empty or fully-excluded subject is an explicit unmeasurable state rather than a ceiling of zero.
- Four source files carried literal NUL bytes that made git treat them as binary and made `grep` skip them silently.
...
[1.53.1]: https://github.com/itechmeat/open-second-brain/compare/v1.53.0...v1.53.1
...
[1.47.0]: https://github.com/itechmeat/open-second-brain/compare/v1.46.0...v1.47.0
[1.46.0]: https://github.com/itechmeat/open-second-brain/compare/v1.45.1...v1.46.0
...
[1.39.0]: https://github.com/itechmeat/open-second-brain/compare/v1.38.0...v1.39.0
