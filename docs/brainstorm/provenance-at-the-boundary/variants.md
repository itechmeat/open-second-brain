# Provenance at the boundary - brainstorm audit trail

**Consultant:** Claude Code (`claude -p`), primary. Exit code 0, three parseable
variants plus a recommendation, so the Codex fallback was not run.

**Prompt:** [`cli-output/prompt.md`](cli-output/prompt.md). The task put to the
consultant was not any single kanban item but the wave-level architectural
question: how ten units that all sit on the vault's write boundary should be
factored so they share one substrate instead of ten bolt-ons.
...
Variant 3 is adopted, in agreement with the consultant, with two amendments
recorded here because they close the two cons it names against itself.

**Amendment 1 - one digest primitive, named before the fan-out.** The
consultant's own second con is that both substrates want a content digest and
an origin notion, and that without deliberately sharing one they reproduce the
duplication the wave exists to avoid. So the digest helper is not left to
whichever unit needs it first: it is part of the seed commit, sourced from
whatever `src/core/integrity/` already provides, and both substrates consume
it. No unit may introduce a second hashing path.

**Amendment 2 - the capability rail must rule on its boundary with the existing
error vocabularies before any unit uses it.** The third con is that a new
capability state and the existing `SearchError` codes will otherwise both
describe an unconfigured provider. The ruling is made in the design document
rather than deferred: the capability state answers *what the operator has
configured*, and `SearchError` answers *what a call attempted and could not
do*. A provider that is absent by configuration is the former; a provider that
is present and refused the request is the latter. Nothing may report both.

The two rejected variants were rejected for the reasons the consultant gives.
Variant 1 is worth one further note: this repository has already shipped a
release whose central finding was that a ratchet enforcing a convention passed
vacuously over an empty set. A write fence enforced by conformance test rather
than by structure is the same class of artefact, and this project has direct
evidence of how that ends.
