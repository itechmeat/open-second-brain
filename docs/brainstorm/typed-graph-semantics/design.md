...
- **Vocabulary is data-driven and open.** A default relation/visibility set lives in the module; unknown relations are preserved and flagged by `brain_doctor`, never silently dropped or hard-rejected at ingest. No closed enum baked into many call sites.
