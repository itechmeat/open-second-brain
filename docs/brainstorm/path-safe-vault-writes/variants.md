# Path-safe vault writes variants

## Consultant output

Primary consultant command failed before model execution because the shell argument exceeded the OS argument length limit. The fallback consultant used the same model after reducing prompt size while preserving the required context.

### Primary output

```text
/usr/bin/bash: line 3: /root/.local/bin/claude: Argument list too long
```
...
Final choice: Variant 3.

I agree with the consultant recommendation. Open Second Brain already has `ensureInsideVault`, guarded Brain path constructors, and `brain_create_note` containment, so the best patch-scope work is to audit the remaining writer call sites, route only confirmed gaps through existing guards, and add regression tests that keep the containment invariant from regressing. Variant 1 risks a default-off writer guard that looks safer than it is, and Variant 2 risks adding a competing sanitizer before proving where an untrusted identifier gap exists.
