...
- **G counts SQL-dangling links and says so.** A link can be `target_document_id IS NULL` yet resolvable by the read-time ladder. The counter uses the SQL definition because it is the one that is stable, index-backed, and reproducible; the ceiling file records the definition token alongside the count so a later change of definition cannot be mistaken for link rot. The count is taken after a full resolution pass, never from a partial incremental state.
