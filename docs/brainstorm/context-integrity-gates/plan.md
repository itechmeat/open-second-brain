...
- **What**: a deterministic vault-wide dangling-link count using the SQL definition (`target_document_id IS NULL AND target_path IS NOT NULL`), taken only after a full resolution pass. A committed ceiling file at the repository root carrying `schema_version`, the count, and the definition token. The script mirrors `scripts/sync-version.ts` exactly: one code path, `--check` disabling writes, exit 1 on a rise, a named fix command on failure, and `link-ratchet` / `link-ratchet:check` npm scripts wired into both workflows. `brain_hygiene` gains an additive top-level key.
