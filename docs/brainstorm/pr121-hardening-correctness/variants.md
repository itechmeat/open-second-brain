# PR #121 hardening + correctness — consultant variants

Verbatim output from the primary consultant (Claude, model `claude-opus-4-8`), 3 variants + recommendation. Primary succeeded (exit 0); no fallback run.

Source: `docs/brainstorm/pr121-hardening-correctness/cli-output/claude.md`
...
**Accepted Variant 1 (no override).** It satisfies all four project-specific constraints that the other variants strain against:

1. **Card-atomic driving model** — the orchestrator drives cards one at a time on a shared branch; Variant 1 is the only shaping whose commit clusters map 1:1 to cards, so per-card TDD attribution and merge-conflict reasoning stay clean. (Variant 3 ignores card boundaries; Variant 2 splits a card across releases.)
2. **Frozen-contract reviewability** — the schema bump (`LATEST_SCHEMA_VERSION`, `basename` column) is isolated as its own auditable commit, not fused with unrelated correctness fixes (Variant 3's failure) or scattered across releases (Variant 2).
3. **Safety/contract decoupling** — the `INDEX_UNREADABLE` self-heal rides with its natural home (t_f27d80fe) and stays decoupled from the reindex-forcing schema bump, so a rollback of one cannot drag the other.
4. **Single-reindex economy** — C.2's unavoidable reindex absorbs the Han-bigram tokenizer's re-tokenization at no extra cost; users pay one reindex for the whole release.

The one real cost (the P4 data-loss fix is gated behind the C.2 reindex for users who upgrade to 1.24.0) is acceptable for a minor release and is itself mitigated by landing t_f27d80fe first — the reindex path is race-safe before the schema bump forces load onto it.
