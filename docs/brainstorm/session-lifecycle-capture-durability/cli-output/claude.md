...
The release scope explicitly catalogues the substrates each task needs — the `[KIND, sessionId, contentHash]` dedupe key, resume-aware `resolveSessionLineage`, and the symmetric `extractPreCompactRecords` pass — so the lowest-risk path is three tight additive diffs that reuse them, fully honouring off-by-default/byte-identical, fail-soft, and KISS. It avoids both Variant 3's risky core-store replacement and Variant 1's premature TS consolidation, while leaving Variant 1's `post-compact-audit.ts` placement available as a later refactor if the parallel paths prove worth unifying.
