# Variants considered - wiring what exists

Consultant: Claude Code, run once against `cli-output/prompt.md`. The primary
returned three parseable variants, so the Codex fallback was not invoked.

The consultant's output is reproduced verbatim below, followed by the
orchestrator's decision and the one place it departs from the recommendation.
...
**Accepted: Variant 3**, with one narrowing.

The consultant's reasoning holds against the project context. Variant 1 would fix
nine instances of a defect whose cause is that no mechanism requires the call, which
leaves the argument of the release unaddressed by the release itself. Variant 2 buys
the guarantee structurally but pays in the one currency this project protects most
carefully: byte-identical behaviour when a feature is absent, measured rather than
asserted. Routing about twenty-five destructive operations and six export paths
through new shared code changes ordering, error shapes and output framing in places
a byte-diff will legitimately flag, and separating the intended differences from the
unintended ones would become the bulk of the work rather than the units themselves.
The consultant is also right that a dispatcher required to know every destructive
operation is exactly the module the acyclic-import census attracts cycles into.

### Where this departs from the recommendation

Variant 3 proposes three registries. Two are built.

The third - a registry of write paths that classify source trust - is not, because
there are exactly three such paths and a census over three items costs more
maintenance than it protects. The precedent in this repository for that size is
prose: `page-lint.ts:28-38` names its non-adopters in the module header rather than
in a table, and that header is what caught this round's A1 in the first place. A1
wires the third path and the header stays the record.

### What the consultant did not have

Two things were known to the orchestrator and not in the prompt, and neither changes
the choice:

- `BRAIN_SNAPSHOT_REASON.manual` is declared, documented, and producerless. The
  proof-of-recoverability path in B1 is the caller it was written for, so that unit
  invents no vocabulary member.
- The population rule for the destructive-site census can be syntactic rather than
  semantic. The consultant flagged "what counts as destructive?" as Variant 3's
  failure mode, and it would be, if the census had to answer it. It does not: the
  rule is which `fs` calls a module makes, which is the write-site census's rule with
  a different call set, and it is mechanically decidable.
