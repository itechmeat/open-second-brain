...
**Rationale**: The seam is the only position that satisfies both Task A and Task C for the internal writers the tasks explicitly list, and it is the position the existing censuses and the vault-identity guard (`vault-identity.ts`, "check before any byte") already occupy, so the architecture test can pin it. Extracting just the shard grammar keeps `log.ts` byte-stable for vaults that change nothing while still putting `resolveDeviceId()` behind one function, and the content-addressed image store gives Task D a real apply path without either bloating the log or depending on opt-in snapshots. Variant 2 is the right long-term shape but does not fit one wave under the bit-identical constraint, and Variant 3 repeats the bypassed-boundary failure the epic exists to fix.
