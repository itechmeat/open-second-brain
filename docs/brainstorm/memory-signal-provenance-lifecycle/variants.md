# Variants — memory-signal provenance and lifecycle integrity
...
because it preserves o2b's load-bearing invariants and resolves every concrete
constraint the 13 cards impose:

1. **Reuse only where a hard edge mandates it.** Two composition edges are real
   (A1→A3 the content-hash manifest; C1→C4 the idempotency-key ledger). Variant 3
   extracts exactly those two modules and validates each against a consumer
   landed immediately after — never guessing an API. Variant 1's speculative full
   substrate would couple the p4 anchors to p2 capstones through unvalidated
   shapes; Variant 2's refusal to extract would produce the dual-hash /
   dual-idempotency drift this release exists to eliminate.
2. **Byte-identity-when-off / absent-by-default.** Every new field is additive
   and optional; dry-run stays a per-surface parameter mirroring the existing
   `import.ts` `opts.dryRun` idiom (not a global wrapper), so each card keeps a
   locally-provable off-branch and preview stays faithful to real extraction.
3. **Shared-file collisions serialized by drive order.** The single topological
   order (A1 → C1 → A2 → D2 → D1 → E1 → C2 → C3 → C5 → C6 → A3 → C4 → D3)
   resolves all five collisions (`ingest.ts` for A1+A3; `import.ts` for A2+C2;
   `contradiction.ts` for D2+D3; `WriteSignalInput` for A2/C5/C1;
   `truth/{fold,conflicts}.ts` owned by D1 alone) and honors all three
   parent→child edges (A1→A3, C1→C4, D2→D3).

One amendment to the consultant's drive order: it listed 12 of 13 cards and
omitted E1 (the isolated Aider wrapper). E1 has no shared-file collision and no
dependency, so it slots cleanly into the p4-anchor group (driven sixth, after the
other three p4 anchors). The variant choice is unaffected.
