# CodeGraph and MCP operational readability variants
...
Chosen variant: Variant 2, Dedicated operational-readability reporting layer.

I agree with the consultant recommendation. It best matches the locked scope by improving operator and agent visibility through additive report surfaces, while keeping partner integration optional and read-only. It also prevents the release from overreaching into new graph schema edges or LLM community labeling, both of which would increase risk beyond the two triage cards.
