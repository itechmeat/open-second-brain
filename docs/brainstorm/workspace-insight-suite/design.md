...
New (CLI): `src/cli/brain/verbs/{project,source,links,profile,sgrep,trigger,synthesis,ideas}.ts` + registration in `verbs/index.ts`, `brain.ts`, `command-manifest.ts`; `--global` flag in `src/cli/search.ts`.
