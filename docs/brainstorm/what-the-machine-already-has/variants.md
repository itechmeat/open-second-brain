# What the machine already has - variant audit trail

## Consultant run

- **Primary consultant**: `claude` (Claude Code), one run, exit 0, well inside the
  five-minute budget. Three parseable variants and one recommendation returned, so the
  Codex fallback was not invoked.
- **Prompt**: `cli-output/prompt.md`
- **Raw output**: `cli-output/claude.md`

The prompt carried the nine tracker items as reconnaissance findings against the live
source rather than as their filed text, because verification had already falsified part
of three of them: a committed vault-level configuration does exist, the tool-profile
mechanism does exist, and the measured tool count is 110 rather than the 90 on the card.
Asking for variants against the filed text would have produced designs for a repository
that does not exist.
...
Variant 2 is adopted, with the consultant's own suggested graft and two refinements.

**Adopted as argued.** The nine items drifted for one reason, and the consultant named
it precisely: a fact was declared in one place and read in none, or read in two places
from two declarations. Variant 1 is more DRY on paper and unions two genuinely different
key spaces into one wide-optional row - which is the shape that produced the dead
`sessionPaths?()` seam this release exists to close, so adopting it would reproduce the
defect while claiming to fix it. Variant 3's owner-local obligation is right for adapters
and structurally cannot reach the state surfaces that have no owning adapter, which are
exactly the ones nobody tracks today.

**Graft taken.** `RUNTIME_FACTS` carries a probe specification, as the recommendation
suggests. Declared and measured stay separate columns; a declared ceiling is never
presented as a measurement, and a probe that could not run reports a named skip reason
rather than an absence.

**Refinement 1 - no fifty-five-site sweep.** The consultant flagged widening
`envOrConfig` across its call sites as the largest risk in its own recommended variant.
That risk is avoidable and the sweep buys nothing: a provenance-returning
`resolveWithOrigin` is added, `envOrConfig` delegates to it and returns `.value`, and no
existing call site changes. The surfaces that need provenance - install, hook generation,
the inventory - are few and named.

**Refinement 2 - three ceiling states, not two.** The consultant's rows imply a ceiling
that is either declared or absent. A host nobody has checked is a third thing, and
folding it into "unbounded" would be a silent default of exactly the kind this project
forbids - the same defect one layer up from the fail-open profile selection that made the
original card true. `TOOL_CEILING_KIND` therefore has `declared`, `unbounded` and
`unknown`, and `unknown` carries its reason into the capability report.

One thing the consultant got right that is worth recording because it changed the plan's
shape: its criticism that Variant 2 "still declares rather than measures" is correct and
is not fully answered. A wrong ceiling in `RUNTIME_FACTS` stays invisible until a user
reports dropped tools. What the plan can do, and does, is make the declaration falsifiable
in one direction - a test proves the selected profile fits under the declared ceiling, so
the pair can never silently disagree with each other, even though neither can prove
itself against the real host.
