# CJK search, schema mutation, lifecycle capture, and recovery - variants
...
- **Trade-offs**:
  ...
  - Con: 4 tasks x multiple sub-surfaces risks brushing the 10-theme ceiling with no consolidation lever.
- **Complexity**: large
- **Risk**: medium
...
Chosen: **Variant 2: Shared Atomic/Audit/Probe Spine**.

The consultant's recommended Variant 3 is technically safer, but it conflicts with the operator's explicit instruction that `t_327f1876`, `t_d6b5632b`, `t_9eaebcad`, and `t_8d8ec450` all ship in full in this PR. Variant 2 is the best fit for that constraint because it keeps the broad scope cohesive: schema mutation, lifecycle capture, and watchdog recovery all share one atomic/audit/probe substrate instead of growing three parallel write paths. The main guardrail is to keep the shared substrate small and boring: atomic file replacement, lock acquisition, JSONL audit append, and probe result types only.
