# Salience, lifecycle, and enrichment wave - variants audit trail

Consultant: Claude Code (claude -p), primary, one pass. Full verbatim output below, followed by the orchestrator decision.
...
Variant B accepted, with two amendments recorded in design.md: the salience gate ships deterministic-only (the optional LLM retriage lane the consultant kept adds a second envelope and a verdict cache for no benefit the deterministic scorer does not already deliver), and the transformer embedding unit is refused outright rather than shipped as a runtime-less seam, because a registered provider that cannot embed is a stub and stubs are forbidden in this project. Variant A was rejected because units 2 and 8 are envelopes by specification, so its savings are illusory - it ships the new shapes anyway while declining the consolidation. Variant C was rejected because refactoring the 51KB WAL-protected proposal machinery is the wrong risk for a single-PR wave.
