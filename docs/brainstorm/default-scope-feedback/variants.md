# Configurable default_scope for feedback signals variants
...
Chosen variant: Variant 1, Core write-boundary fallback with policy-validated default.

I agree with the consultant recommendation. A write-boundary fallback gives the strongest parity guarantee for this small config feature because both CLI and MCP already converge on the signal write path. To preserve deterministic core behavior, the implementation should pass the validated default into the writer through an additive option or helper rather than making the writer read vault config itself.
