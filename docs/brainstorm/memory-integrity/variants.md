# Memory Integrity Suite - brainstorm audit trail

Consultant: Claude CLI (`claude -p`), prompt in `cli-output/prompt.md`, raw output in `cli-output/claude.md`. Fallback consultant (Codex) was not needed - the primary returned three parseable variants on the first run.
...
**Rationale (verbatim)**: It captures the only genuine synergies - the two regex-over-turns features share one turn seam and a suppress-then-extract ordering, and a small canonicalization kernel lets extracted facts resolve to canonical entities - while leaving the orthogonal sharding work decoupled and independently shippable, exactly as its "decided design" expects. This avoids both Variant 1's duplicated turn-walking and normalization logic and Variant 3's over-engineered substrate that the tasks explicitly warn against, and it keeps each piece testable, fail-soft, and backward-compatible within a single bundled release.

## Orchestrator decision

Variant 2, accepted as recommended. Project context confirms both synergies the consultant identified: `captureSessionLifecycleEvent` and `importSession` are verifiably the only two ingestion seams (so one pipeline covers everything), and the entity task's own acceptance criteria require alias resolution during lookup AND search - the same normalization the fact router needs. Variant 3 was additionally ruled out by the live-verified reader inventory (five independent directory scanners would all sit on top of a new substrate in one release - exactly the regression surface the constraints forbid). One refinement over the consultant's framing: the in-memory identity index is built per read rather than maintained as a cache file, because a persisted index file under Syncthing would reintroduce the conflict class the log-shard task removes.
