# SOLID/DRY refactor - brainstorm audit trail

Consultant: Claude Code (`claude -p`), single run, no fallback needed. Raw output: `cli-output/claude.md`. Prompt: `cli-output/prompt.md`.
...
Variant 2. "The task's stated goal is removing duplication, and Variant 1 mostly relocates it behind shims while Variant 3 smuggles a framework redesign into a pure-refactor PR with a frozen v1.0.0 surface. Variant 2 deletes the duplicates outright, follows the already-established per-domain split pattern in src/mcp/, and keeps each unit a self-contained conventional commit; its main risk (wikilink behavior drift) is contained by writing characterization tests for the divergent edge cases before unifying."

## Orchestrator decision

Adopt Variant 2 with two amendments borrowed from the other variants:

1. From Variant 1: `brain-tools.ts` stays as the aggregation seam exporting `BRAIN_TOOLS` - not as a compatibility shim but because `tools.ts` and six test files already treat it as the registration point; removing it would be churn without benefit.
2. From Variant 3: the tool-name parity test is adopted as a cheap mechanical guard for the frozen v1.0.0 MCP surface.

Shim-based variant 1 is rejected because the operator brief explicitly forbids crutch-style indirection that hides duplication instead of removing it. Variant 3 is rejected as scope creep inside a zero-behavior-change release.
