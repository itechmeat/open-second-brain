...
| Daily discipline report  | `o2b discipline {report\|install\|uninstall}` | — (CLI-only) | renders a deterministic Telegram MarkdownV2 block comparing per-agent brain-events vs runtime-agnostic activity (git/mtime/vault delta). `install` writes a Hermes cron entry (`--telegram-target` required; no private chat id is baked in). Status is binary: `alert` when taste events (`feedback`+`apply_evidence`) are zero while activity is non-zero. |
