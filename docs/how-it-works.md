...
- **Session import** - `o2b brain import-session <path>` reads a Claude Code / Codex CLI / Hermes session JSONL and extracts both `@osb` markers from message text and replays of `brain_feedback` tool-use calls. Useful when MCP was not available at recording time or the agent did not make the call live. Since v1.50.0 the same verb also sweeps the machine: `--status` reports per-runtime coverage over the session roots this build declares, `--discover` lists the logs that have never been imported, and `--discover --all` imports them through the same code path a named path takes. A content-digest ledger under `<vault>/.open-second-brain/` keeps a re-run from re-parsing a log whose bytes have not changed.
...
`delete-by-source`, `entity-prune`, `note-delete`, `note-revert`,
`payload-gc` for the eight destructive
...
Three rules are enforced at the commit boundary, not by convention: `create` intent never overwrites an existing file, `merge` appends a session-stamped delimited section without touching existing bytes, and a target outside the agent page lanes (`sources/`, `reports/`, `distillations/`, `notes/`, `decisions/panels/` under `Brain/`) is refused before a session even opens, then again at commit against where the bytes actually land. Every terminal transition - committed, failed, abandoned, approved - lands exactly one `write-session` audit event in the Brain log.
...
**Custody, not just redaction.** Secrets live as per-value AES-256-GCM ciphertext beside a 0600 keyfile in the vault-local state dir (`.open-second-brain/secrets/`), never returned by any surface. That directory carries a `.gitignore` so a vault that is a git repository never commits it; Syncthing does not read `.gitignore`, so a vault inside a Syncthing folder needs `/<path-to-vault>/.open-second-brain/secrets` in the folder's root `.stignore` - `o2b brain doctor` (`secrets-sync-exposed`) and `secret set` warn with the exact line when it is missing, and never edit `.stignore` themselves. An agent uses a credential by running an allowlisted command with the value injected into the subprocess env; captured output comes back with the value scrubbed as a known literal, the material enters only via the operator's stdin, and every operation lands a no-values audit record under `Brain/log/secret-custody/`. The threat model is stated honestly: this protects against context leakage, git exposure, and casual reads - sync exposure only once the syncer's own ignore list covers the directory - and not against root.
