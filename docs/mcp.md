...
- `open-second-brain` - the full surface (56 advertised tools plus 18 hidden deprecated aliases, including `brain_brief`, `brain_analytics`, `schema_inspect`, `brain_health`, `brain_mcp_landscape`, `brain_agent_query`, `brain_agent_diff`, `brain_recall_gate`, `brain_pinned_context`, `brain_pre_compress_pack`, `brain_audit`, `brain_sources`, and `brain_switch_vault`); subject to Claude Code's `MCPSearch` tool-search deferral when MCP definitions push the system prompt past 10% of the context window.
