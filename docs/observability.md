...
Every continuity record shares one envelope: `schema`, `id`, `kind`, `createdAt`, `sourceRefs`, `payload`, `private`, `redacted`, plus an optional `scope` present only when the appending caller declared one (see "Payload safety"). The table marks how each kind is gated - this is the always-on vs opt-in matrix, verified against the call sites named in the right column.
...
| `context_pack_outcome` | config key `context_pack_outcome_enabled` (default off) | `brain_context_pack_outcome` action `post` (`emitContextPackOutcome` in `src/core/brain/context-pack-outcome.ts`) - one compact per-sample outcome row for a carried context-pack quality-sample id: first-pass/repair/retry counters plus three STRICTLY SEPARATE token signals (`exact_prompt_token_savings`, `modeled_inference_avoidance`, `observed_provider_tokens`), never merged; also composes a matching `token_impact_outcome` calibration post; payload carries counters + an opaque `sample_id` only, never raw prompts or recalled text, and a caller-omitted field is not invented |
...
- The masking policy lives in the read-model (`src/core/brain/continuity/read-model.ts`) and governs only the readers that go through it: records flagged `private` are dropped by default and kept only on explicit request; masked text is never un-masked. That is FOUR modules - the `o2b brain continuity` verbs (trajectory export and listing), the bench harness, `brain_event_trace`, and skill-usage aggregation. Twenty other modules call `listContinuityRecords` on the store directly and see raw records: no `private` drop, no schema-version dispatch, no scope filter. The policy is read-model-wide, NOT store-wide. Both lists are asserted in `tests/core/brain/continuity/reader-census.test.ts`, which fails when a module joins or leaves either set; auditing the direct readers is tracked separately.
- A record may additionally carry a caller-DECLARED `scope` (`shared` today). Unlike `private`, it is never inferred from content - there is no `<shared>` marker and adding one would make visibility a second guess about text. Absent is the default and is byte-identical to a record written before the field existed, dedup id included. On the read side a scoped record is retained only by a read-model filter naming that exact scope, so a declaration can never widen what an existing reader already sees; requesting a scope is independent of `keepPrivate` and is not a way around the masking drop. The write vocabulary is closed (an unknown token is refused at the store boundary); the read side keeps an unrecognised token readable and excluded, so a scope from a newer version is neither leaked nor discarded.
