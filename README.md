...
Open Second Brain 1.26.0 closes five memory-integrity gaps in one coherent scope, each surface additive and byte-identical when off. Ingest now classifies sources by SHA-256 content hash and skips the extraction pass for unchanged files, and batch remember and import stamp each signal with the original turn event time instead of the import moment. Writes are idempotent under retry through client idempotency keys, where the same key and hash is a no-op and the same key with a different hash is an explicit error; extraction offers a faithful dry-run preview, and a bounded verbatim last-N-turns buffer survives compaction. Preferences and signals accept a caller-settable expiration date, a dry-run-by-default delete-by-source command reports blast radius before removing derived entries, and a whole-session checkpoint saves signals, learnings, and summary in one round-trip. On the truth side, a signed source-diversity grounding score quantifies how much independent support a slot carries, note-position contradiction detection pairs same-subject notes with opposite stances, and a declared-thesis register evaluates incoming notes against standing positions. A new Aider wrapper brackets a session end to end, mirroring the Hermes lifecycle. The kernel still calls no LLM.
