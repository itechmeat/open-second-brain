...
Open Second Brain 1.25.0 adds an optional economics and observability layer for the context pack, each surface off by default with byte-identical output when unconfigured. A value-per-token density ranking comparator breaks within-tier ties in the context pack after session focus and before recency, so a dense high-signal page can outrank a sparse recent one inside the same tier (tier stays the coarse gate). A durable token-impact ledger records the tokenizer-exact prompt-token delta every context pack saves (baseline minus packed), kept strictly separate from a modeled inference-avoidance estimate that outcomes can later calibrate, so a measured count is never averaged in with a model. An agent-operable context-pack outcome loop closes the quality feedback path: the agent posts first-pass, repair, and retry counters per carried sample, and three token signals stay separate rather than merged. Route-level MCP tool latency metrics show which endpoint blocks an agent turn by route rather than by aggregate benchmark, and an active-memory budget-pressure watermark surfaces proactive pressure before the pack is forced to trim. Every ledger is payload-safe (counts and opaque ids only, never prompts or recalled text), reads ignore the write gate, and the kernel still calls no LLM.
