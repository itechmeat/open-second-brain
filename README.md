...
Open Second Brain 1.39.0 is a context-integrity wave: ten units that turn a silent failure in the read and delivery path into an explicit, named one. Owner scoping now reaches every content-returning surface rather than search alone, and fails closed when a page's ownership cannot be read. Context packs carry a stamp of the vault state they were built from and an expiry, so a stale pack is refused instead of served. Session continuation abstains with a named reason when it cannot tell one candidate from another, and the lineage ledger is hash-chained, locked, and compacted without losing history. A frontmatter line the parser does not understand, an embedding store written against a different model or sqlite-vec ABI, a vault root that is not the one you meant, and a broken-link count that only ever rises are each reported now instead of passing quietly. Two shared seams carry the wave - a closed-vocabulary degradation channel and a stamp-and-compare module whose gates the operator sets to `off`, `warn`, or `fail`. Every new surface is byte-identical when its flag, argument, or config key is absent.

Previous release, 1.38.0, added baselining for semantic-health advisories: an optional `health.silence_before` watermark in `_brain.yaml`, managed by `o2b brain health-baseline set <date>|now`, `get`, and `clear`, filters findings whose underlying entries are entirely older than the watermark and always reports how many it hid. Details live in the [CHANGELOG](CHANGELOG.md).
