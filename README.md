...
Open Second Brain 1.32.0 is a memory write-path integrity and store safety release. Every durable write choke point now validates its input: embedding vectors are rejected before they can poison the semantic index, entity labels are stripped of Markdown decoration and structurally validated before graph persistence, and extracted facts pass a deterministic durability gate (with an optional operator write-approval queue) before landing in the brain. Destructive vault operations (`forget-source`, entity prune) take a snapshot first and report the recovery point. Embeddings quota exhaustion is now a distinct, actionable outcome with Retry-After-aware backoff, e5-family models get their required instruction prefixes, and MCP responses carry opaque store references instead of absolute host paths. The kernel still calls no LLM.
