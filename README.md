...
| 6   | Browse your memory as a graph                     | `o2b brain explorer` opens a force-directed HTML graph of every rule, every link, every retirement. Preference files can now declare typed relationships such as `depends_on:` and `refines:`, plus `memory_layer` / `memory_branch` labels and optional `schema_type:` taxonomy metadata; `o2b brain schema` audits those tokens without changing the Markdown source of truth. Double-click a node — Obsidian opens the underlying note.                                                               |
...
o2b brain schema --json               # Runtime schema vocabulary + usage report
