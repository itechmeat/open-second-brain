...
Open Second Brain 1.40.0 is a no-dead-ends wave: eleven units that make the system say what to do next, do it where the fix is provably mechanical, and refuse by name where it is not. Every issue `o2b brain doctor` reports now carries the exact command that acts on it, on the human surface and as a field over MCP - and where no single command resolves a class, the doctor gives the reason rather than falling silent, so an operator can tell "there is no command for this" from "the system forgot". Terminal states that used to exit zero without a word now name their next move through one rail that a ratchet keeps singular. `o2b brain dream` gained a single-step run and a one-run gate override, so a targeted pass no longer means editing `_brain.yaml` and remembering to revert it; a step that cannot run alone is refused with the coupling named. The dream journal marks a phase complete where its writes actually land instead of before they begin, `_brain.yaml` is generated from the resolver's own defaults so sixteen silently-defaulted keys are visible, and the repair planner consumes the doctor's findings instead of re-scanning behind it. Most of this release is not new machinery: reconnaissance invalidated at least one premise in eight of the ten scoped sources, and what shipped is the delivery of mechanisms that already existed and did not reach the caller.

Previous release, 1.39.0, was a context-integrity wave: owner scoping reached every content-returning surface, context packs gained a provenance stamp and an expiry, session continuation learned to abstain by name, and a frontmatter line the parser cannot read is reported instead of dropped. Details live in the [CHANGELOG](CHANGELOG.md).
