...
Open Second Brain 1.26.1 is an upstream-alignment guard release. Two regression suites now pin the Hermes-facing surface against landed upstream Hermes changes: the memory-provider tool schemas are proven flat and named so the core normalize/validate-before-wrap step never disables the toolset on a strict provider, and the in-place compaction lifecycle is shown to flush each boundary exactly once under one stable session id. No runtime behavior changes, no source edits; the kernel still calls no LLM.
