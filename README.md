...
Open Second Brain 1.24.0 is a hardening and correctness pass. The search index rebuild is now guarded by a separate reindex lock so a concurrent or interrupted reindex cannot swap an empty or stale database over the live index, and an unreadable index self-heals instead of surfacing a raw read error. Wikilink resolution runs over a new `documents.basename` column and index instead of a per-link SUBSTR scan, and the entity, backlink, and heal hot paths are memoized so they are not rebuilt on every query. Continuity records now require a full canonical timestamp, card snippet truncation no longer splits an astral character into a lone replacement character, and `since` / `until` filtering handles second precision consistently. The secret-exec audit trail redacts the command field before it is persisted, and the CJK bigram tokenizer stays inside contiguous Han spans. The `basename` column requires a schema bump, so every existing install reindexes once on next open. The kernel still calls no LLM.
