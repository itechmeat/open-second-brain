...
Open Second Brain 1.56.0 teaches this install to answer questions about itself. `o2b version` prints the installed version and `--version` is routed to the same renderer rather than printing a line of its own, because two spellings of one question that answer differently are worse than one spelling; `--json` renders `{"version":"…"}` for a caller that parses, and `o2b version latest` is a usage error rather than a print of the local number, because nothing in this build learns what any other version is. The MCP handshake has reported that same fact as `serverInfo.version` since the server existed, and both surfaces now read one constant that the tree imports exactly once, held there by a census with a positive control. `second_brain_wiring` puts two readers that had CLI-only consumers on the MCP surface, behind a required `view` and with no aggregate member, so a projects read never pays for a host probe: `view=projects` returns one entry per registered project link with the four-value pointer state and whether the vault it names still exists, and `view=hosts` returns the per-target verdict of the same `verify()` call `o2b install --check` makes, over the same registry and the same install environment, which is now built in one place instead of two. Paths do not travel with either answer - both sides of a project link, and the host home inside an adapter's own sentences, render through the shipped `expose_host_paths` contract, opaque by default and raw only when the operator sets the flag. And the instruction block every agent reads at connect time is rendered from the runtime capability window this server has evaluated for several releases and never consulted: guidance whose tool the host disabled is removed rather than contradicted, each removal named with the reason the evaluator produced, and a paragraph whose every tool-bearing clause dropped renders nothing at all, its lead-in included, because "Five tools live here" stops being true the moment one of them is withheld. With nothing withheld, the rendered text is byte-identical to the three frozen strings it replaces, pinned by fixtures captured from them - save for one sentence in the catalog body, which claimed every tool omitted from `tools/list` stays callable through `tools/call`. That holds for the tools the catalog surface hides and not for the ones a window withholds, so it now separates the two cases.

Nothing in this release computes a new fact. Four surfaces start reading what four readers already produce, and the machinery that made them unreachable is the whole of what changed. Every card in scope was read against the live source before anything was designed, and three of the five verdicts amended the card: the version card asked to reuse an existing manifest import and understated the problem, because three independent reads of that field already existed and a fourth would have written the defect into the change meant to answer it; the linked-projects card is smaller than it states, both readers being already pure, typed and tolerant; and the connector-health card is largely refuted, the aggregate view its own validator believed was the remaining gap having already shipped as `o2b install --check`. One card was refused rather than built. Release awareness asked for a surface reporting whether a newer version is available by reading only cached update state, and there is no producer for that cache - nothing in this tree ever learns what the latest release is, filling it requires a network check, and this tool renders to the operator, verbatim, that it has no update check. Both honest alternatives amend an operator-facing promise about network behaviour, which is an operator policy decision rather than an engineering one, and it is recorded in the [CHANGELOG](CHANGELOG.md) for the operator to take rather than taken on their behalf.
...
| OpenAI Codex                                                     | `codex plugin marketplace add ...` then `o2b install --target codex --apply` (MCP servers via `codex mcp add`, or a `config.toml` merge without the binary) - [`install/codex.md`](install/codex.md) |
...
- **Who wrote what, and how to take it back.** Every note write is a log event with a write id, the agent, the device, and the content hash before and after, and the bytes it replaced are kept. `o2b brain writes` reads the record; `o2b brain writes revert --agent <name>` plans a restore of exactly that agent's writes, refuses by name where another writer sits on top, and applies only the plan you read. `o2b brain freeze` stops every content writer on every device that syncs the vault until you say otherwise, and the log carries a per-shard hash chain so a line that was edited or removed after the fact has a file and a line number.
...
- **It knows what it is, and what it is wired into.** `o2b version` answers on the command line the question the MCP handshake has always answered - one constant behind both, so the two surfaces cannot disagree. `second_brain_wiring` puts two CLI-only answers on the MCP surface: `view=projects` lists every project directory registered against this vault with the state of its pointer file, and `view=hosts` runs the same per-adapter verification `o2b install --check` runs, so an agent can ask whether the runtimes this install wired up can still be reached. Paths in those answers are opaque references unless you asked for raw ones. And the instructions block every agent reads at connect time is now rendered from what this runtime actually left callable: guidance for a tool the host disabled is removed rather than contradicted, and each removal is named with its reason.
