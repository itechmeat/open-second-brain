...
| 9   | Hybrid search that explains itself                | `o2b search "<query>" --property type=decision --property status=open --evidence-pack` — SQLite + FTS5 and an optional semantic layer, with CJK-aware token expansion for unspaced Chinese/Japanese/Korean text. Recall is sharpened by MMR diversity, link-graph traversal, entity boosting, structured query lanes, session focus, and header-anchored chunking; opt-in evidence packs show matched/missing terms, abstention text, terminal-state downranks, and `why_retrieved` reasons. |
