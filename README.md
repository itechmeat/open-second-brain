...
Open Second Brain 1.21.0 is a safety-hardening pass around the surfaces that write to or trust the vault and the codegraph. The secret redactor now enforces a fail-closed 1 MiB scan window instead of silently truncating oversized input (always on), with opt-in infrastructure-topology detectors that extend coverage beyond secrets when enabled. Before the partner surface mutates the codegraph, a read-only health check refuses the write when the index is stale or inconsistent rather than importing into a graph it cannot trust (always on). A report-only hardcoded-path check surfaces path leakage in shipped source without failing on pre-existing hits (always on), and the recall-telemetry cost meter now separates memory write volume from read volume (always on). Two quiet loss and collision gaps close as well: punctuation-only note titles get a deterministic `unnamed-<hash>` slug instead of collapsing into one shared note (always on), and session provenance is recorded as a stable logical key so a vault moved between machines keeps its lookups (always on). The opt-in pieces (the extra infra-topology detectors) are off by default and byte-identical when the new path is unused, and the kernel still calls no LLM.
