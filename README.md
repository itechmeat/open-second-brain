...
| 2 | The agent learns what you actually want | Repeat a correction three times on the same topic and it becomes a rule. The rule's confidence rises every time the agent applies it correctly and drops when it slips. No LLM inside the algorithm — counters and atomic file moves, nothing else. |
