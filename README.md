...
Open Second Brain 1.23.0 adds two optional precision layers that complete the retrieval-precision loop started in 1.22.0, each off by default with byte-identical output when unconfigured. A semantic entity dedup pass surfaces lexical entity-name variants as proposal-only alias-merge candidates when entity_semantic_dedup_enabled is set, via a deterministic jaccard layer with an embedding-cosine reader that reuses the configured embedding provider, and never rewrites the deterministic identity key. An optional cross-encoder rerank stage re-scores the top-K fused search candidates jointly against the query through an OpenAI-compatible endpoint (search_rerank_enabled, off by default), most valuable on the thorough profile, with zero HTTP cost and unchanged ordering when disabled and a graceful degrade to heuristic ordering on endpoint error. The kernel still calls no LLM.
