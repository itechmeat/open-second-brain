...
Open Second Brain 1.42.0 is a structural wave with no new capability, and one behavioural correction worth knowing about: a `_brain.yaml` that is present but malformed no longer reads as one that is absent, so a typo in your config stops being served back as the documented defaults while every surface reports healthy, and a diagnostic that could not read a directory now says so instead of rendering as a clean bill of health. The rest is the six modules that cost the most to change, chosen by measurement rather than impression - a config validator with a single twelve-hundred-line function, the search store as one class over fourteen tables, the recall pipeline as one function running twelve stages, every `o2b search` verb in one file, the vault doctor as twenty-six unrelated checks in one body, and the dream pass with its orchestration fused into the mechanics it orchestrates. Both import cycles in the graph are gone, and a ratchet now fails the build on any new one - after review proved that ratchet could pass on an empty set, which is fixed and demonstrated. Behaviour is byte-identical everywhere it is claimed to be, and where that guarantee is load-bearing it was measured rather than asserted: the validator against its predecessor over 3135 generated configs, the command line over 92 invocations with stdout, stderr and exit code diffed byte for byte, and the store's schema statements and the pipeline's cache-key composition diffed line for line, since either changing would strand data already on disk without failing a test. The [CHANGELOG](CHANGELOG.md) has the detail.
