...
describe("statOrAbsent separates absence from a failure to look", () => {
  ...
  test.skipIf(CHMOD_CANNOT_DENY)(
    "a file behind an untraversable parent raises rather than reporting absence",
    () => {
      seal();
...
describe("isFile and isDir stay lenient, and that is the reason they are separate", () => {
  // chmod cannot deny access on Windows or as root (tests/helpers/platform.ts).
  test.skipIf(CHMOD_CANNOT_DENY)(
    "both answer false for the unreachable file without raising",
    () => {
      seal();
...
describe("doctor names an unreadable manifest instead of calling it missing", () => {
  // chmod cannot deny access on Windows or as root (tests/helpers/platform.ts).
  test.skipIf(CHMOD_CANNOT_DENY)(
    "a JSON manifest behind an untraversable parent is reported unreadable",
    () => {
      seal();
  ...
  // chmod cannot deny access on Windows or as root (tests/helpers/platform.ts).
  test.skipIf(CHMOD_CANNOT_DENY)("the Hermes manifest check makes the same distinction", () => {
  ...
  // chmod cannot deny access on Windows or as root (tests/helpers/platform.ts).
  test.skipIf(CHMOD_CANNOT_DENY)(
    "an unreadable OpenClaw extension entry is not reported as missing",
    () => {
