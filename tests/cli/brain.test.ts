...
describe("brain feedback", () => {
  test("writes a signal file with all required fields", async () => {
    ...
    const inbox = readdirSync(join(vault, "Brain", "inbox")).filter((n) =>
      n.endsWith(".md"),
    );
...
describe("brain reject", () => {
  test("moves a pref to retired/", async () => {
    ...
    expect(
      existsSync(join(vault, "Brain", "preferences", "pref-obsolete.md")),
    ).toBe(false);
    expect(existsSync(join(vault, "Brain", "retired", "ret-obsolete.md"))).toBe(
      true,
    );
  });
  ...
  test("pinned pref refuses without --yes", async () => {
    ...
    expect(
      existsSync(join(vault, "Brain", "preferences", "pref-important.md")),
    ).toBe(true);
  });
  ...
  test("pinned pref retires with --yes", async () => {
    ...
    expect(r.returncode).toBe(0);
    expect(
      existsSync(join(vault, "Brain", "retired", "ret-important.md")),
    ).toBe(true);
...
describe("brain rollback", () => {
  ...
  test("aborts on drift without --force-rollback (§5-tail)", async () => {
    ...
    const r = await runCli(
      ["brain", "rollback", "--vault", vault, dpayload.run_id, "--yes"],
      {
        env: { OPEN_SECOND_BRAIN_CONFIG: config },
      },
    );
...
describe("brain scan-inline", () => {
  test("finds inline marker, creates signal, rewrites source file", async () => {
    ...
    const r = await runCli(
      ["brain", "scan-inline", "--vault", vault, "--path", "Daily"],
      {
        env: { OPEN_SECOND_BRAIN_CONFIG: config },
      },
    );
  ...
  test("--dry-run does not write signals or rewrite files", async () => {
    ...
    const inbox = join(vault, "Brain", "inbox");
    expect(readdirSync(inbox).filter((n) => n.startsWith("sig-")).length).toBe(
      0,
    );
  });
