...
- **Codex on Windows** runs a hook as `%COMSPEC% /C "<command>"`, which
  cannot parse the POSIX form, so the Codex mirror
  (`plugins/codex/hooks/hooks.json`, generated by
  `bun run sync-plugin-mirrors`) gives every hook a `commandWindows`:
  `set NoDefaultCurrentDirectoryInExePath=1& where /q $PATH:o2b-hook && o2b-hook <name> & exit /b 0`.
  It runs the PATH `o2b-hook.cmd` that `o2b install-cli` writes, never
  one in the project Codex opened, and always exits `0`.
