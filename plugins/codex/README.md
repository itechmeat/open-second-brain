...
**Native Windows** (10/11, x64) is supported alongside Linux and macOS: Bun from `install.ps1`, `.cmd` launchers instead of symlinks, config under `%LOCALAPPDATA%` - [`install/windows.md`](https://github.com/itechmeat/open-second-brain/blob/main/install/windows.md).
...
- **Knowledge you can share without sharing the vault.** `o2b brain knowledge-pack export --select topic:…|tag:…|<id>` packs a chosen subset - rules and the pages that hold runbooks and conventions - into an OKF-based bundle sealed with sha256, blocking pages marked `visibility:` or carrying an `owner:` and redacting secrets on the way out. The receiving vault can `preview` it (manifest, samples, integrity, conflicts, prompt-injection warnings), `install` it as untrusted candidates - rules arrive unconfirmed on a fresh trial window, pages stage in `OKF Review/`, each stamped `knowledge_pack: <name>@<sha>` - and `uninstall` it as a unit behind a snapshot. See [docs/cli-reference.md](https://github.com/itechmeat/open-second-brain/blob/main/docs/cli-reference.md#knowledge-packs).
